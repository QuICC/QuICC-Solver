...
#define DECL_MUMPS(PREFIX, UPREFIX, KEYTYPE)                                   \
   ...
   inline void MumpsLU_mumps(MumpsStrucType<KEYTYPE>::StrucType* idptr,        \
      KEYTYPE)                                                                 \
   {                                                                           \
...
inline float MumpsLU_convert(const float val)
{
   return val;
};
inline double MumpsLU_convert(const double val)
{
   return val;
};
inline mumps_complex MumpsLU_convert(const std::complex<float> val)
{
   return (mumps_complex){.r = val.real(), .i = val.imag()};
};
inline mumps_double_complex MumpsLU_convert(const std::complex<double> val)
{
   return (mumps_double_complex){.r = val.real(), .i = val.imag()};
};
inline std::complex<float> MumpsLU_convert(const mumps_complex val)
{
   return std::complex<float>(val.r, val.i);
};
inline std::complex<double> MumpsLU_convert(const mumps_double_complex val)
{
   return std::complex<double>(val.r, val.i);
};

template <typename TMatrixType> class MumpsLU : internal::noncopyable
{
public:
...
public:
   ...
   inline void setIterativeRefinement(const int iterations)
   {
   ...
   {
      return m_id.infog[16 - 1];
   }
   inline int getTotalMemory()
   {
      return m_id.infog[17 - 1];
   }
   ...
   inline const internal::solve_retval<MumpsLU, Rhs> solve(
      const MatrixBase<Rhs>& b) const
   {
      eigen_assert(m_isInitialized && "MumpsLU is not initialized.");
      eigen_assert(
         rows() == b.rows() && "MumpsLU::solve(): invalid number of rows of "
    ...
    */
   template <typename Rhs>
   inline const internal::sparse_solve_retval<MumpsLU, Rhs> solve(
    ...
    *
    * The given matrix must have the same sparcity than the matrix on which the
    * pattern analysis has been performed.
    *
   ...
   void factorize(const MatrixType& matrix)
   {
      ...
      eigen_assert(
         m_analysisIsOk && "MumpsLU: you must first call analyzePattern()");
...
protected:
   ...
   {
      eigen_assert(
         mat.rows() == mat.cols() && "MumpsLU requires a square matrix");

      if ((!m_isParallel && m_id.a != 0 && freeMemory) ||
          (m_isParallel && m_id.a_loc != 0 && freeMemory))
      {
         if (m_isParallel)
         {
            free(m_id.irn_loc);
      ...
      if (needCopy)
      {
         ...
         if (m_isParallel)
         {
            ...
            m_id.a_loc =
               (MumpsScalar*)malloc(sizeof(MumpsScalar) * m_id.nz_loc);
   ...
   void copyRhs(const Scalar* pData, const bool reallocRhs) const
   {
      ...
      if (m_isParallel)
      {
         ...
         MPI_Reduce(const_cast<Scalar*>(pData), mTmp.data(),
            m_id.nrhs * m_id.lrhs, QuICC::Environment::MpiTypes::type<Scalar>(),
            MPI_SUM, 0, m_comm);
         pRhs = mTmp.data();
      ...
      if (m_isHost)
      {
         ...
         if (reallocRhs)
         {
            free(m_id.rhs);
            m_id.rhs =
               (MumpsScalar*)malloc(sizeof(MumpsScalar) * m_id.nrhs * m_id.n);
            m_nrhsMem = m_id.nrhs;
         }
   ...
   void copySolution(Scalar* pData) const
   {
      ...
      {
         DetailedProfilerMacro_start(QuICC::ProfilerMacro::TSTEPMPI);
         MPI_Bcast(pData, nK, QuICC::Environment::MpiTypes::type<Scalar>(), 0,
            m_comm);
         DetailedProfilerMacro_stop(QuICC::ProfilerMacro::TSTEPMPI);
      }
#endif // QUICC_MPI
...
template <typename BDerived, typename XDerived>
bool MumpsLU<MatrixType>::_solve(const MatrixBase<BDerived>& b,
   MatrixBase<XDerived>& x) const
{
   ...
   eigen_assert((BDerived::Flags & RowMajorBit) == 0 &&
                "MumpsLU backend does not support non col-major rhs yet");
   eigen_assert((XDerived::Flags & RowMajorBit) == 0 &&
                "MumpsLU backend does not support non col-major result yet");
#ifdef QUICC_MPI
   eigen_assert((!m_isParallel || b.cols() == 1) &&
                "Parallel MumpsLU is implemented for a single RHS");
   eigen_assert(b.derived().data() != x.derived().data() &&
                "Parallel execution does not allow x and b to be the same");
#endif // QUICC_MPI
