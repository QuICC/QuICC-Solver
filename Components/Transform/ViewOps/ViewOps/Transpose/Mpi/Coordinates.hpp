...
std::vector<point_t> getCoo(const Tv& view)
{
    ...
    else if constexpr(std::is_same_v<typename Tv::AttributesType, DCCSC3D> && std::is_same_v<Perm, p120_t>)
    {
        std::size_t itCoo = 0;
        for (std::size_t ptr = 0; ptr < pointers.size()-1; ++ptr)
        {
    ...
    else if constexpr(std::is_same_v<typename Tv::AttributesType, S1CLCSC3D> && std::is_same_v<Perm, p120_t>)
    {
        std::size_t itCoo = 0;
        for (std::size_t ptr = 0; ptr < pointers.size()-1; ++ptr)
        {
