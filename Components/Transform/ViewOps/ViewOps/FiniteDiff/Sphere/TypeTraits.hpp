...
/// @brief Generic template to check if an operator is in CSC format
/// @tparam TView
/// @tparam TDirection
template <class TView, class TDirection> struct is_csc : std::false_type
{
};

/// @brief Specialization for col major projector
template <class TDirection> struct is_csc<cscop_t, TDirection> : std::true_type
{
};
...
/// @tparam TDirection
template <class TView, class TDirection>
inline constexpr bool is_csc_v = is_csc<TView, TDirection>::value;
