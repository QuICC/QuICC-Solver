...
DiffOp<Tout, Tin, Order, Direction, Treatment>::DiffOp(ScaleType scale) :
    mScale(scale){};

template <class Tout, class Tin, std::size_t Order, class Direction,
   std::uint16_t Treatment>
void DiffOp<Tout, Tin, Order, Direction, Treatment>::applyImpl(Tout& out,
   const Tin& in, const ScaleType fftScaling)
{
   ...
   if constexpr (std::is_same_v<Direction, bwd_t> && Treatment == none_m &&
                 Order == 0)
   {
   ...
   using float_t =
      typename DiffOp<Tout, Tin, Order, Direction, Treatment>::ScaleType;
   ...
   const std::size_t mStart =
      dealias::getMstart<Tout, Tin, Order, Direction, Treatment>(out, in);
   ...
   for (std::size_t col = 0; col < columns; ++col)
   {
      ...
      {
         assert(nk + m < out.size());
         out.data()[nk + m] = in.data()[nk + m] * c *
                              static_cast<float_t>(sgn) *
