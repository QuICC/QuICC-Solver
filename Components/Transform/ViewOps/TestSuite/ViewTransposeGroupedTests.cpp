...
TEST_CASE("Serial DCCSC3D to DCCSC3D 201", "SerialDCCSC3DtoDCCSC3D201")
{
   ...
   std::vector<View<double, outTy>> viewsOut;
   std::vector<View<double, inTy>> viewsIn;
   viewsOut.push_back(viewOut);
   viewsIn.push_back(viewIn);

   // Transpose op
   using namespace QuICC::Transpose::Cpu;
   using namespace QuICC::Transpose;
   auto transposeOp =
      std::make_unique<OpGrouped<std::vector<View<double, outTy>>,
         std::vector<View<double, inTy>>, p201_t>>();
