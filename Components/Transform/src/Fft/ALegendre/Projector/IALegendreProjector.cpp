 ...
 * @brief Source of the interface for a generic FFT based ALegendre projector
 */
...
namespace Projector {
   ...
   void IALegendreProjector::initBackend() const
   {
   ...
   void IALegendreProjector::transformBlock(MatrixZ& rOut, const MatrixZ& in, const bool isEven, const bool useReal) const
   {
   ...
   void IALegendreProjector::transformBlock(Matrix& rOut, const Matrix& in, const bool isEven) const
   {
      Profiler::RegionStart<5> (this->mProfileTag + "-pre");
   ...
   void IALegendreProjector::transform(MatrixZ& rOut, const MatrixZ& in) const
   {
   ...
   void IALegendreProjector::transform(Matrix& rOut, const Matrix& in) const
   {
      Profiler::RegionFixture<2> fix(this->mProfileTag + "::transform");
   ...
   void IALegendreProjector::transform(Matrix& rOut, const MatrixZ& in) const
   {
      IALegendreOperator::transform(rOut, in);
   }

   void IALegendreProjector::transform(MatrixZ& rOut, const Matrix& in) const
   {
      IALegendreOperator::transform(rOut, in);
   }
