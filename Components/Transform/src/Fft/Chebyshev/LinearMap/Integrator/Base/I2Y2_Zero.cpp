...
void I2Y2_Zero<base_t>::initOperator() const
{
   int size =
      this->mspSetup->specSize() + std::min(1, this->mspSetup->padSize());
   ::QuICC::SparseSM::Chebyshev::LinearMap::I2Y2 op(size, size,
...
void I2Y2_Zero<base_t>::applyPostOperator(Eigen::Ref<Matrix> rOut) const
{
   this->mBackend.outputSpectral(rOut);
}

void I2Y2_Zero<base_t>::applyPreOperator(Matrix& tmp, const Eigen::Ref<const MatrixZ>& in,
   const bool useReal) const
{
   this->mBackend.input(tmp, in, useReal);
}

void I2Y2_Zero<base_t>::applyPostOperator(Eigen::Ref<MatrixZ> rOut, const Matrix& tmp,
   const bool useReal) const
{
