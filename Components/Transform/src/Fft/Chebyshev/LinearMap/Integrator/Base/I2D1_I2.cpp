...
void I2D1_I2<base_t>::applyPostOperator(Matrix& rOut) const
{
   this->mBackend.outputSpectral(rOut);
}

void I2D1_I2<base_t>::applyPreOperator(Matrix& tmp, const MatrixZ& in,
   const bool useReal) const
{
   this->mBackend.input(tmp, in, useReal);
}

void I2D1_I2<base_t>::applyPostOperator(MatrixZ& rOut, const Matrix& tmp,
   const bool useReal) const
{
