...
namespace Integrator {
   ...
   void P_Zero<base_t>::applyPostOperator(Matrix& rOut) const
   {
      this->mBackend.output(rOut);
   }

   void P_Zero<base_t>::applyPreOperator(Matrix& tmp, const MatrixZ& in, const bool useReal) const
   {
      this->mBackend.input(tmp, in, useReal);
   }

   void P_Zero<base_t>::applyPostOperator(MatrixZ& rOut, const Matrix& tmp, const bool useReal) const
   {
