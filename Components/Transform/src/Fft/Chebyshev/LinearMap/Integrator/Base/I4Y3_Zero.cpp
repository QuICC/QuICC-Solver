/**
 * @file I4Y3_Zero.cpp
 * @brief Source of the implementation of the  Chebyshev I^4 Y^4 of 1/Y
...
void I4Y3_Zero<base_t>::initOperator() const
{
   int size =
      this->mspSetup->specSize() + std::min(7, this->mspSetup->padSize());
   ::QuICC::SparseSM::Chebyshev::LinearMap::I4Y3 op(size, size,
...
void I4Y3_Zero<base_t>::applyPostOperator(Eigen::Ref<Matrix> rOut) const
{
   this->mBackend.outputSpectral(rOut);
}

void I4Y3_Zero<base_t>::applyPreOperator(Matrix& tmp, const Eigen::Ref<const MatrixZ>& in,
   const bool useReal) const
{
   this->mBackend.input(tmp, in, useReal);
}

void I4Y3_Zero<base_t>::applyPostOperator(Eigen::Ref<MatrixZ> rOut, const Matrix& tmp,
   const bool useReal) const
{
