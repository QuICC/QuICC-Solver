 ...
 * @brief Source of the implementation of the  Chebyshev I2R2 of 1/R D R
...
void I2Y1D1Y1_Zero<base_t>::initOperator() const
{
   int size =
      this->mspSetup->specSize() + std::min(1, this->mspSetup->padSize());
   ::QuICC::SparseSM::Chebyshev::LinearMap::I2Y1D1Y1 op(size, size,
...
void I2Y1D1Y1_Zero<base_t>::applyPostOperator(Eigen::Ref<Matrix> rOut) const
{
   this->mBackend.outputSpectral(rOut);
}

void I2Y1D1Y1_Zero<base_t>::applyPreOperator(Matrix& tmp, const Eigen::Ref<const MatrixZ>& in,
   const bool useReal) const
{
   this->mBackend.input(tmp, in, useReal);
}

void I2Y1D1Y1_Zero<base_t>::applyPostOperator(Eigen::Ref<MatrixZ> rOut, const Matrix& tmp,
   const bool useReal) const
{
