...
void ChebyshevIntegrator::init(const SetupType& setup) const
{
   ...
   for (int i = 0; i < this->mNStreams; i++)
   {
      ...
      cudaMalloc((void**)&(this->mcuBwd.back()),
         sizeof(cufftDoubleComplex) * cuBwdSize * this->mBlockSize);
      cudaMalloc((void**)&(this->mcuFwd.back()),
         sizeof(cufftDoubleReal) * this->mFwdSize * this->mBlockSize);
      cudaMalloc((void**)&(this->mcuWork.back()),
         sizeof(cufftDoubleReal) * this->mFwdSize * this->mBlockSize);
      ...
      cufftHandle plan;
      // Create the complex to real plan
      if (cufftPlanMany(&plan, 1, fftSize, NULL, 1, 0, NULL, 1, 0, CUFFT_D2Z,
             this->mBlockSize) != CUFFT_SUCCESS)
      {
...
void ChebyshevIntegrator::io(double* out, const double* in) const
{
   ...
   new (&this->mOutMap)
      Eigen::Map<Matrix>(this->mpOut, this->mBwdSize, this->mBlockSize);
}
...
void ChebyshevIntegrator::applyFft() const
{
   ...
   for (int i = 0; i < this->mNBatches; i++)
   {
      ...
      cudaMemcpyAsync(this->mcuFwd.at(sid), this->mpIn + fshift,
         sizeof(cufftDoubleReal) * this->mFwdSize * this->mBlockSize,
         cudaMemcpyHostToDevice, this->mStreams.at(sid));
      fshift += this->mFwdSize * this->mBlockSize;
      Gpu::buildDCTInput<<<1, kblocks, 0, this->mStreams.at(sid)>>>(
         this->mcuWork.at(sid), this->mcuFwd.at(sid), this->mFwdSize,
         this->mBlockSize);
      cufftExecD2Z(this->mPlans.at(sid), this->mcuWork.at(sid),
         this->mcuBwd.at(sid));
      Gpu::buildDCTOutput<<<1, kblocks, 0, this->mStreams.at(sid)>>>(
         this->mcuWork.at(sid), this->mcuBwd.at(sid), this->mBwdSize,
         this->mBlockSize);
      cudaMemcpyAsync(this->mpOut + bshift, this->mcuWork.at(sid),
         sizeof(cufftDoubleReal) * this->mBwdSize * this->mBlockSize,
         cudaMemcpyDeviceToHost, this->mStreams.at(sid));
...
void ChebyshevIntegrator::output(Matrix& rOut) const
{
   ...
   {
      rOut.block(0, 0, this->mSpecSize, 1) =
         this->mFftScaling * this->mMeanOp *
         this->mOutMap.block(0, 0, this->mMeanOp.cols(), 1);
      rOut.block(0, 1, this->mSpecSize, rOut.cols() - 1) =
         this->mFftScaling *
         this->mOutMap.block(0, 1, this->mSpecSize, rOut.cols() - 1);
   }
   else
   {
      rOut.topRows(this->mSpecSize) =
         this->mFftScaling * this->mOutMap.topRows(this->mSpecSize);
   }
}
...
void ChebyshevIntegrator::outputSpectral(Matrix& rOut) const
{
   if (this->mMeanOp.size() > 0)
   {
      rOut.block(0, 0, this->mSpecSize, 1) =
         ...
         this->mOutMap.block(0, 0, this->mMeanOp.cols(), 1);
      rOut.block(0, 1, this->mSpecSize, rOut.cols() - 1) =
         this->mFftScaling * this->mSpecOp *
         this->mOutMap.block(0, 1, this->mSpecOp.cols(), rOut.cols() - 1);
   }
   else
   {
      rOut.topRows(this->mSpecSize) =
         this->mFftScaling * this->mSpecOp *
         this->mOutMap.topRows(this->mSpecOp.cols());
   }
}
...
void ChebyshevIntegrator::output(MatrixZ& rOut, const bool useReal) const
{
   if (this->mMeanOp.size() > 0)
   {
      if (useReal)
      {
         rOut.block(0, 0, this->mSpecSize, 1).real() =
            this->mFftScaling * this->mMeanOp *
            this->mOutMap.block(0, 0, this->mMeanOp.cols(), 1);
         rOut.block(0, 1, this->mSpecSize, rOut.cols() - 1).real() =
            this->mFftScaling *
            this->mOutMap.block(0, 1, this->mSpecSize, rOut.cols() - 1);
      }
      else
      {
         rOut.block(0, 0, this->mSpecSize, 1).imag() =
            this->mFftScaling * this->mMeanOp *
            this->mOutMap.block(0, 0, this->mMeanOp.cols(), 1);
         rOut.block(0, 1, this->mSpecSize, rOut.cols() - 1).imag() =
            this->mFftScaling *
            this->mOutMap.block(0, 1, this->mSpecSize, rOut.cols() - 1);
      }
   }
   else
   {
      if (useReal)
      {
         rOut.topRows(this->mSpecSize).real() =
            this->mFftScaling * this->mOutMap.topRows(this->mSpecSize);
      }
      else
      {
         rOut.topRows(this->mSpecSize).imag() =
            this->mFftScaling * this->mOutMap.topRows(this->mSpecSize);
      }
   }
}

void ChebyshevIntegrator::outputSpectral(MatrixZ& rOut,
