...
void ChebyshevIntegrator::init(const SetupType& setup) const
{
   ...
   this->mPlan = fftw_plan_many_r2r(1, fftSize, blockSize, tmpF.data(), NULL, 1,
      fwdSize, tmpB.data(), NULL, 1, bwdSize, fwdKind,
...
void ChebyshevIntegrator::output(Matrix& rOut) const
{
   if (this->mMeanOp.size() > 0)
   {
      rOut.block(0, 0, this->mSpecSize, 1) =
         ...
         rOut.block(0, 0, this->mMeanOp.cols(), 1);
      rOut.block(0, 1, this->mSpecSize, rOut.cols() - 1) =
         this->mFftScaling * rOut.block(0, 1, this->mSpecSize, rOut.cols() - 1);
   }
   else
   {
      rOut.topRows(this->mSpecSize) =
         this->mFftScaling * rOut.topRows(this->mSpecSize);
   }
}
...
void ChebyshevIntegrator::outputSpectral(Matrix& rOut) const
{
   if (this->mMeanOp.size() > 0)
   {
      ...
      rOut.block(0, 1, this->mSpecSize, rOut.cols() - 1) =
         ...
         rOut.block(0, 1, this->mSpecOp.cols(), rOut.cols() - 1);
   }
   else
   {
      rOut.topRows(this->mSpecSize) =
         this->mFftScaling * this->mSpecOp * rOut.topRows(this->mSpecOp.cols());
   }
}
   ...
   if (this->mMeanOp.size() > 0)
   {
      if (useReal)
      {
         rOut.block(0, 0, this->mSpecSize, 1).real() =
            this->mFftScaling * this->mMeanOp *
            tmp.block(0, 0, this->mMeanOp.cols(), 1);
         rOut.block(0, 1, this->mSpecSize, rOut.cols() - 1).real() =
            this->mFftScaling *
            tmp.block(0, 1, this->mSpecSize, rOut.cols() - 1);
      }
      else
      {
   ...
   else
   {
      if (useReal)
      {
         rOut.topRows(this->mSpecSize).real() =
            this->mFftScaling * tmp.topRows(this->mSpecSize);
      }
      else
      {
   ...
   const bool useReal) const
{
   if (this->mMeanOp.size() > 0)
   {
      if (useReal)
      {
         ...
         rOut.block(0, 1, this->mSpecSize, rOut.cols() - 1).real() =
            this->mFftScaling * this->mSpecOp *
            tmp.block(0, 1, this->mSpecOp.cols(), rOut.cols() - 1);
      }
      else
      {
         rOut.block(0, 0, this->mSpecSize, 1).imag() =
