 ...
 * @brief Source of the interface for a generic FFTW based ALegendre projector
 */
...
void ALegendreProjector::init(const SetupType &setup, const int lshift,
...
#endif
  // Initialize parent
  IALegendreBackend::init(setup, lshift, extraN, lshiftOnlyParity,
                        alwaysZeroNegative);
...
int ALegendreProjector::lSize(const int l) const { return this->mWSize - l / 2; }

int ALegendreProjector::lSize(const int i, const int) const {
...
void ALegendreProjector::input(const MatrixZ &in) const {
  ...
  /* assert(
   ...
   IALegendreBackend::transferDataToGPU(&bufferSolveRes2, (void*)in.data(), 0,
     0,
...
void ALegendreProjector::input(const Matrix &in, const bool isEven,
...
void ALegendreProjector::input(const MatrixZ &in, const bool isEven,
                             const bool useReal, const bool needPadding) const {
  ALegendreProjector::input(in);
}

void ALegendreProjector::output(MatrixZ &rOut) const {
...
void ALegendreProjector::output(MatrixZ &rOut, const bool isEven,
                              const bool useReal) const {
  ALegendreProjector::output(rOut);
}

void ALegendreProjector::applyPadding(Matrix &rData, const int extraRows) const {
  if (extraRows >= 0) {
...
void ALegendreProjector::partialBackwardALegendre(
...
void ALegendreProjector::backwardALegendre(const bool isEven0,
  ...
  for (int isEven = 0; isEven < 2; isEven++) {
    ...
    if (pL->begin() != pL->end()) {
      ...
      for (auto loc = pL->rbegin(); loc != pL->rend(); ++loc) {
        l = std::get<4>(*loc);
        this->partialBackwardALegendre(l, id, maxL / 2 - 1, start, cols, banded,
                                     bandedGPU, inTmp);
        maxL = l;
        cols += std::get<2>(*loc);
        start -= std::get<2>(*loc);
...
void ALegendreProjector::lowerAlpha(const MHDFloat alpha, const bool isEven0,
...
void ALegendreProjector::lowerBeta(const MHDFloat alpha, const bool isEven0,
                                 const unsigned int id,
                                 const MHDFloat norm) const {
  PfSolve::PfSolveResult resSolve = PfSolve::PFSOLVE_SUCCESS;
  for (int isEven = 0; isEven < 2; isEven++) {
...
void ALegendreProjector::lowerR2Beta(const MHDFloat alpha, const bool isEven0,
                                   const unsigned int id,
                                   const MHDFloat norm) const {
  PfSolve::PfSolveResult resSolve = PfSolve::PFSOLVE_SUCCESS;
  for (int isEven = 0; isEven < 2; isEven++) {
...
void ALegendreProjector::initBanded(const bool isEven) const {
...
void ALegendreProjector::freeBanded(const bool isEven) const {
  if (this->pLoc(isEven, 0)->size() > 0) {
    std::vector<void *> *pBandedGPU;
    if (isPhysEven(isEven)) {
      pBandedGPU = &this->mEBandedGPU;
