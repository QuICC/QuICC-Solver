/**
 * @file I3_Zero.cpp
 * @brief Source of the implementation of the Worland I3 integrator
 */
...
namespace Integrator {
   ...
   I3_Zero<base_t>::I3_Zero()
   {
      this->setProfileTag();
   }

   void I3_Zero<base_t>::makeOperator(Matrix& op, const Internal::Array& igrid, const Internal::Array& iweights, const int i) const
   {
      ...
      } else
      {
         ...
         auto a = wnl.alpha(l);
         auto b = wnl.dBeta();
         ::QuICC::SparseSM::Worland::I3 spasm(nN, nN, a, b, l, 1*this->mcTruncQI);
         tOp = (spasm.mat()*tOp.transpose()).transpose();
         op = tOp.cast<MHDFloat>().leftCols(nPoly);
      }
   }

   void I3_Zero<base_t>::applyOperator(Eigen::Ref<MatrixZ> rOut, const int i, const Eigen::Ref<const MatrixZ>& in) const
   {
