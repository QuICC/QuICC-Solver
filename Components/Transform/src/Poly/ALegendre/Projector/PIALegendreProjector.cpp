...
namespace Projector {
   ...
   PIALegendreProjector<OpTypes>::PIALegendreProjector()
      : IALegendreProjector<OpTypes>()
   {
   }
   ...
   template <typename OpTypes>
   void PIALegendreProjector<OpTypes>::initSpecial() const {}

   template<typename OpTypes>
   int PIALegendreProjector<OpTypes>::outRows() const
   {
      return this->mspSetup->fwdSize();
   }

   template<typename OpTypes>
   int PIALegendreProjector<OpTypes>::outCols() const
   {
      return this->mspSetup->blockSize();
   }

   template<typename OpTypes>
   void PIALegendreProjector<OpTypes>::initOperators(const OpArray& igrid, const OpArray& iweights) const
   {
   ...
   template <typename OpTypes>
   void PIALegendreProjector<OpTypes>::applyOperators(
      OpMatrixZ &rOut, const OpMatrixZ &in) const
   {
      Profiler::RegionFixture<3> fix("PIALegendreProjector::applyOperators");
      ...
      Profiler::RegionStart<4>("PIALegendreProjector::hostScan");
      ...
      Profiler::RegionStop<4>("PIALegendreProjector::hostScan");
      ...
      Profiler::RegionStart<4>("PIALegendreProjector::applyUnitOperator");

      this->applyUnitOperator(rOutView, inView, scan, total);

      Profiler::RegionStop<4>("PIALegendreProjector::applyUnitOperator");

      DeepCopyEigen(rOut, rOutView, col_size);
#endif
   }

   template class PIALegendreProjector<PIALegendreOperatorTypes>;
   template class PIALegendreProjector<CudaIALegendreOperatorTypes>;
}
}
}
}
