/**
 * @file IBesselIntegrator.cpp
 * @brief Source of the interface to a spherical Bessel based integrator
 */
...
IBesselIntegrator::IBesselIntegrator() :
    IBesselOperator()
   {
      this->mProfileTag += "-Integrator";
   }

   void IBesselIntegrator::initOperators(const Internal::Array& igrid, const Internal::Array& iweights) const
   {
   ...
   void IBesselIntegrator::applyOperators(MatrixZ& rOut, const MatrixZ& in) const
   {
   ...
   void IBesselIntegrator::applyOperators(Matrix& rOut, const MatrixZ& in) const
   {
      throw std::logic_error("Interface not used");
   }

   MHDFloat IBesselIntegrator::requiredStorage() const
   {
      MHDFloat mem = 0.0;
   ...
   void IBesselIntegrator::defaultApplyOperator(OpMatrixR rOut, const int i, const OpMatrixCR& in) const
   {
      rOut = this->mOps.at(i).transpose()*in;
   }

   int IBesselIntegrator::outRows() const
   {
      return this->mspSetup->fastSize(0);
   }

   int IBesselIntegrator::outCols() const
   {
