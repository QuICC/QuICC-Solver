/**
 * @file Transform.cpp
 * @brief Source of the implementation of the spherical Bessel transform
 */
...
namespace Bessel {
   ...
   Array Transform::generateGrid(const int size)
   {
      ...
      Polynomial::Quadrature::SphericalBesselRule quad;
   ...
   void Transform::transform(MatrixZ& rOut, const MatrixZ& in, const IBesselOperator& op)
   {
      if(!op.isInitialized())
      {
   ...
   void Transform::transform(Matrix& rOut, const MatrixZ& in, const IBesselOperator& op)
   {
      if(!op.isInitialized())
      {
         op.init(this->mspSetup, this->mIGrid, this->mIWeights);
