/**
 * @file Tester.hpp
 * @brief Tester for Finite Difference Sphere transforms
 */

#ifndef QUICC_TESTSUITE_TRANSFORM_FINITEDIFF_TESTER_HPP
#define QUICC_TESTSUITE_TRANSFORM_FINITEDIFF_TESTER_HPP
...
namespace FiniteDiff {
   ...
   template <typename TOp, typename TOp2 = void> class Tester: public Transform::TesterBase<TOp>
   {
      ...
      private:
         ...
         /**
          * @brief Initialize FD operator
          */
         template <typename T> void initOperator(T& op, Internal::Array& igrid, const transf::FiniteDiff::SharedSetup spSetup) const;
   ...
   template <typename TOp, typename TOp2> template <typename T> void Tester<TOp,TOp2>::initOperator(T& op, Internal::Array& igrid, const transf::FiniteDiff::SharedSetup spSetup) const
   {
      // Create quadrature
      ::QuICC::FiniteDiff::Sphere::UniformRadialGrid quad;
      quad.computeGrid(igrid, spSetup->fwdSize());

      op.init(spSetup, igrid);
   }
   ...
   template <typename TOp, typename TOp2> std::shared_ptr<typename TOp::SetupType> Tester<TOp,TOp2>::buildSetup(const ParameterType& param, const TestType type) const
   {
      ...
      assert(specN == physN);
      int nModes = meta(2);
      auto spSetup = std::make_shared<typename TOp::SetupType>(physN, GridPurpose::SIMULATION);
...
#endif //QUICC_TESTSUITE_TRANSFORM_FINITEDIFF_TESTER_HPP
