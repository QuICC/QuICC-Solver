...
#ifndef QUICC_TESTSUITE_TRANSFORM_BESSEL_TESTER_HPP
#define QUICC_TESTSUITE_TRANSFORM_BESSEL_TESTER_HPP
...
namespace Bessel {
   ...
   template <typename TOp, typename TOp2 = void> class Tester: public Transform::TesterBase<TOp>
   {
      public:
          ...
          * @param keepData   Keep computed data?
          */
         Tester(const std::string& bname, const std::string& fname, const bool keepData);
      ...
      private:
          ...
          */
         void appendPath(const std::string& bname);
   ...
   template <typename TOp, typename TOp2> Tester<TOp,TOp2>::Tester(const std::string& bname, const std::string& fname, const bool keepData)
      : Transform::TesterBase<TOp>(fname, keepData, false)
   {
      this->appendPath(bname);
   }

   template <typename TOp, typename TOp2> void Tester<TOp,TOp2>::appendPath(const std::string& bname)
   {
      this->mPath += "Bessel/" + bname + "/";
   }
   ...
   template <typename TOp, typename TOp2> template <typename T> void Tester<TOp,TOp2>::initOperator(T& op, Internal::Array& igrid, const transf::Poly::SharedSetup spSetup) const
   {
      ...
      ::QuICC::Polynomial::Quadrature::SphericalBesselRule quad;
...
#endif //QUICC_TESTSUITE_TRANSFORM_BESSEL_TESTER_HPP
