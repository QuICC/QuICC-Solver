/**
 * @file Transform.hpp
 * @brief Implementation of the FFT based ALegendre transform
 */

#ifndef QUICC_TRANSFORM_FFT_ALEGENDRE_TRANSFORM_HPP
#define QUICC_TRANSFORM_FFT_ALEGENDRE_TRANSFORM_HPP
...
namespace ALegendre {
   ...
   /**
    * @brief Implementation of the FFT base ALegendre transform
    */
   class Transform
   {
      public:
          ...
          * @param in   Input values
          */
         void transform(Matrix& rOut, const MatrixZ& in, const IALegendreOperator& op);
   ...
   template <typename TOp> void Transform::addOperator(const std::size_t id)
   {
      ...
      if(!ret.second)
      {
         throw std::logic_error("ALegendre FFT operator could not be added to transform");
      }
   }
