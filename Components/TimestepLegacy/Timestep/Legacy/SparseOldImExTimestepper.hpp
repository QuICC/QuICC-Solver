...
#ifndef QUICC_TIMESTEP_LEGACY_SPARSEOLDIMEXTIMESTEPPER_HPP
#define QUICC_TIMESTEP_LEGACY_SPARSEOLDIMEXTIMESTEPPER_HPP
...
#include "Timestep/Legacy/IImExOldScheme.hpp"

namespace QuICC {

namespace Timestep {

/// @brief This namespace contains legacy timestepping scheme
namespace Legacy {

namespace details {
template <typename TOperator, typename TData>
void computeMVPAY(TData& y, const TOperator& A, const TData& x,
   const MHDFloat a);

void computeMVPAY(DecoupledZMatrix& y, const SparseMatrix& A,
   const DecoupledZMatrix& x, const MHDFloat a);


template <typename TOperator, typename TData>
void computeRHSNoFieldMemory(const int step, TData& rRHS, const TOperator& mat,
   const TData& sol, std::vector<TData>& rOldNL, TData& rTmp,
   SharedIImExOldScheme spScheme);

template <typename TOperator, typename TData>
void computeRHSNoNLMemory(const int step, TData& rRHS, const TOperator& mat,
   const TData& sol, const std::vector<TOperator>& oldMat,
   std::vector<TData>& rOldSol, SharedIImExOldScheme spScheme);

template <typename TOperator, typename TData>
void computeRHSMemory(const int step, TData& rRHS, const TOperator& mat,
   const TData& sol, const std::vector<TOperator>& oldMat,
   std::vector<TData>& rOldSol, std::vector<TData>& rOldNL, TData& rTmp,
   SharedIImExOldScheme spScheme);
} // namespace details
...
class SparseOldImExTimestepper
...
public:
    ...
    */
   void buildOperators(const int idx,
...
template <typename TOperator, typename TData, template <typename> class TSolver>
SparseOldImExTimestepper<TOperator, TData, TSolver>::SparseOldImExTimestepper(
...
template <typename TOperator, typename TData, template <typename> class TSolver>
MHDFloat SparseOldImExTimestepper<TOperator, TData, TSolver>::error() const
{
   return -1.0;
}

template <typename TOperator, typename TData, template <typename> class TSolver>
bool SparseOldImExTimestepper<TOperator, TData, TSolver>::finished()
{
...
template <typename TOperator, typename TData, template <typename> class TSolver>
MHDFloat
SparseOldImExTimestepper<TOperator, TData, TSolver>::stepFraction() const
{
   return this->mspScheme->cEx(this->mStep);
}

template <typename TOperator, typename TData, template <typename> class TSolver>
bool SparseOldImExTimestepper<TOperator, TData, TSolver>::preSolve()
{
   this->mId = this->mspScheme->lhsT(this->mStep);
   MHDFloat rhsId = this->mspScheme->rhsT(0, this->mStep);

   if (this->mspScheme->fieldMemory() == 0 &&
       this->mspScheme->nonlinearMemory() == 0)
   {
      for (size_t i = this->mZeroIdx; i < this->nSystem(); i++)
      {
         details::computeMVPAY<TOperator, TData>(
            this->reg(Register::Rhs::id()).at(i), this->rRHSMatrix(rhsId, i),
            this->reg(Register::Solution::id()).at(i),
            this->mspScheme->rhsN(0, this->mStep));
      }
   }
   else if (this->mspScheme->fieldMemory() == 0)
   {
      ...
      for (size_t i = this->mZeroIdx; i < this->nSystem(); i++)
      {
         details::computeRHSNoFieldMemory<TOperator, TData>(this->mStep,
            this->reg(Register::Rhs::id()).at(i), this->rRHSMatrix(rhsId, i),
            this->reg(Register::Solution::id()).at(i), this->mOldRHS.at(i), tmp,
            this->mspScheme);
      }
   }
   else if (this->mspScheme->nonlinearMemory() == 0)
   {
      ...
      for (size_t i = this->mZeroIdx; i < this->nSystem(); i++)
      {
         details::computeRHSNoNLMemory<TOperator, TData>(this->mStep,
            this->reg(Register::Rhs::id()).at(i), this->rRHSMatrix(rhsId, i),
            this->reg(Register::Solution::id()).at(i),
            this->rOldRHSMatrices(rhsId, i), this->mOldSolution.at(i), tmp,
            this->mspScheme);
      }
   }
   else
   {
      ...
      for (size_t i = this->mZeroIdx; i < this->nSystem(); i++)
      {
         details::computeRHSMemory<TOperator, TData>(this->mStep,
            this->reg(Register::Rhs::id()).at(i), this->rRHSMatrix(rhsId, i),
            this->reg(Register::Solution::id()).at(i),
            this->rOldRHSMatrices(rhsId, i), this->mOldSolution.at(i),
            this->mOldRHS.at(i), tmp, tmp, this->mspScheme);
      }
   }
...
template <typename TOperator, typename TData, template <typename> class TSolver>
void SparseOldImExTimestepper<TOperator, TData, TSolver>::updateTimeMatrix(
   const MHDFloat dt)
{
   ...
   for (int step = 0; step < this->mspScheme->steps(); ++step)
   {
      ...
      for (std::size_t i = 0; i < this->nSystem(); ++i)
      {
         ...
         for (size_t k = 0;
              k < static_cast<size_t>(this->mTMatrix.at(i).outerSize()); ++k)
         {
            typename TOperator::InnerIterator lhsIt(
               this->rLHSMatrix(this->mspScheme->lhsT(step), i), lhsJ);
            typename TOperator::InnerIterator rhsIt(
               this->rRHSMatrix(this->mspScheme->rhsT(0, step), i), rhsJ);
            std::vector<std::shared_ptr<typename TOperator::InnerIterator>>
               oldRhsIt;
            for (int r = 0; r < this->mspScheme->fieldMemory(); r++)
            {
               std::shared_ptr<typename TOperator::InnerIterator> spTmpIt =
                  std::make_shared<typename TOperator::InnerIterator>(
                     this->rOldRHSMatrix(this->mspScheme->rhsT(0, step), i, r),
                     oldRhsJ.at(r));
               oldRhsIt.push_back(spTmpIt);
            }
            for (typename TOperator::InnerIterator timeIt(this->mTMatrix.at(i),
                    k);
                 timeIt; ++timeIt)
            {
               ...
               if (nnz > 0)
               {
                  ...
                  if (timeIt.col() == lhsIt.col())
                  {
                     ...
                     {
                        // Update values
                        lhsIt.valueRef() += this->mspScheme->lhsT(step) *
                                            (1.0 / oldDt - 1.0 / this->mDt) *
                                            timeIt.value();
                  ...
                  if (timeIt.col() == rhsIt.col())
                  {
                     ...
                     {
                        // Update values
                        rhsIt.valueRef() += this->mspScheme->rhsT(0, step) *
                                            (1.0 / oldDt - 1.0 / this->mDt) *
                                            timeIt.value();
                  ...
                  for (int r = 0; r < this->mspScheme->fieldMemory(); r++)
                  {
                     if (timeIt.col() == oldRhsIt.at(r)->col())
                     {
                        ...
                        {
                           // Update values
                           oldRhsIt.at(r)->valueRef() +=
                              this->mspScheme->rhsT(r + 1, step) *
                              (1.0 / oldDt - 1.0 / this->mDt) * timeIt.value();
...
template <typename TOperator, typename TData, template <typename> class TSolver>
void SparseOldImExTimestepper<TOperator, TData, TSolver>::setScheme(
   SharedIImExOldScheme spScheme)
{
...
template <typename TOperator, typename TData, template <typename> class TSolver>
void SparseOldImExTimestepper<TOperator, TData, TSolver>::initMatrices(
   const int n)
{
   // Initialise base matrices
   for (int i = 0; i < this->mspScheme->steps(); i++)
   {
      this->initMatrices(this->mspScheme->lhsT(i), this->mspScheme->rhsT(0, i),
         n);
   }
}

template <typename TOperator, typename TData, template <typename> class TSolver>
void SparseOldImExTimestepper<TOperator, TData, TSolver>::initSolutions()
{
   for (size_t k = 0; k < this->mOldSolution.size(); ++k)
   {
      for (size_t j = 0; j < this->mOldSolution.at(k).size(); ++j)
      {
         this->mOldSolution.at(k).at(j) =
            this->reg(Register::Solution::id()).at(k);
      }
   }
}

template <typename TOperator, typename TData, template <typename> class TSolver>
TOperator& SparseOldImExTimestepper<TOperator, TData, TSolver>::rRHSMatrix(
   const MHDFloat id, const int idx)
{
   return this->mRHSMatrix.find(id)->second.at(idx);
}

template <typename TOperator, typename TData, template <typename> class TSolver>
std::vector<TOperator>&
SparseOldImExTimestepper<TOperator, TData, TSolver>::rOldRHSMatrices(
   const MHDFloat id, const int idx)
{
...
template <typename TOperator, typename TData, template <typename> class TSolver>
TOperator& SparseOldImExTimestepper<TOperator, TData, TSolver>::rOldRHSMatrix(
   const MHDFloat id, const int idx, const int i)
{
   return this->mOldRHSMatrix.find(id)->second.at(idx).at(i);
}

template <typename TOperator, typename TData, template <typename> class TSolver>
TOperator& SparseOldImExTimestepper<TOperator, TData, TSolver>::rTMatrix(
   const int idx)
{
...
template <typename TOperator, typename TData, template <typename> class TSolver>
void SparseOldImExTimestepper<TOperator, TData, TSolver>::addStorage(
   const int rows, const int cols)
{
   ...
   Solver::SparseLinearSolver<TOperator, TData, TSolver>::addStorage(rows,
      cols);
...
template <typename TOperator, typename TData, template <typename> class TSolver>
void SparseOldImExTimestepper<TOperator, TData, TSolver>::buildOperators(
   ...
   const MHDFloat dt, const int size)
{
   ...
   for (int i = 0; i < this->mspScheme->steps(); ++i)
   {
      ...
      Solver::details::addOperators(
         this->rLHSMatrix(this->mspScheme->lhsT(i), idx),
         this->mspScheme->lhsL(i), iOpA->second);
      Solver::details::addOperators(
         this->rLHSMatrix(this->mspScheme->lhsT(i), idx),
         -this->mspScheme->lhsT(i) / this->mDt, iOpB->second);
      Solver::details::addOperators(
         this->rLHSMatrix(this->mspScheme->lhsT(i), idx), 1.0, iOpC->second);

      // Set explicit matrix
      this->rRHSMatrix(this->mspScheme->rhsT(0, i), idx).resize(size, size);
      this->rRHSMatrix(this->mspScheme->rhsT(0, i), idx).resize(size, size);
      Solver::details::addOperators(
         this->rRHSMatrix(this->mspScheme->rhsT(0, i), idx),
         -this->mspScheme->rhsL(0, i), iOpA->second);
      Solver::details::addOperators(
         this->rRHSMatrix(this->mspScheme->rhsT(0, i), idx),
         -this->mspScheme->rhsT(0, i) / this->mDt, iOpB->second);

      // Set explicit matrix for t_(n-i)
      for (int j = 0; j < this->mspScheme->fieldMemory(); j++)
      {
         this->rOldRHSMatrix(this->mspScheme->rhsT(0, i), idx, j)
            .resize(size, size);
         this->rOldRHSMatrix(this->mspScheme->rhsT(0, i), idx, j)
            .resize(size, size);
         Solver::details::addOperators(
            this->rOldRHSMatrix(this->mspScheme->rhsT(0, i), idx, j),
            -this->mspScheme->rhsL(j + 1, i), iOpA->second);
         Solver::details::addOperators(
            this->rOldRHSMatrix(this->mspScheme->rhsT(0, i), idx, j),
            -this->mspScheme->rhsT(j + 1, i) / this->mDt, iOpB->second);
      }
   }
}
...
template <typename TOperator, typename TData, template <typename> class TSolver>
void SparseOldImExTimestepper<TOperator, TData, TSolver>::initMatrices(
   const MHDFloat lhsId, const MHDFloat rhsId, const int n)
{
   Solver::SparseLinearSolver<TOperator, TData, TSolver>::initMatrices(lhsId,
      n);

   // Do not reinitialise if work already done by other field
   if (this->mRHSMatrix.count(rhsId) == 0 ||
       this->mRHSMatrix.find(rhsId)->second.size() == 0)
   {
      if (this->mRHSMatrix.count(rhsId) == 0)
      {
         this->mRHSMatrix.insert(
            std::make_pair(rhsId, std::vector<TOperator>()));
      }

      typename std::map<MHDFloat, std::vector<TOperator>>::iterator it =
         this->mRHSMatrix.find(rhsId);
   ...
   if ((this->mOldRHSMatrix.count(rhsId) == 0 ||
          this->mOldRHSMatrix.find(rhsId)->second.size() == 0) &&
       this->mspScheme->fieldMemory() > 0)
   {
      if (this->mOldRHSMatrix.count(rhsId) == 0)
      {
         this->mOldRHSMatrix.insert(
            std::make_pair(rhsId, std::vector<std::vector<TOperator>>()));
      }

      typename std::map<MHDFloat, std::vector<std::vector<TOperator>>>::iterator
         it = this->mOldRHSMatrix.find(rhsId);
...
namespace details {
template <typename TOperator, typename TData>
inline void computeMVPAY(TData& y, const TOperator& A, const TData& x,
   const MHDFloat a)
{
   y = A * x + a * y;
}
...
template <typename TOperator, typename TData>
inline void computeRHSNoFieldMemory(const int step, TData& rRHS,
   const TOperator& mat, const TData& sol, std::vector<TData>& rOldNL,
   TData& rTmp, SharedIImExOldScheme spScheme)
{
   // Store RHS at t_n
   rTmp = rRHS;
...
template <typename TOperator, typename TData>
inline void computeRHSNoNLMemory(const int step, TData& rRHS,
   const TOperator& mat, const TData& sol, const std::vector<TOperator>& oldMat,
   std::vector<TData>& rOldSol, SharedIImExOldScheme spScheme)
{
   // Compute RHS part from solution and nonlinear term at t_n
   computeMVPAY<TOperator, TData>(rRHS, mat, sol, spScheme->rhsN(0, step));
...
template <typename TOperator, typename TData>
inline void computeRHSMemory(const int step, TData& rRHS, const TOperator& mat,
   const TData& sol, const std::vector<TOperator>& oldMat,
   std::vector<TData>& rOldSol, std::vector<TData>& rOldNL, TData& rTmp,
   SharedIImExOldScheme spScheme)
{
...
template <>
inline void computeMVPAY<SparseMatrix, DecoupledZMatrix>(DecoupledZMatrix& y,
   const SparseMatrix& A, const DecoupledZMatrix& x, const MHDFloat a)
{
...
template <>
inline void computeRHSNoFieldMemory<SparseMatrix, DecoupledZMatrix>(
   const int step, DecoupledZMatrix& rRHS, const SparseMatrix& mat,
   const DecoupledZMatrix& sol, std::vector<DecoupledZMatrix>& rOldNL,
   DecoupledZMatrix& rTmp, SharedIImExOldScheme spScheme)
{
   //
   ...
   // Compute RHS part from solution and nonlinear term at t_n
   computeMVPAY<SparseMatrix, Matrix>(rRHS.real(), mat, sol.real(),
      spScheme->rhsN(0, step));
   ...
   // Compute RHS part from solution and nonlinear term at t_n
   computeMVPAY<SparseMatrix, Matrix>(rRHS.imag(), mat, sol.imag(),
      spScheme->rhsN(0, step));
...
template <>
inline void computeRHSNoNLMemory<SparseMatrix, DecoupledZMatrix>(const int step,
   DecoupledZMatrix& rRHS, const SparseMatrix& mat, const DecoupledZMatrix& sol,
   const std::vector<SparseMatrix>& oldMat,
   std::vector<DecoupledZMatrix>& rOldSol, SharedIImExOldScheme spScheme)
{
   // Compute RHS part from solution and nonlinear term at t_n
   computeMVPAY<SparseMatrix, DecoupledZMatrix>(rRHS, mat, sol,
      spScheme->rhsN(0, step));
...
template <>
inline void computeRHSMemory<SparseMatrix, DecoupledZMatrix>(const int step,
   DecoupledZMatrix& rRHS, const SparseMatrix& mat, const DecoupledZMatrix& sol,
   const std::vector<SparseMatrix>& oldMat,
   std::vector<DecoupledZMatrix>& rOldSol,
   std::vector<DecoupledZMatrix>& rOldNL, DecoupledZMatrix& rTmp,
   SharedIImExOldScheme spScheme)
{
   //
   // rTmp is only a temporary variable, using only real reduces memory usage
...
#endif // QUICC_TIMESTEP_LEGACY_SPARSEOLDIMEXTIMESTEPPER_HPP
