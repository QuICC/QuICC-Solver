...
namespace QuICC {
/// @brief This namespace groups the code needed to connect QuICC and quiccir, a
/// MLIR dialect
namespace Graph {
...
{
   return ViewDescriptor<T, std::uint32_t, 3>{
      {view.dims()[0], view.dims()[1], view.dims()[2]},
...
template <std::uint32_t RANK = 3u> class Jit
{
private:
...
public:
   ...
   /// @tparam Targs...
   /// @param args
   template <class... Targs> void apply(Targs...);
   ...
   /// @param ret0V
   /// @param arg0V
   template <class Trets, class Targs> void apply(Trets ret0V, Targs arg0V);
   ...
   /// @param arg2V
   template <class Trets, class Targs>
   void apply(Trets ret0V, Trets ret1V, Trets ret2V, Targs arg0V, Targs arg1V,
      Targs arg2V);
   ...
   void apply(TyOnerets retTyOne0V, TyOnerets retTyOne1V, TyOnerets retTyOne2V,
   ...
   template <class TyOnerets, class TyTworets, class Targs>
   void apply(TyOnerets retTyOne0V, TyOnerets retTyOne1V, TyOnerets retTyOne2V,
      TyOnerets retTyOne3V, TyOnerets retTyOne4V, TyTworets retTyTwo0V,
      TyTworets retTyTwo1V, TyTworets retTyTwo2V, TyTworets retTyTwo3V,
      TyTworets retTyTwo4V, TyTworets retTyTwo5V, Targs arg0V, Targs arg1V,
      Targs arg2V, Targs arg3V, Targs arg4V);
};

template <std::uint32_t RANK> void Jit<RANK>::setDialects()
{
...
void setOpt(const std::array<std::uint32_t, RANK> physDims,
   const std::array<std::uint32_t, RANK> modsDims,
   const std::array<std::array<std::string, 2>, RANK> layout, const Stage stage,
   std::vector<std::int64_t>& dim, std::string& lay)
{
   dim.resize(RANK);
   switch (stage)
   {
   ...
   const Stage inStage)
{
   ...
   details::setOpt<RANK>(physDims, modsDims, lay, inStage, dimArgs[0],
      layArgs[0]);
   ...
   details::setOpt<RANK>(physDims, modsDims, lay, outStage, dimRets[0],
      layRets[0]);
...
template <std::uint32_t RANK>
void Jit<RANK>::setLayout(
   const std::array<std::array<std::string, 2>, 3> layOpt)
{
...
void Jit<RANK>::setMeta(const std::vector<View::ViewBase<std::uint32_t>>& meta)
{
   ...
   if (meta.size() > 0)
   {
      ...
      // idx Fr
      _metaStore[0] = {meta[0].data(), meta[0].data(), 0,
         {static_cast<intptr_t>(meta[0].size())}, {1}};
      _metaMap[0] = &_metaStore[0];
      // ptr Fr
      _metaStore[1] = {meta[1].data(), meta[1].data(), 0,
         {static_cast<intptr_t>(meta[1].size())}, {1}};
      _metaMap[1] = &_metaStore[1];
      // idx AL
      _metaStore[2] = {meta[2].data(), meta[2].data(), 0,
         {static_cast<intptr_t>(meta[2].size())}, {1}};
...
template <std::uint32_t RANK> void Jit<RANK>::lower()
{
   // Top level (module) pass manager
   mlir::PassManager pm(&_ctx);
...
template <std::uint32_t RANK> void Jit<RANK>::setEngineAndJit()
{
   // _module->dump();
...
Jit<RANK>::Jit(const std::string modStr,
   ...
   const Stage outStage, const Stage inStage,
   const std::vector<View::ViewBase<std::uint32_t>>& meta,
   const PhysicalParameters<double> physParams, const PipelineOptions options) :
    _physParams(physParams), _opt(options)
{
   setDialects();
   // Load module
...
void Jit<RANK>::apply(Trets ret0V, Targs arg0V, Targs arg1V, Targs arg2V)
{
   ...
   // Apply graph
   auto fun = (void (*)(void*, void*, decltype(ret0)*, decltype(arg0)*,
      decltype(arg1)*, decltype(arg2)*))_funSym;
   fun(_metaMap.data(), thisArr.data(), &ret0, &arg0, &arg1, &arg2);
}

template <std::uint32_t RANK>
template <class Trets, class Targs>
void Jit<RANK>::apply(Trets ret0V, Trets ret1V, Trets ret2V, Targs arg0V,
   Targs arg1V, Targs arg2V)
{
   // Map view to MLIR view descritor
   auto arg0 = getViewDescriptor(arg0V);
   auto arg1 = getViewDescriptor(arg1V);
   ...
   // Apply graph
   auto fun =
      (void (*)(void*, void*, decltype(ret0)*, decltype(ret1)*, decltype(ret2)*,
         decltype(arg0)*, decltype(arg1)*, decltype(arg2)*))_funSym;
   fun(_metaMap.data(), thisArr.data(), &ret0, &ret1, &ret2, &arg0, &arg1,
      &arg2);
}

template <std::uint32_t RANK>
template <class TyOnerets, class TyTworets, class Targs>
void Jit<RANK>::apply(TyOnerets retTyOne0V, TyOnerets retTyOne1V,
   TyOnerets retTyOne2V, TyTworets retTyTwo0V, TyTworets retTyTwo1V,
   TyTworets retTyTwo2V, Targs arg0V, Targs arg1V, Targs arg2V)
{
   // Map view to MLIR view descritor
   auto arg0 = getViewDescriptor(arg0V);
   auto arg1 = getViewDescriptor(arg1V);
   ...
   // Apply graph
   auto fun = (void (*)(void*, void*, decltype(ret0)*, decltype(ret1)*,
      decltype(ret2)*, decltype(ret3)*, decltype(ret4)*, decltype(ret5)*,
      decltype(arg0)*, decltype(arg1)*, decltype(arg2)*))_funSym;
   fun(_metaMap.data(), thisArr.data(), &ret0, &ret1, &ret2, &ret3, &ret4,
      &ret5, &arg0, &arg1, &arg2);
}
...
template <class TyOnerets, class TyTworets, class Targs>
void Jit<RANK>::apply(TyOnerets retTyOne0V, TyOnerets retTyOne1V,
   TyOnerets retTyOne2V, TyOnerets retTyOne3V, TyOnerets retTyOne4V,
   TyTworets retTyTwo0V, TyTworets retTyTwo1V, TyTworets retTyTwo2V,
   ...
   Targs arg0V, Targs arg1V, Targs arg2V, Targs arg3V, Targs arg4V)
{
   ...
   auto fun = (void (*)(void*, void*, decltype(ret0)*, decltype(ret1)*,
      decltype(ret2)*, decltype(ret3)*, decltype(ret4)*, decltype(ret5)*,
      decltype(ret6)*, decltype(ret7)*, decltype(ret8)*, decltype(ret9)*,
      decltype(ret10)*, decltype(arg0)*, decltype(arg1)*, decltype(arg2)*,
      decltype(arg3)*, decltype(arg4)*))_funSym;
   fun(_metaMap.data(), thisArr.data(), &ret0, &ret1, &ret2, &ret3, &ret4,
      &ret5, &ret6, &ret7, &ret8, &ret9, &ret10, &arg0, &arg1, &arg2, &arg3,
      &arg4);
}
