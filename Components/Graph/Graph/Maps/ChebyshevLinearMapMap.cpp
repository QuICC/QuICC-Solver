...
void MapOps::setChebyshevLinearMapPrj(mlir::quiccir::CLPOp op)
{
   ...
   if (_thisArr[index] == nullptr)
   {
      ...
      else
      {
         ...
         using Top = QuICC::View::View<double, QuICC::View::CSL3DJIK>;
         using backend_t =
            QuICC::Transform::Quadrature::Cuda::ImplOp<Tout, Tin, Top>;
         using op_t = Op<Tout, Tin, Top, backend_t>;
         _ops.push_back(std::make_unique<op_t>(_mem));
         auto* ptr =
            std::get<std::shared_ptr<UnaryOp<Tout, Tin>>>(_ops.back()).get();
...
void MapOps::setChevbyshevLinearMapInt(mlir::quiccir::CLIOp op)
{
   // Get index from MLIR source
   std::uint64_t index = op.getImplptr().value();
   if (index >= _thisArr.size())
