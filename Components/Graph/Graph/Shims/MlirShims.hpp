...
namespace QuICC {
namespace Graph {
template <typename Tdata, typename Tmeta, std::size_t N> struct ViewDescriptor
{
   Tmeta dims[N];
   Tmeta* pos;
   Tmeta posSize;
...
/// @brief compute pointers and index meta data for fully populated
/// tensor as ouput of a Transpose operation between AL and JW/CL
...
/// @brief compute pointers and index meta data for fully populated
/// tensor as ouput of a Transpose operation between JW/CL and AL
/// @tparam C_S1CLCSC3D_t new buffer type
/// @tparam C_DCCSC3D_t producer type
/// @param dims output dimensions
/// @return
template <>
ptrAndIdx denseTransposePtrAndIdx<C_S1CLCSC3D_t, C_DCCSC3D_t>(
   const std::array<std::uint32_t, 3> dims);
...
#endif
#ifndef NDEBUG
   std::cout << "alloc, bytes: " << sizeByte << "\tis cpu: " << isCpuMem
             << "\tptr: " << *newPtr << '\n';
#endif
}
...
#endif
#ifndef NDEBUG
   std::cout << "dealloc, bytes: " << sizeByte << "\tis cpu: " << isCpuMem
             << "\tptr: " << pBuffer->data << '\n';
#endif
   if (isCpuMem)
   {
...
#ifdef QUICC_HAS_CUDA_BACKEND
std::uint32_t getSizeS1CLCSC3DJIK(const std::uint32_t* ptr,
   const std::uint32_t size, const std::uint32_t lds);
#endif
