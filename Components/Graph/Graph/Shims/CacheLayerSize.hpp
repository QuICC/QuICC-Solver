...
template <class T> class CacheLayerSize
{
public:
   ...
   /// @brief check if a value is in the cache and return it, otherwise return
   /// zero
...
template <class T> CacheLayerSize<T>& CacheLayerSize<T>::getInstance()
{
   static CacheLayerSize instance;
   return instance;
}

template <class T> void CacheLayerSize<T>::invalidate()
{
   _ptr2size.clear();
}

template <class T> void CacheLayerSize<T>::invalidate(const T* ptr)
{
   _ptr2size.erase(ptr);
}
