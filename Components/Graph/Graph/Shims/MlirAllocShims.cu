...
/// @param lds
/// @return
__global__ void kernelGetSizeS1CLCSC3DJIK(std::uint32_t* pCumSliceSize,
   const std::uint32_t* ptr, const std::uint32_t size, const std::uint32_t lds)
{
   ...
   const std::uint32_t size, const std::uint32_t lds)
{
   ...
   cudaErrChk(cudaMalloc(reinterpret_cast<void**>(&pCumSliceSize),
      sizeof(std::uint32_t)));
   kernelGetSizeS1CLCSC3DJIK<<<numBlocks, blockSize>>>(pCumSliceSize, ptr, size,
      lds);
   cudaErrChk(cudaMemcpy(&cumSliceSize, pCumSliceSize, sizeof(std::uint32_t),
