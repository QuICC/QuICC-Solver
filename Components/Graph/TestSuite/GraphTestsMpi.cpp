...
TEST_CASE("Parallel 3D Fwd", "[Parallel3DFwd]")
{
   ...
   std::array<std::vector<std::uint32_t>, dim> pointersPhys{
      {{}, setup.metaFT.ptr, {}}};
   std::array<std::vector<std::uint32_t>, dim> indicesPhys{
      {{}, setup.metaFT.idx, {}}};

   // Modal space (aka JW space, Stage::PMM and Stage::MMM, QuICC Stage0)
   std::array<std::vector<std::uint32_t>, dim> pointersMods{
      {{}, setup.metaJW.ptr, {}}};
   std::array<std::vector<std::uint32_t>, dim> indicesMods{
      {{}, setup.metaJW.idx, {}}};
   ...
   Jit<dim> Jitter(std::move(sourceMgr), mem, setup.physDims, setup.modsDims,
      layOpt, Stage::MMM, Stage::PPP, meta);
   ...
   QuICC::Memory::MemBlock<std::complex<double>> modsOut(
      setup.modsDims[0] * indicesMods[1].size(), mem.get());

   // host view
   std::array<std::uint32_t, dim> physDims{setup.physDims[2], setup.physDims[1],
      setup.physDims[0]};
   R_DCCSC3D_t RView({R.data(), R.size()}, physDims, pointersPhys, indicesPhys);
   C_DCCSC3D_t modsOutView({modsOut.data(), modsOut.size()},
      {setup.modsDims[0], setup.modsDims[2], setup.modsDims[1]}, pointersMods,
      indicesMods);
   ...
   try
   {
      ...
      CHECK(
         std::abs(modsOutView[m].real() - sqrt(2.0) * QuICC::Math::PI) <= eps);
...
TEST_CASE("Parallel 3D Bwd", "[Parallel3DBwd]")
{
   ...
   using namespace QuICC::Graph;
   Jit<dim> Jitter(std::move(sourceMgr), mem, setup.physDims, setup.modsDims,
      layOpt, Stage::PPP, Stage::MMM, meta);
   ...
   std::array<std::uint32_t, dim> physDims{setup.physDims[2], setup.physDims[1],
      setup.physDims[0]};
   R_DCCSC3D_t RView({R.data(), R.size()}, physDims, pointersPhys, indicesPhys);
   C_DCCSC3D_t modsInView({modsIn.data(), modsIn.size()},
      {setup.modsDims[0], setup.modsDims[2], setup.modsDims[1]}, pointersMods,
      indicesMods);
...
TEST_CASE("Parallel 3D Loop", "[Parallel3DLoop]")
{
   ...
   using namespace QuICC::Graph;
   Jit<dim> Jitter(std::move(sourceMgr), mem, setup.physDims, setup.modsDims,
      layOpt, Stage::MMM, Stage::MMM, meta);
...
TEST_CASE("Parallel Multi Var 3D Fwd", "[ParallelMultiVar3DFwd]")
{
   ...
   std::array<std::uint32_t, dim> physDims{setup.physDims[2], setup.physDims[1],
      setup.physDims[0]};
   R_DCCSC3D_t RView({R.data(), R.size()}, physDims, pointersPhys, indicesPhys);
   R_DCCSC3D_t PhiView({Phi.data(), Phi.size()}, physDims, pointersPhys,
      indicesPhys);
   R_DCCSC3D_t ThetaView({Theta.data(), Theta.size()}, physDims, pointersPhys,
      indicesPhys);
   C_DCCSC3D_t modsOutView({modsOut.data(), modsOut.size()},
      {setup.modsDims[0], setup.modsDims[2], setup.modsDims[1]}, pointersMods,
      indicesMods);
