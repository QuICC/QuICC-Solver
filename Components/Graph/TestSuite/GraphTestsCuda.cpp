...
TEST_CASE("One Dimensional Loop Fourier Gpu", "[OneDimLoopFourierGpu]")
{
   ...
   Jit<rank> Jitter(modStr, memDev, physDims, modsDims, layOpt, Stage::MPP,
      Stage::MPP, meta);
   ...
   // host view
   using namespace QuICC::Graph;
   C_DCCSC3D_t modsInView({modsIn.data(), modsIn.size()}, modsDimensions,
      pointers, indices);
   C_DCCSC3D_t modsOutView({modsOut.data(), modsOut.size()}, modsDimensions,
      pointers, indices);

   // device block
   QuICC::Memory::MemBlock<std::complex<double>> modsInDev(modsS, memDev.get());
   QuICC::Memory::MemBlock<std::complex<double>> modsOutDev(modsS,
      memDev.get());

   QuICC::Memory::MemBlock<std::uint32_t> memBlockPointersDev(
      pointers[1].size(), memDev.get());
   QuICC::Memory::MemBlock<std::uint32_t> memBlockIndicesDev(indices[1].size(),
      memDev.get());
   ...
   // device view
   C_DCCSC3D_t modsInViewDev(modsInDev.data(), modsInDev.size(),
      modsDimensions.data(), pointersDev, indicesDev);
   C_DCCSC3D_t modsOutViewDev(modsOutDev.data(), modsOutDev.size(),
      modsDimensions.data(), pointersDev, indicesDev);
   ...
   cudaErrChk(cudaMemcpy(modsOut.data(), modsOutDev.data(),
      modsS * sizeof(std::complex<double>), cudaMemcpyDeviceToHost));
...
TEST_CASE("One Dimensional Loop Associated Legendre Gpu", "[OneDimLoopALGpu]")
{
   ...
   using namespace QuICC::Graph;
   std::vector<QuICC::View::ViewBase<std::uint32_t>> meta;
   Jit<rank> Jitter(modStr, memDev, physDims, modsDims, layOpt, Stage::MPM,
      Stage::MPM, meta);
   ...
   // host view
   using namespace QuICC::Graph;
   C_S1CLCSC3DJIK_t modsInView({modsIn.data(), modsIn.size()}, dimensions,
      pointers, indices);
   C_S1CLCSC3DJIK_t modsOutView({modsOut.data(), modsOut.size()}, dimensions,
      pointers, indices);
   ...
   // device view
   C_S1CLCSC3DJIK_t modsInViewDev(modsInDev.data(), modsInDev.size(),
      dimensions.data(), pointersDev, indicesDev);
   C_S1CLCSC3DJIK_t modsOutViewDev(modsOutDev.data(), modsOutDev.size(),
      dimensions.data(), pointersDev, indicesDev);
...
TEST_CASE("One Dimensional Loop Worland Gpu", "[OneDimLoopJWGpu]")
{
   ...
   using namespace QuICC::Graph;
   std::vector<QuICC::View::ViewBase<std::uint32_t>> meta;
   Jit<rank> Jitter(modStr, memDev, physDims, modsDims, layOpt, Stage::MMM,
      Stage::MMM, meta);
   ...
   // host view
   using namespace QuICC::Graph;
   C_DCCSC3DJIK_t modsInView({modsIn.data(), modsIn.size()}, dimensions,
      pointers, indices);
   C_DCCSC3DJIK_t modsOutView({modsOut.data(), modsOut.size()}, dimensions,
      pointers, indices);
   ...
   // device view
   C_DCCSC3DJIK_t modsInViewDev(modsInDev.data(), modsInDev.size(),
      dimensions.data(), pointersDev, indicesDev);
   C_DCCSC3DJIK_t modsOutViewDev(modsOutDev.data(), modsOutDev.size(),
      dimensions.data(), pointersDev, indicesDev);
...
TEST_CASE("Serial 3D Fwd Gpu", "[Serial3DFwdGpu]")
{
   ...
   using namespace QuICC::Graph;
   auto metaAL =
      denseTransposePtrAndIdx<C_S1CLCSC3D_t, C_DCCSC3D_t>({modsN, K, modsM});

   // Populate meta for fully populated tensor
   // Modal space (Stage::PMM and Stage::MMM)
   auto metaJW = denseTransposePtrAndIdx<C_DCCSC3D_t, C_S1CLCSC3D_t>(
      {modsK, modsM, modsN});
   std::array<std::vector<std::uint32_t>, rank> pointersMods{
      {{}, metaJW.ptr, {}}};
   std::array<std::vector<std::uint32_t>, rank> indicesMods{
      {{}, metaJW.idx, {}}};
   // device meta
   auto mem = std::make_shared<QuICC::Memory::Cpu::NewDelete>();
   auto memDev = std::make_shared<QuICC::Memory::Cuda::Malloc>();
   QuICC::Memory::MemBlock<std::uint32_t> metaFTptrDev(ptrPhys.size(),
      memDev.get());
   QuICC::Memory::MemBlock<std::uint32_t> metaFTidxDev(idxPhys.size(),
      memDev.get());
   QuICC::Memory::MemBlock<std::uint32_t> metaALptrDev(metaAL.ptr.size(),
      memDev.get());
   QuICC::Memory::MemBlock<std::uint32_t> metaALidxDev(metaAL.idx.size(),
      memDev.get());
   QuICC::Memory::MemBlock<std::uint32_t> metaJWptrDev(metaJW.ptr.size(),
      memDev.get());
   QuICC::Memory::MemBlock<std::uint32_t> metaJWidxDev(metaJW.idx.size(),
      memDev.get());
   ...
   // Setup Jitter
   Jit<rank> Jitter(std::move(sourceMgr), memDev, physDims, modsDims, layOpt,
      Stage::MMM, Stage::PPP, meta);

   // host mem block
   std::size_t physS = M * indicesPhys[1].size();
   QuICC::Memory::MemBlock<double> R(physS, mem.get());
   QuICC::Memory::MemBlock<std::complex<double>> modsOut(
      modsK * metaJW.idx.size(), mem.get());

   // host view
   R_DCCSC3D_t RView({R.data(), R.size()}, {M, N, K}, pointersPhys,
      indicesPhys);
   C_DCCSC3DJIK_t modsOutView({modsOut.data(), modsOut.size()},
      {modsK, modsM, modsN}, pointersMods, indicesMods);
   ...
   QuICC::Memory::MemBlock<std::complex<double>> modsOutDev(
      modsK * metaJW.idx.size(), memDev.get());

   // set device pointers and indices device
   using namespace QuICC::View;
   ViewBase<std::uint32_t> pointersPhysDev[rank];
   pointersPhysDev[1] =
      ViewBase<std::uint32_t>(metaFTptrDev.data(), metaFTptrDev.size());
   ViewBase<std::uint32_t> indicesPhysDev[rank];
   indicesPhysDev[1] =
      ViewBase<std::uint32_t>(metaFTidxDev.data(), metaFTidxDev.size());
   ViewBase<std::uint32_t> pointersModsDev[rank];
   pointersModsDev[1] =
      ViewBase<std::uint32_t>(metaJWptrDev.data(), metaJWptrDev.size());
   ViewBase<std::uint32_t> indicesModsDev[rank];
   indicesModsDev[1] =
      ViewBase<std::uint32_t>(metaJWidxDev.data(), metaJWidxDev.size());

   // device view
   std::array<std::uint32_t, rank> dimPhys = {M, N, K};
   R_DCCSC3D_t RViewDev(RDev.data(), RDev.size(), dimPhys.data(),
      pointersPhysDev, indicesPhysDev);
   std::array<std::uint32_t, rank> dimMods = {modsK, modsM, modsN};
   C_DCCSC3DJIK_t modsOutViewDev(modsOutDev.data(), modsOutDev.size(),
      dimMods.data(), pointersModsDev, indicesModsDev);

   // cpu -> gpu data
   cudaErrChk(cudaMemcpy(RDev.data(), R.data(), R.size() * sizeof(double),
...
TEST_CASE("Serial 3D Loop Gpu", "[Serial3DLoopGpu]")
{
   ...
   // AL space (Stage::PPM and Stage::MPM)
   using namespace QuICC::Graph;
   auto metaAL = denseTransposePtrAndIdx<C_S1CLCSC3DJIK_t, C_DCCSC3DJIK_t>(
      {modsN, K, modsM});

   // Populate meta for fully populated tensor
   // Modal space
   using namespace QuICC::Graph;
   auto metaJW = denseTransposePtrAndIdx<C_DCCSC3DJIK_t, C_S1CLCSC3DJIK_t>(
      {modsK, modsM, modsN});
   ...
   std::array<std::vector<std::uint32_t>, rank> indicesMods{
      {{}, metaJW.idx, {}}};
   C_DCCSC3DJIK_t modsInView({modsIn.data(), modsIn.size()},
      {modsK, modsM, modsN}, pointersMods, indicesMods);
   C_DCCSC3DJIK_t modsOutView({modsOut.data(), modsOut.size()},
      {modsK, modsM, modsN}, pointersMods, indicesMods);
...
TEST_CASE("Serial Multi Var 3D Fwd Gpu", "[SerialMultiVar3DFwdGpu]")
{
   ...
   using namespace QuICC::Graph;
   auto metaJW =
      denseTransposePtrAndIdx<C_DCCSC3DJIK_t, C_S1CLCSC3DJIK_t>(outDims);
   ...
   R_DCCSC3D_t PhiView({Phi.data(), Phi.size()}, inDims, pointersPhys,
      indicesPhys);
   R_DCCSC3D_t ThetaView({Theta.data(), Theta.size()}, inDims, pointersPhys,
      indicesPhys);
   C_DCCSC3DJIK_t modsOutView({modsOut.data(), modsOut.size()}, outDims,
      pointersMods, indicesMods);
   ...
   // device view
   R_DCCSC3D_t RViewDev(RDev.data(), RDev.size(), inDims.data(),
      pointersPhysDev, indicesPhysDev);
   R_DCCSC3D_t PhiViewDev(PhiDev.data(), PhiDev.size(), inDims.data(),
      pointersPhysDev, indicesPhysDev);
   R_DCCSC3D_t ThetaViewDev(ThetaDev.data(), ThetaDev.size(), inDims.data(),
      pointersPhysDev, indicesPhysDev);
   C_DCCSC3DJIK_t modsOutViewDev(modsOutDev.data(), modsOutDev.size(),
      outDims.data(), pointersModsDev, indicesModsDev);
   ...
   // cpu -> gpu data
   cudaErrChk(cudaMemcpy(RDev.data(), R.data(), physS * sizeof(double),
      cudaMemcpyHostToDevice));
   cudaErrChk(cudaMemcpy(PhiDev.data(), Phi.data(), physS * sizeof(double),
      cudaMemcpyHostToDevice));
   cudaErrChk(cudaMemcpy(ThetaDev.data(), Theta.data(), physS * sizeof(double),
      cudaMemcpyHostToDevice));
