...
cufftHandle* setPlanRtoC(const int fwdSize, const int blockSize)
{
   ...
   auto err = cufftPlanMany(plan, rank, const_cast<int*>(&fwdSize), NULL,
      istride, idist, NULL, ostride, odist, CUFFT_D2Z, blockSize);
   if (err != CUFFT_SUCCESS)
   {
      throw std::logic_error(
         "CuFFT plan failed! error code: " + std::to_string(err));
   }
   return plan;
}
} // namespace details
   ...
   const View::View<double, AttIn>& phys)
{
   ...
   auto err = cufftExecD2Z(*static_cast<cufftHandle*>(_plan), phys.data(),
