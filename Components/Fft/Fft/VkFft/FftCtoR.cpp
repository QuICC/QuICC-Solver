...
namespace details
{
    VkFFTApplication* setPlanCtoR(const int fwdSize, const int blockSize)
    {
        ...
        sprintf(fname, "%s/VkFFT_%d_%d_%d_c2r_qddp", VKFFT_KERNELS_DIR, VkFFTGetVersion(), fwdSize, blockSize);
#else
        sprintf(fname, "%s/VkFFT_%d_%d_%d_c2r_dp", VKFFT_KERNELS_DIR, VkFFTGetVersion(), fwdSize, blockSize);
#endif
