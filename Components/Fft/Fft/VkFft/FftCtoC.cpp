/**
 * @file FftCtoC.cpp
 * @brief VkFft Complex to Complex backend
 */
...
namespace details
{
    VkFFTApplication* setPlanCtoC(const int fwdSize, const int blockSize)
    {
        // Create the complex to complex plan
        ...
        sprintf(fname, "%s/VkFFT_%d_%d_%d_c2c_qddp", VKFFT_KERNELS_DIR, VkFFTGetVersion(), fwdSize, blockSize);
#else
        sprintf(fname, "%s/VkFFT_%d_%d_%d_c2c_dp", VKFFT_KERNELS_DIR, VkFFTGetVersion(), fwdSize, blockSize);
#endif
