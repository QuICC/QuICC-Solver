   ...
   const T3& rhs, const std::tuple<int,int,int,int>& blkInfo)
{
   ...
   {
      setMatrixProduct(rField, start, mat, rhs.block(std::get<0>(blkInfo), std::get<1>(blkInfo), std::get<2>(blkInfo), std::get<3>(blkInfo)));
   }
}
   ...
   const T3& rhs, const std::tuple<int,int,int,int>& blkInfo)
{
   ...
   {
      addMatrixProduct(rField, start, mat, rhs.block(std::get<0>(blkInfo), std::get<1>(blkInfo), std::get<2>(blkInfo), std::get<3>(blkInfo)));
   }
}
