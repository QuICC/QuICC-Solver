...
ConfigurationReader::ConfigurationReader(const int dim,
   const std::vector<bool>& isPeriodicBox, const std::string& type) :
    IConfigurationFile<Io::Xml::IXmlReader>(dim, isPeriodicBox,
       ConfigurationReader::NAME, type)
{}
...
void ConfigurationReader::readBlock(SharedIConfigurationBlock spBlock,
   const bool required)
{
   if (spBlock->size() > 0)
   {
      // Get master pointer to framework XML code
      rapidxml::xml_node<>* pMaster =
         this->mpRoot->first_node(spBlock->tag().c_str());
      ...
      else
      {
         if (required)
         {
            throw std::logic_error("Couldn't find " + spBlock->tag() +
                                   " master tag from configuration file!");
         }
      }
   }
}
