...
public:
    ...
    */
   IXmlReader(std::string name, std::string ext, std::string header,
      std::string type, std::string version, std::string root);
...
protected:
    ...
    *
    * rapidXML reads the value from the file as a string. The conversion to an
    * other format is selected by the template parameter (ie. type of val). If
    * wrong type is given the obtained value will be wrong.
    *
    ...
    */
   template <typename T>
   void readValue(T& val, rapidxml::xml_node<>* base, const std::string& tag);

private:
};

template <typename T>
void IXmlReader::readValue(T& val, rapidxml::xml_node<>* base,
