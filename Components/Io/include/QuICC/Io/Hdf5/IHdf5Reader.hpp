...
public:
    ...
    */
   IHdf5Reader(std::string name, std::string ext, std::string header,
      std::string type, std::string version);
...
protected:
    ...
    */
   template <typename T>
   void readScalar(hid_t loc, const std::string dsname, T& val) const;
    ...
    */
   template <typename T>
   void readArray(hid_t loc, const std::string dsname,
      Eigen::Matrix<T, Eigen::Dynamic, 1>& arr) const;
    ...
    */
   template <typename T>
   void readMatrix(hid_t loc, const std::string dsname,
      Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>& mat) const;
    ...
    * @param loc     Base location in HDF5 file
    * @param dsname  HDF5 dataset name
    * @param storage Storage for the data to read (0 -> rows, 1 -> cols, 2 ->
    * data pointer)
    *
    * \tparam T Type of the scalars
    */
   template <typename T>
   void readIrregularField(hid_t loc, const std::string dsname,
    ...
    */
   template <typename T>
   void readRegularField(hid_t loc, const std::string dsname,
      std::vector<std::tuple<int, int, T*>>& storage);
...
template <typename T>
void IHdf5Reader::readScalar(hid_t loc, const std::string dsname, T& val) const
{
...
template <typename T>
void IHdf5Reader::readArray(hid_t loc, const std::string dsname,
   Eigen::Matrix<T, Eigen::Dynamic, 1>& arr) const
{
   ...
   if (Hdf5Types::isSupported<T>(type))
   {
      ...
      if (arr.size() != dim)
      {
         throw std::logic_error(
            "Provided storage for HDF5 data has wrong size!");
      }
...
template <typename T>
void IHdf5Reader::readMatrix(hid_t loc, const std::string dsname,
   Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>& mat) const
{
...
template <typename T>
void IHdf5Reader::readRegularField(hid_t loc, const std::string dsname,
   std::vector<std::tuple<int, int, T*>>& storage)
{
   // Open dataset in file
   hid_t dataset = H5Dopen(loc, dsname.c_str(), H5P_DEFAULT);
   ...
   if (Hdf5Types::isSupported<T>(type))
   {
      ...
      for (int i = 0; i < this->mCollIoRead; ++i)
      {
         ...
         // Select memory hyperslabs (i.e. minimum between file and memory
         // spaces) Check that at least one index is stored in file
         if (this->mFileOffsets.at(i).size() > 0)
         {
            iDims[0] = std::min(std::get<1>(storage.at(sidx)),
               std::max(0, static_cast<int>(fDims[nDims - 2] -
                                            this->mFileOffsets.at(i).at(1))));
         }
         else
         {
            iDims[0] = 0;
         ...
         H5Sselect_hyperslab(memspace, H5S_SELECT_SET, memOffset, NULL, iDims,
            NULL);
         ...
         H5Sselect_hyperslab(filespace, H5S_SELECT_SET, pOffset, NULL, dims,
            NULL);

         // Read file hyperslab into memory hyperslab
         H5Dread(dataset, type, memspace, filespace, dsPList,
            std::get<2>(storage.at(sidx)));
      ...
      //
      for (unsigned int i = this->mCollIoRead; i < this->mFileOffsets.size();
           ++i)
      {
         // Get storage index
         int sidx = this->mFileOffsets.at(i).at(2);
         ...
         // Select corresponding hyperslab
         dims[nDims - 2] = std::min(std::get<1>(storage.at(sidx)),
            static_cast<int>(fDims[nDims - 2]));
         H5Sselect_hyperslab(filespace, H5S_SELECT_SET, pOffset, NULL, dims,
            NULL);

         // Read file hyperslab into memory hyperslab and update offset
         H5Dread(dataset, type, memspace, filespace, H5P_DEFAULT,
            std::get<2>(storage.at(sidx)));
...
template <typename T>
void IHdf5Reader::readIrregularField(hid_t loc, const std::string dsname,
   std::vector<std::tuple<int, int, T*>>& storage)
{
   // Open dataset in file
   hid_t dataset = H5Dopen(loc, dsname.c_str(), H5P_DEFAULT);
   ...
   if (Hdf5Types::isSupported<T>(type))
   {
      ...
      for (int i = 0; i < this->mCollIoRead; ++i)
      {
         ...
         {
            // Uniform block
            blk = std::vector<hsize_t>(this->mFileOffsets.at(i).size(),
               this->mBlock.at(i).at(0));
         }
         else
         {
            // Non uniform block
         ...
         // Set size of read block (file resolution and memory resolution might
         // be different) Select memory hyperslabs
         ...
         for (size_t j = 0; j < this->mFileOffsets.at(i).size(); ++j)
         {
            iDims[1] = blk.at(j);
            memOffset[0] = j;
            H5Sselect_hyperslab(memspace, H5S_SELECT_OR, memOffset, NULL, iDims,
               NULL);
         }
         ...
         for (unsigned int j = 0; j < this->mFileOffsets.at(i).size(); ++j)
         {
            ...
            H5Sselect_hyperslab(filespace, H5S_SELECT_OR, pOffset, NULL, iDims,
               NULL);
         }

         // Read file hyperslab into memory hyperslab and update offset
         H5Dread(dataset, type, memspace, filespace, dsPList,
            std::get<2>(storage.at(i)));
