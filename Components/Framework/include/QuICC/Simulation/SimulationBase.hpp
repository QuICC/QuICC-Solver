...
public:
    ...
    */
   template <typename TEquation, typename TOptions>
   std::shared_ptr<TEquation> addEquation(std::shared_ptr<TOptions> spOptions);
   ...
   std::shared_ptr<TEquation> addEquation(
...
void SimulationBase::initResolution(std::shared_ptr<TScheme> spScheme)
{
   ...
   splitter.init(spBuilder, {this->mSimIoCtrl.config().algorithm()},
      ...
      this->mSimIoCtrl.config().cpuFactors());
...
template <typename TEquation>
std::shared_ptr<TEquation> SimulationBase::addEquation()
{
   ...
   std::shared_ptr<Model::IModelBackend> spBackend)
{
   ...
   std::shared_ptr<TEquation> spEq = std::make_shared<TEquation>(
      this->mspEqParams, this->mspRes->sim().spSpatialScheme(), spBackend);
...
{
   // Create shared equation
   std::shared_ptr<TEquation> spEq =
      std::make_shared<TEquation>(this->mspEqParams,
