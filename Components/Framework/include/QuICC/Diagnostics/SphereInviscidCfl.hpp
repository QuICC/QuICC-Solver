/**
 * @file SphereInviscidCfl.hpp
 * @brief CFL constraint in a full sphere
 */

#ifndef QUICC_DIAGNOSTICS_SPHEREINVISCIDCFL_HPP
#define QUICC_DIAGNOSTICS_SPHEREINVISCIDCFL_HPP
...
namespace Diagnostics {
   ...
   class SphereInviscidCfl: public TCfl
   {
      public:
         /**
          * @brief Constructor
          */
         SphereInviscidCfl(const std::map<std::size_t,NonDimensional::SharedINumber>& params, const MHDFloat courant);

         /**
          * @brief Destructor
          */
         virtual ~SphereInviscidCfl() = default;
   ...
   template <typename TCfl>
   SphereInviscidCfl<TCfl>::SphereInviscidCfl(const std::map<std::size_t,NonDimensional::SharedINumber>& params, const MHDFloat courant)
      : TCfl(params, courant)
   {
   }

   template <typename TCfl>
   MHDFloat SphereInviscidCfl<TCfl>::effectiveMaxL(const MHDFloat r) const
   {
      const auto& field = this->mFields.begin()->second;

      MHDFloat l = 0;
      int lB = field->res().sim().dim(Dimensions::Simulation::SIM2D, Dimensions::Space::SPECTRAL)/2+1;
   ...
   template <typename TCfl>
   MHDFloat SphereInviscidCfl<TCfl>::jacobiRoot(const MHDFloat l) const
   {
      const auto& field = this->mFields.begin()->second;
...
#endif // QUICC_DIAGNOSTICS_SPHEREINVISCIDCFL_HPP
