...
protected:
    ...
    */
   template <typename T>
   void readSpectralScalar(const std::string& name,
      typename Framework::Selector::ScalarField<T>& rScalar,
    ...
    */
   template <typename T>
   void readSpectralVector(const std::string& name,
      std::map<FieldComponents::Spectral::Id,
         typename Framework::Selector::ScalarField<T>>& rVector,
      const bool isRequired);
    ...
    */
   template <typename T>
   void readSpectralComponent(const std::string& name,
      FieldComponents::Spectral::Id id,
      typename Framework::Selector::ScalarField<T>& rComp,
      const bool isRequired);
    ...
    */
   template <typename T>
   void adaptData(typename Framework::Selector::ScalarField<T>& rComp);
...
template <typename T>
void StateFileReader::readSpectralScalar(const std::string& name,
   typename Framework::Selector::ScalarField<T>& rScalar, const bool isRequired)
{
   ...
   if (group >= 0)
   {
      ...
      std::vector<std::tuple<int, int,
         typename Framework::Selector::ScalarField<T>::PointType*>>
...
template <typename T>
void StateFileReader::readSpectralVector(const std::string& name,
   std::map<FieldComponents::Spectral::Id,
      typename Framework::Selector::ScalarField<T>>& rVector,
   const bool isRequired)
{
...
template <typename T>
void StateFileReader::readSpectralComponent(const std::string& name,
   FieldComponents::Spectral::Id id,
   typename Framework::Selector::ScalarField<T>& rComp, const bool isRequired)
{
   // Open the vector field group
   hid_t group = H5Gopen(this->file(), name.c_str(), H5P_DEFAULT);
   ...
   if (group >= 0)
   {
      ...
      else
      {
         // Read the field component
         this->readIrregularField(group,
            name + "_" + Tools::IdToHuman::toTag(id), fieldInfo);
      }
...
template <typename T>
void StateFileReader::adaptData(
   typename Framework::Selector::ScalarField<T>& rField)
{
   if (this->res().sim().ss().has(SpatialScheme::Feature::FourierIndex23))
   {
      const auto& tRes =
         *this->res().cpu()->dim(Dimensions::Transform::SPECTRAL);
      for (int k = 0; k < tRes.dim<Dimensions::Data::DAT3D>(); k++)
      {
         if (tRes.idx<Dimensions::Data::DAT2D>(0, k) == 0 &&
             tRes.idx<Dimensions::Data::DAT3D>(k) >
                this->res().sim().dim(Dimensions::Simulation::SIM2D,
                   Dimensions::Space::SPECTRAL) /
                   2)
         {
            rField.setProfile(
               Eigen::Matrix<T, Eigen::Dynamic, 1>::Zero(this->res().sim().dim(
                  Dimensions::Simulation::SIM1D, Dimensions::Space::SPECTRAL)),
               0, k);
         }
      }
   }
