...
#ifndef QUICC_TIMESTEP_SPARSEIMEXRK3RTIMESTEPPER_HPP
#define QUICC_TIMESTEP_SPARSEIMEXRK3RTIMESTEPPER_HPP
...
namespace Timestep {
    ...
    * @brief Implementation of a templated (coupled) equation timestepper for Implicit-Explicit Runge-Kutta (3R) schemes
    */
   template <typename TOperator,typename TData,template <typename> class TSolver> class SparseImExRK3RTimestepper: public ISparseImExRKnRTimestepper<TOperator,TData,TSolver>
   {
      public:
         /**
          * @brief Constructor
   ...
   template <typename TOperator,typename TData,template <typename> class TSolver> SparseImExRK3RTimestepper<TOperator,TData,TSolver>::SparseImExRK3RTimestepper(const int start, const std::size_t timeId)
   ...
   template <typename TOperator,typename TData,template <typename> class TSolver> bool SparseImExRK3RTimestepper<TOperator,TData,TSolver>::preSolve()
   {
      ...
      if(this->mStep == 0)
      {
         ...
         for(size_t i = this->mZeroIdx; i < this->nSystem(); i++)
         {
            details::computeMV(this->reg(Register::Temporary::id()).at(i), this->mMassMatrix.at(i), this->reg(Register::Intermediate::id()).at(i));
         }
      ...
      } else
      {
         if(this->mRegisterId == Register::Implicit::id())
         {
            ...
            for(size_t i = this->mZeroIdx; i < this->nSystem(); i++)
            {
               details::computeXPAY(this->reg(Register::Explicit::id()).at(i), this->reg(Register::Temporary::id()).at(i), aEx);
            }
            ...
            if(this->mStep < this->mspScheme->steps()-1)
            {
               ...
               for(size_t i = this->mZeroIdx; i < this->nSystem(); i++)
               {
                  details::computeXPAY(this->reg(Register::Temporary::id()).at(i), this->reg(Register::Explicit::id()).at(i), -1.0);
                  details::computeAMXPYPBZ(this->reg(Register::Temporary::id()).at(i), this->mMassMatrix.at(i), aIm, this->reg(Register::Implicit::id()).at(i), this->reg(Register::Intermediate::id()).at(i), aEx);
               }
            }
            ...
            for(size_t i = this->mZeroIdx; i < this->nSystem(); i++)
            {
               details::computeAMXPY(this->reg(Register::Explicit::id()).at(i), this->mMassMatrix.at(i), aIm, this->reg(Register::Implicit::id()).at(i));
   ...
   template <typename TOperator,typename TData,template <typename> class TSolver> void SparseImExRK3RTimestepper<TOperator,TData,TSolver>::addStorage(const int rows, const int cols)
   {
...
#endif // QUICC_TIMESTEP_SPARSEIMEXRK3RTIMESTEPPER_HPP
