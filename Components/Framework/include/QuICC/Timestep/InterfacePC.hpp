/**
 * @file InterfacePC.hpp
 * @brief Implementation of an interface to the ImEx predictor-corrector schemes
 */

#ifndef QUICC_TIMESTEP_INTERFACEPC_HPP
#define QUICC_TIMESTEP_INTERFACEPC_HPP
...
namespace Timestep {
    ...
    */
   template <typename TScheme> class InterfacePC: public Interface
   {
      public:
          ...
          */
         InterfacePC(const MHDFloat time, const Matrix& cfl, const MHDFloat maxError, const ScalarEquation_range& scalEq, const VectorEquation_range& vectEq);

         /**
          * @brief Destructor
          */
         virtual ~InterfacePC() = default;
   ...
   template <typename TScheme> InterfacePC<TScheme>::InterfacePC(const MHDFloat time, const Matrix& cfl, const MHDFloat maxError, const ScalarEquation_range& scalEq, const VectorEquation_range& vectEq)
   ...
   template <typename TScheme> void InterfacePC<TScheme>::tuneAdaptive(const MHDFloat time)
   {
      this->mStepTime = time;
   }

   template <typename TScheme> bool InterfacePC<TScheme>::finishedStep() const
   {
      return this->mSolverCoord.finishedStep();
   }

   template <typename TScheme> void InterfacePC<TScheme>::setSolveTime(const std::size_t timeId)
   {
      this->mSolverCoord.setSolveTime(timeId);
   }

   template <typename TScheme> void InterfacePC<TScheme>::getExplicitInput(const std::size_t opId, const ScalarEquation_range& scalEq, const VectorEquation_range& vectEq, const ScalarVariable_map& scalVar, const VectorVariable_map& vectVar)
   {
      this->mSolverCoord.getExplicitInput(opId, scalEq, vectEq, scalVar, vectVar);
   }

   template <typename TScheme> void InterfacePC<TScheme>::adaptTimestep(const Matrix& cfl, const ScalarEquation_range&, const VectorEquation_range&)
   {
      // Store old timestep
   ...
   template <typename TScheme> void InterfacePC<TScheme>::stepForward(const ScalarEquation_range& scalEq, const VectorEquation_range& vectEq, const ScalarVariable_map& scalVar, const VectorVariable_map& vectVar)
   {
   ...
   template <typename TScheme> void InterfacePC<TScheme>::updateMatrices()
   {
   ...
   template <typename TScheme> void InterfacePC<TScheme>::printInfo(std::ostream& stream)
   {
...
#endif // QUICC_TIMESTEP_INTERFACEPC_HPP
