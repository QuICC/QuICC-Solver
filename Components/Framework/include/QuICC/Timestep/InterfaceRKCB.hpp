/**
 * @file InterfaceRKCB.hpp
 * @brief Implementation of a interface to the RK CB schemes
 */

#ifndef QUICC_TIMESTEP_INTERFACERKCB_HPP
#define QUICC_TIMESTEP_INTERFACERKCB_HPP
...
namespace Timestep {
    ...
    */
   template <typename TScheme> class InterfaceRKCB: public Interface
   {
      public:
         /// Typedef for solver implementation
         template <typename T1, typename T2, template <typename> class T3>
            using SolverImplementationType = typename TScheme::template ImplementationType<T1,T2,T3>;

         /// Typedef for parent coordinator
         typedef TimestepSolverCoordinator<SolverImplementationType> SolverCoordinator;

         /// Typedef for a shared real operator solver
         typedef typename SolverCoordinator::SharedRealSolverType  SharedRealSolverType;

         /// Typedef for a shared complex operator solver
         typedef typename SolverCoordinator::SharedComplexSolverType  SharedComplexSolverType;
          ...
          */
         InterfaceRKCB(const MHDFloat time, const Matrix& cfl, const MHDFloat maxError, const ScalarEquation_range& scalEq, const VectorEquation_range& vectEq);

         /**
          * @brief Destructor
          */
         virtual ~InterfaceRKCB() = default;
          ...
          */
         void getExplicitInput(const std::size_t opId, const ScalarEquation_range& scalEq, const VectorEquation_range& vectEq, const ScalarVariable_map& scalVar, const VectorVariable_map& vectVar) final;
          ...
          * @param vectEq  Shared vector equations
          */
         void adaptTimestep(const Matrix& cfl, const ScalarEquation_range& scalEq, const VectorEquation_range& vectEq) final;
          ...
          */
         void stepForward(const ScalarEquation_range& scalEq, const VectorEquation_range& vectEq, const ScalarVariable_map& scalVar, const VectorVariable_map& vectVar) final;
   ...
   template <typename TScheme> InterfaceRKCB<TScheme>::InterfaceRKCB(const MHDFloat time, const Matrix& cfl, const MHDFloat maxError, const ScalarEquation_range& scalEq, const VectorEquation_range& vectEq)
      : Interface(time, cfl, maxError, scalEq, vectEq)
   {
   ...
   template <typename TScheme> void InterfaceRKCB<TScheme>::tuneAdaptive(const MHDFloat time)
   {
      this->mStepTime = time;
   }

   template <typename TScheme> bool InterfaceRKCB<TScheme>::finishedStep() const
   {
      return this->mSolverCoord.finishedStep();
   }

   template <typename TScheme> void InterfaceRKCB<TScheme>::setSolveTime(const std::size_t timeId)
   {
      this->mSolverCoord.setSolveTime(timeId);
   }

   template <typename TScheme> void InterfaceRKCB<TScheme>::getExplicitInput(const std::size_t opId, const ScalarEquation_range& scalEq, const VectorEquation_range& vectEq, const ScalarVariable_map& scalVar, const VectorVariable_map& vectVar)
   {
      this->mSolverCoord.getExplicitInput(opId, scalEq, vectEq, scalVar, vectVar);
   }

   template <typename TScheme> void InterfaceRKCB<TScheme>::adaptTimestep(const Matrix& cfl, const ScalarEquation_range&, const VectorEquation_range&)
   {
      // Store old timestep
      ...
      {
         newErrorDt = this->timestep()*std::pow(this->mMaxError/error,1./this->mspScheme->order())/this->mcUpWindow;

      // Error is small, increase timestep
      } else if(error < this->mMaxError/(this->mcMaxJump*0.9) && this->mCnstSteps >= this->mcMinCnst)
      {
         newErrorDt = std::min(this->timestep()*std::pow(this->mMaxError/error, 1./this->mspScheme->order()), this->timestep()*this->mcMaxJump);
      ...
      if(this->timestep() != this->mOldDt && this->timestep() > 0.0)
      {
         ...
         Solver::updateSolvers<SolverImplementationType,typename Solver::SparseCoordinatorBase<SolverImplementationType>::ComplexSolver_iterator>(this->mSolverCoord);
         ...
         Solver::updateSolvers<SolverImplementationType,typename Solver::SparseCoordinatorBase<SolverImplementationType>::RealSolver_iterator>(this->mSolverCoord);
   ...
   template <typename TScheme> void InterfaceRKCB<TScheme>::stepForward(const ScalarEquation_range& scalEq, const VectorEquation_range& vectEq, const ScalarVariable_map& scalVar, const VectorVariable_map& vectVar)
   {
      ...
      // Update current time
      this->mTime = this->mRefTime + this->mSolverCoord.stepFraction()*this->timestep();
   }

   template <typename TScheme> void InterfaceRKCB<TScheme>::updateMatrices()
   {
      // Loop over all complex operator, complex field timesteppers
      Solver::updateTimeMatrixSolvers<SolverImplementationType,typename Solver::SparseCoordinatorBase<SolverImplementationType>::ComplexSolver_iterator>(this->mSolverCoord, this->timestep());

      // Loop over all real operator, complex field timesteppers
      Solver::updateTimeMatrixSolvers<SolverImplementationType,typename Solver::SparseCoordinatorBase<SolverImplementationType>::RealSolver_iterator>(this->mSolverCoord, this->timestep());
   }

   template <typename TScheme> void InterfaceRKCB<TScheme>::printInfo(std::ostream& stream)
   {
...
#endif // QUICC_TIMESTEP_INTERFACERKCB_HPP
