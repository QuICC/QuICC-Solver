...
#ifndef QUICC_EQUATIONS_COPYUNKNOWNFUNCTORS_HPP
#define QUICC_EQUATIONS_COPYUNKNOWNFUNCTORS_HPP
...
template <>
template <bool IsSet, typename TData, typename TField> void CopyUnknownFunctor<CouplingIndexType::SINGLE>::apply(const TField& field, TData& storage, const int start)
{
      ...
      for(int j = 0; j < tRes.template dim<Dimensions::Data::DAT2D>(k); j++)
      {
         ...
         for(int i = 0; i < sRes.dim(Dimensions::Simulation::SIM1D, Dimensions::Space::SPECTRAL); i++)
         {
            ...
            if constexpr(IsSet)
            {
               ...
               Arithmetics::assignScalar<Arithmetics::Operation::Set>(storage, l, field.comp(compId).point(i,j,k));
            }
            else
            {
               // Add field value to storage
               Arithmetics::assignScalar<Arithmetics::Operation::Plus>(storage, l, field.comp(compId).point(i,j,k));
            }
         }
      }
   }
...
#endif // QUICC_EQUATIONS_COPYUNKNOWNFUNCTORS_HPP
