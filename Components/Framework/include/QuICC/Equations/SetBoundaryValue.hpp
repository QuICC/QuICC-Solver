...
namespace Equations {
    ...
    */
   template <typename TData, typename TField> void setBoundaryValue(const Resolution& res, const CouplingInformation& cinfo, Spectral::Kernel::SharedISpectralKernel spBoundary, const TField& field, TData& storage, const int matIdx, const int start);
   ...
   template <typename TData, typename TField> void setBoundaryValue(const Resolution& res, const CouplingInformation& cinfo, Spectral::Kernel::SharedISpectralKernel spBoundary, const TField& field, TData& storage, const int matIdx, const int start)
   {
      ...
      if(cinfo.hasBoundaryValue())
      {
         ...
         {
            details::SetBoundaryValueFunctor<CouplingIndexType::SLOWEST_SINGLE_RHS> func(res, cinfo, spBoundary, matIdx);
         ...
         {
            details::SetBoundaryValueFunctor<CouplingIndexType::SLOWEST_MULTI_RHS> func(res, cinfo, spBoundary, matIdx);
            func.apply(field, storage, start);
         }
         ...
         {
            details::SetBoundaryValueFunctor<CouplingIndexType::MODE> func(res, cinfo, spBoundary, matIdx);
            func.apply(field, storage, start);
         }
         // There is a single matrix
         else if(cinfo.indexType() == CouplingIndexType::SINGLE)
         {
            details::SetBoundaryValueFunctor<CouplingIndexType::SINGLE> func(res, cinfo, spBoundary, matIdx);
            func.apply(field, storage, start);
         }
      }
   }
