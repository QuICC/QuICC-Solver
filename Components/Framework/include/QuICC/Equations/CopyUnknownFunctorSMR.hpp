   ...
   for(int j = zeroCol; j < cols; j++)
   {
      ...
      for(int i = zeroRow; i < rows; i++)
      {
         if constexpr(IsSet)
         {
            ...
            Arithmetics::assignScalar<Arithmetics::Operation::Set>(storage, i - zeroRow + start, j - zeroCol, field.comp(compId).point(i,j,matIdx));
         }
         else
         {
            // Add field value to storage
            Arithmetics::assignScalar<Arithmetics::Operation::Plus>(storage, i - zeroRow + start, j - zeroCol, field.comp(compId).point(i,j,matIdx));
         }
      }
   }
}
