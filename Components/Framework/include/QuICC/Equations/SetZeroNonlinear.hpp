...
namespace Equations {
    ...
    */
   template <typename TData, typename TField> void setZeroNonlinear(const Resolution& res, const CouplingInformation& cinfo, const TField& field, TData& storage, const int matIdx, const int start);
   ...
   template <typename TData, typename TField> void setZeroNonlinear(const Resolution& res, const CouplingInformation& cinfo, const TField& field, TData& storage, const int matIdx, const int start)
   {
      ...
      {
         details::SetZeroNonlinearFunctor<CouplingIndexType::SLOWEST_SINGLE_RHS> func(res, cinfo, matIdx);
      ...
      {
         details::SetZeroNonlinearFunctor<CouplingIndexType::SLOWEST_MULTI_RHS> func(res, cinfo, matIdx);
         func.apply(field, storage, start);
      }
      ...
      {
         details::SetZeroNonlinearFunctor<CouplingIndexType::MODE> func(res, cinfo, matIdx);
         func.apply(field, storage, start);
      }
      else if(cinfo.indexType() == CouplingIndexType::SINGLE)
      {
         details::SetZeroNonlinearFunctor<CouplingIndexType::SINGLE> func(res, cinfo, matIdx);
         func.apply(field, storage, start);
      }
   }
