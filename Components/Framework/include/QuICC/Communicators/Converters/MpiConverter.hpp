...
public:
    ...
    */
   template <typename TFwd, typename TBwd>
   void init(SharedResolution spRes, const Dimensions::Transform::Id fwdDim,
      TFwd& fwdTmp, TBwd& bwdTmp, const ArrayI& fwdPacks,
      const ArrayI& bwdPacks, std::shared_ptr<IIndexConv> spIdxConv);

   /**
    * @brief Convert real data from Fwd to Bwd
    */
   virtual void convertFwd(const RealFwdData& in,
      DynamicPairProvider& storage) override;

   /**
    * @brief Convert complex data from Fwd to Bwd
    */
   virtual void convertFwd(const ComplexFwdData& in,
      DynamicPairProvider& storage) override;

   /**
    * @brief Convert real data from Bwd to Fwd
    */
   virtual void convertBwd(const RealBwdData& in,
      DynamicPairProvider& storage) override;

   /**
    * @brief Convert complex data from Bwd to Fwd
    */
   virtual void convertBwd(const ComplexBwdData& in,
      DynamicPairProvider& storage) override;

   /**
    * @brief Get the converted real data from Bwd to Fwd
    */
   virtual void getFwd(RealFwdData*& pOut,
      DynamicPairProvider& storage) override;

   /**
    * @brief Get the converted complex data from Bwd to Fwd
    */
   virtual void getFwd(ComplexFwdData*& pOut,
      DynamicPairProvider& storage) override;

   /**
    * @brief Get the converted real data from Fwd to Bwd
    */
   virtual void getBwd(RealBwdData*& pOut,
      DynamicPairProvider& storage) override;

   /**
    * @brief Get the converted complex data from Fwd to Bwd
    */
   virtual void getBwd(ComplexBwdData*& pOut,
      DynamicPairProvider& storage) override;
...
private:
    ...
    */
   template <typename T>
   void convertFwdImpl(const T& in, DynamicPairProvider& storage);

   /**
    * @brief Convert real data from Bwd to Fwd
    */
   template <typename T>
   void convertBwdImpl(const T& in, DynamicPairProvider& storage);

   /**
    * @brief Get the converted complex data from Bwd to Fwd
    */
   template <typename T>
   void getFwdImpl(T*& pOut, DynamicPairProvider& storage);

   /**
    * @brief Get the converted real data from Fwd to Bwd
    */
   template <typename T>
   void getBwdImpl(T*& pOut, DynamicPairProvider& storage);
    ...
    */
   template <typename TFwd, typename TBwd>
   void initPackTypes(SharedResolution spRes, TFwd& fTmp, TBwd& bTmp);
...
template <typename T>
void MpiConverter::convertFwdImpl(const T& in, DynamicPairProvider&)
{
   // Send the data
   this->sendFwd(in);
}
...
template <typename T>
void MpiConverter::convertBwdImpl(const T& in, DynamicPairProvider&)
{
   // Send the data
   this->sendBwd(in);
}
...
template <typename T>
void MpiConverter::getFwdImpl(T*& pData, DynamicPairProvider& storage)
{
   if (pData == nullptr)
   {
...
template <typename T>
void MpiConverter::getBwdImpl(T*& pData, DynamicPairProvider& storage)
{
   if (pData == nullptr)
   {
      // Get storage for output value
...
template <typename TFwd, typename TBwd>
void MpiConverter::init(SharedResolution spRes,
   const Dimensions::Transform::Id fwdDim, TFwd& fwdTmp, TBwd& bwdTmp,
   const ArrayI& fwdPacks, const ArrayI& bwdPacks,
   std::shared_ptr<IIndexConv> spIdxConv)
{
   StageTimer stage;
...
template <typename TFwd, typename TBwd>
void MpiConverter::initPackTypes(SharedResolution spRes, TFwd& fTmp, TBwd& bTmp)
{
   ...
   MpiConverterTools::buildLocalBwdMap(localBwdMap, spRes, this->mTraId,
      this->idxConv());
...
// Create TBwd datatypes
      ...
      MpiConverterTools::buildDatatype<TBwd, Dimensions::Data::DATF1D>(
         this->mBTypes.back(), localBwdMap, spRes, this->mTraId, this->mTraId,
         bTmp, id);
#else
      // Buffer index to local index or reverse depending on direction
      MPI_Datatype type =
         MpiConverterTools::buildDatatype<TBwd, Dimensions::Data::DATF1D>(
            localBwdMap, spRes, this->mTraId, this->mTraId, bTmp, id);
...
// Create TFwd datatypes
      ...
      MpiConverterTools::buildDatatype<TFwd, Dimensions::Data::DATB1D>(
         this->mFTypes.back(), localFwdMap, spRes, this->mTraId,
         Dimensions::jump(this->mTraId, 1), fTmp, id);
#else
      type = MpiConverterTools::buildDatatype<TFwd, Dimensions::Data::DATB1D>(
         localFwdMap, spRes, this->mTraId, Dimensions::jump(this->mTraId, 1),
         fTmp, id);
