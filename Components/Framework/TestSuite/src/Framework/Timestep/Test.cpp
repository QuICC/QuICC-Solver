...
namespace Timestep {
   ...
   Test::Test(const int id)
      : nN(-1), basisId(BasisId::WORLAND), l(-1), dt(-1), timeCoeffs(0), tEnd(0), startRow(1), start(0), cols(1), t0(0.0), id(id)
   {
      this->configure();
   }
   ...
   void Test::configure()
   {
      const std::string refdir =  "_refdata/Framework/Timestep/";
      ...
      switch(this->id)
      {
         ...
         case 17:
            ...
            this->fbase = refdir + "Worland/bidiffusion_id" + std::to_string(this->id-10);
            break;
         case 100:
            this->basisId = BasisId::CHEBYSHEV;
            this->equationId = EquationId::DIFFUSION;
            this->fbase = refdir + "Chebyshev/diffusion_id" + std::to_string(this->id-100);
            break;
         default:
