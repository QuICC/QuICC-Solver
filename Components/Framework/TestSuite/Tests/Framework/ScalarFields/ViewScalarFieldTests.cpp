#include <catch2/catch.hpp>

#include "QuICC/Enums/FieldIds.hpp"
#include "details/Helper.hpp"

TEST_CASE("ViewScalarField get point data", "[ViewScalarField::point]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;
   
   std::vector<std::vector<int>> pts = {{0,0,0}, {3,0,0}, {1,1,0}, {1,0,1}, {3,2,3}};
   for(auto&& pt: pts)
   {
      CHECK( field.point(pt.at(0),pt.at(1),pt.at(2)) == details::fieldValueA<double>(pt.at(0),pt.at(1),pt.at(2)) );
   }
}

TEST_CASE("ViewScalarField get point data from coord", "[ViewScalarField::point_coord]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;
   
   std::vector<std::vector<int>> pts = {{0,0,0}, {3,0,0}, {1,1,0}, {1,0,1}, {3,2,3}};
   for(auto&& pt: pts)
   {
      CHECK( field.point(pt) == details::fieldValueA<double>(pt.at(0),pt.at(1),pt.at(2)) );
   }
}

TEST_CASE("ViewScalarField get profile data", "[ViewScalarField::profile]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;
 
   std::vector<std::pair<int,int>> jks = {{1,0},{2,3},{5,4}};
   for(auto&& jk: jks)
   {
      auto&& p = field.profile(jk.first, jk.second);
      for(int i = 0; i < dim1D; i++)
      {
         CHECK( p(i) == details::fieldValueA<double>(i,jk.first,jk.second) );
      }
   }
}

TEST_CASE("ViewScalarField get slice data", "[ViewScalarField::slice]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;
 
   std::vector<int> ks = {0, 2, 4};
   for(auto&& k: ks)
   {
      auto&& s = field.slice(k);
      for(int j = 0; j < k + 2; j++)
      {
         for(int i = 0; i < dim1D; i++)
         {
            CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) );
         }
      }
   }
}

TEST_CASE("ViewScalarField set point data", "[ViewScalarField::setPoint]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   field.setPoint(-700.007, 0, 1, 0);
   field.setPoint(-700.008, 2, 1, 1);
   field.setPoint(-700.009, 7, 3, 3);
   field.setPoint(-700.017, dim1D-1, 1, 4);

   CHECK( field.point(0, 1, 0) == -700.007);
   CHECK( field.point(2, 1, 1) == -700.008);
   CHECK( field.point(7, 3, 3) == -700.009);
   CHECK( field.point(dim1D-1, 1, 4) == -700.017);
}

TEST_CASE("ViewScalarField set point data from variant", "[ViewScalarField::setPoint_variant]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;
   QuICC::MHDVariant pt;

   pt = -700.007;
   field.setPoint(pt, 0, 1, 0);
   pt = -700.008;
   field.setPoint(pt, 2, 1, 1);
   pt = -700.009;
   field.setPoint(pt, 7, 3, 3);
   pt = -700.017;
   field.setPoint(pt, dim1D-1, 1, 4);

   CHECK( field.point(0, 1, 0) == -700.007);
   CHECK( field.point(2, 1, 1) == -700.008);
   CHECK( field.point(7, 3, 3) == -700.009);
   CHECK( field.point(dim1D-1, 1, 4) == -700.017);
}

TEST_CASE("ViewScalarField set profile data", "[ViewScalarField::setProfile]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   std::vector<double> pData(dim1D);
   QuICC::View::ViewBase<double> pView(pData.data(), pData.size());
   std::array<std::uint32_t, 1> dims = {static_cast<std::uint32_t>(dim1D)};
   QuICC::View::View<double, QuICC::View::dense1D> np(pView, dims);
   for(int i = 0; i < dim1D; i++)
   {
      np(i) = details::fieldValueB<double>(i,0,0);
   }

   std::vector<std::pair<int,int>> jks = {{0,0}, {3,2}, {3,3}, {1,4}};
   for(auto jk: jks)
   {
      field.setProfile(np, jk.first, jk.second);
   }
  
   {
      std::vector<std::pair<int,int>> jks = {{0,0}, {3,2}, {3,3}, {1,4}};
      for(auto jk: jks)
      {
         auto j = jk.first;
         auto k = jk.second;
         auto&& p = field.profile(j, k);
         for(int i = 0; i < dim1D; i++)
         {
            CHECK( p(i) == details::fieldValueB<double>(i,0,0) );
         }
      }
   }
  
  
   {
      std::vector<std::pair<int,int>> jks = {{2,2}};
      for(auto jk: jks)
      {
         auto j = jk.first;
         auto k = jk.second;
         auto&& p = field.profile(j, k);
         for(int i = 0; i < dim1D; i++)
         {
            CHECK( p(i) == details::fieldValueA<double>(i,j,k) );
         }
      }
   }
}

TEST_CASE("ViewScalarField add profile data", "[ViewScalarField::addProfile]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   std::vector<double> pData(dim1D);
   QuICC::View::ViewBase<double> pView(pData.data(), pData.size());
   std::array<std::uint32_t, 1> dims = {static_cast<std::uint32_t>(dim1D)};
   QuICC::View::View<double, QuICC::View::dense1D> np(pView, dims);
   for(int i = 0; i < dim1D; i++)
   {
      np(i) = details::fieldValueB<double>(i,0,0);
   }

   std::vector<std::pair<int,int>> jks = {{0,0}, {3,2}, {3,3}, {1,4}};
   for(auto jk: jks)
   {
      field.addProfile(np, jk.first, jk.second);
   }
  
   {
      std::vector<std::pair<int,int>> jks = {{0,0}, {3,2}, {3,3}, {1,4}};
      for(auto jk: jks)
      {
         int j = jk.first;
         int k = jk.second;
         auto&& p = field.profile(j, k);
         for(int i = 0; i < dim1D; i++)
         {
            CHECK( p(i) == details::fieldValueA<double>(i,j,k) +  (details::fieldValueB<double>(i,0,0)) );
         }
      }
   }
  
   {
      std::vector<std::pair<int,int>> jks = {{2,2}};
      for(auto jk: jks)
      {
         int j = jk.first;
         int k = jk.second;
         auto&& p = field.profile(j, k);
         for(int i = 0; i < dim1D; i++)
         {
            CHECK( p(i) == details::fieldValueA<double>(i,j,k) );
         }
      }
   }
}

TEST_CASE("ViewScalarField subtract profile data", "[ViewScalarField::subProfile]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   std::vector<double> pData(dim1D);
   QuICC::View::ViewBase<double> pView(pData.data(), pData.size());
   std::array<std::uint32_t, 1> dims = {static_cast<std::uint32_t>(dim1D)};
   QuICC::View::View<double, QuICC::View::dense1D> np(pView, dims);
   for(int i = 0; i < dim1D; i++)
   {
      np(i) = details::fieldValueB<double>(i,0,0);
   }

   std::vector<std::pair<int,int>> jks = {{0,0}, {3,2}, {3,3}, {1,4}};
   for(auto jk: jks)
   {
      field.subProfile(np, jk.first, jk.second);
   }
  
   {
      std::vector<std::pair<int,int>> jks = {{0,0}, {3,2}, {3,3}, {1,4}};
      for(auto jk: jks)
      {
         int j = jk.first;
         int k = jk.second;
         auto&& p = field.profile(j, k);
         for(int i = 0; i < dim1D; i++)
         {
            CHECK( p(i) == details::fieldValueA<double>(i,j,k) - (details::fieldValueB<double>(i,0,0)) );
         }
      }
   }
  
   {
      std::vector<std::pair<int,int>> jks = {{2,2}};
      for(auto jk: jks)
      {
         int j = jk.first;
         int k = jk.second;
         auto&& p = field.profile(j, k);
         for(int i = 0; i < dim1D; i++)
         {
            CHECK( p(i) == details::fieldValueA<double>(i,j,k) );
         }
      }
   }
}

TEST_CASE("ViewScalarField set slice data", "[ViewScalarField::setSlice]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   {
      std::vector<double> sdata(dim1D*(dim3D + 2));
      QuICC::View::ViewBase<double> sView(sdata.data(), sdata.size());

      std::vector<int> ks = {0, 2, 4};

      for(int k: ks)
      {
         std::array<std::uint32_t, 2> dims = {static_cast<std::uint32_t>(dim1D), static_cast<std::uint32_t>(k + 2)};
         QuICC::View::View<double, QuICC::View::dense2D> ns(sView, {dims});
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               ns(i, j) = details::fieldValueB<double>(i,j,k);
            }
         }

         field.setSlice(ns, k);
      }
   }

   {
      std::vector<int> ks = {0, 2, 4};
      for(int k: ks)
      {
         auto&& s = field.slice(k);
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               CHECK( s(i, j) == details::fieldValueB<double>(i,j,k) );
            }
         }
      }
   }

   {
      std::vector<int> ks = {1};
      for(int k: ks)
      {
         auto&& s = field.slice(k);
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) );
            }
         }
      }
   }

}

TEST_CASE("ViewScalarField add slice data", "[ViewScalarField::addSlice]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   {
      std::vector<double> sdata(dim1D*(dim3D + 2));
      QuICC::View::ViewBase<double> sView(sdata.data(), sdata.size());

      std::vector<int> ks = {0, 2, 4};

      for(int k: ks)
      {
         std::array<std::uint32_t, 2> dims = {static_cast<std::uint32_t>(dim1D), static_cast<std::uint32_t>(k + 2)};
         QuICC::View::View<double, QuICC::View::dense2D> ns(sView, {dims});
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               ns(i, j) = details::fieldValueB<double>(i,j,k);
            }
         }

         field.addSlice(ns, k);
      }
   }

   {
      std::vector<int> ks = {0,2,4};
      for(int k: ks)
      {
         auto&& s = field.slice(k);
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) + (details::fieldValueB<double>(i,j,k)) );
            }
         }
      }
   }

   {
      std::vector<int> ks = {1};
      for(int k: ks)
      {
         auto&& s = field.slice(k);
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) );
            }
         }
      }
   }
}

TEST_CASE("ViewScalarField subtract slice data", "[ViewScalarField::subSlice]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   {
      std::vector<double> sdata(dim1D*(dim3D + 2));
      QuICC::View::ViewBase<double> sView(sdata.data(), sdata.size());

      std::vector<int> ks = {0, 2, 4};

      for(int k: ks)
      {
         std::array<std::uint32_t, 2> dims = {static_cast<std::uint32_t>(dim1D), static_cast<std::uint32_t>(k + 2)};
         QuICC::View::View<double, QuICC::View::dense2D> ns(sView, {dims});
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               ns(i, j) = details::fieldValueB<double>(i,j,k);
            }
         }

         field.subSlice(ns, k);
      }
   }

   {
      std::vector<int> ks = {0,2,4};
      for(int k: ks)
      {
         auto&& s = field.slice(k);
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) - (details::fieldValueB<double>(i,j,k)) );
            }
         }
      }
   }

   {
      std::vector<int> ks = {1};
      for(int k: ks)
      {
         auto&& s = field.slice(k);
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) );
            }
         }
      }
   }
}

TEST_CASE("ViewScalarField set top rows of slice data", "[ViewScalarField::setSlice_rows]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   {
      std::vector<double> sdata(dim1D*(dim3D + 2));
      QuICC::View::ViewBase<double> sView(sdata.data(), sdata.size());

      std::vector<int> ks = {0, 2, 4};

      for(int k: ks)
      {
         std::array<std::uint32_t, 2> dims = {static_cast<std::uint32_t>(dim1D/2), static_cast<std::uint32_t>(k + 2)};
         QuICC::View::View<double, QuICC::View::dense2D> ns(sView, {dims});
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D/2; i++)
            {
               ns(i, j) = details::fieldValueB<double>(i,j,k);
            }
         }

         field.setSlice(ns, k, dim1D/2);
      }
   }

   {
      std::vector<int> ks = {0,2,4};
      for(int k: ks)
      {
         auto&& s = field.slice(k);
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D/2; i++)
            {
               CHECK( s(i, j) == details::fieldValueB<double>(i,j,k) );
            }
            for(int i = dim1D/2; i < dim1D; i++)
            {
               CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) );
            }
         }
      }
   }

   {
      std::vector<int> ks = {1};
      for(int k: ks)
      {
         auto&& s = field.slice(k);
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) );
            }
         }
      }
   }
}

TEST_CASE("ViewScalarField add to top rows of slice data", "[ViewScalarField::addSlice_rows]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   {
      std::vector<double> sdata(dim1D*(dim3D + 2));
      QuICC::View::ViewBase<double> sView(sdata.data(), sdata.size());

      std::vector<int> ks = {0, 2, 4};

      for(int k: ks)
      {
         std::array<std::uint32_t, 2> dims = {static_cast<std::uint32_t>(dim1D), static_cast<std::uint32_t>(k + 2)};
         QuICC::View::View<double, QuICC::View::dense2D> ns(sView, {dims});
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D/2; i++)
            {
               ns(i, j) = details::fieldValueB<double>(i,j,k);
            }
         }

         field.addSlice(ns, k, dim1D/2);
      }
   }

   {
      std::vector<int> ks = {0,2,4};
      for(int k: ks)
      {
         auto&& s = field.slice(k);
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D/2; i++)
            {
               CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) + (details::fieldValueB<double>(i,j,k)) );
            }
            for(int i = dim1D/2; i < dim1D; i++)
            {
               CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) );
            }
         }
      }
   }

   {
      std::vector<int> ks = {1};
      for(int k: ks)
      {
         auto&& s = field.slice(k);
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) );
            }
         }
      }
   }
}

TEST_CASE("ViewScalarField subtract from top rows of slice data", "[ViewScalarField::subSlice_rows]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   {
      std::vector<double> sdata(dim1D*(dim3D + 2));
      QuICC::View::ViewBase<double> sView(sdata.data(), sdata.size());

      std::vector<int> ks = {0, 2, 4};

      for(int k: ks)
      {
         std::array<std::uint32_t, 2> dims = {static_cast<std::uint32_t>(dim1D), static_cast<std::uint32_t>(k + 2)};
         QuICC::View::View<double, QuICC::View::dense2D> ns(sView, {dims});
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D/2; i++)
            {
               ns(i, j) = details::fieldValueB<double>(i,j,k);
            }
         }

         field.subSlice(ns, k, dim1D/2);
      }
   }

   {
      std::vector<int> ks = {0,2,4};
      for(int k: ks)
      {
         auto&& s = field.slice(k);
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D/2; i++)
            {
               CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) - (details::fieldValueB<double>(i,j,k)) );
            }
            for(int i = dim1D/2; i < dim1D; i++)
            {
               CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) );
            }
         }
      }
   }

   {
      int k = 1;
      auto&& s = field.slice(k);
      for(int j = 0; j < k + 2; j++)
      {
         for(int i = 0; i < dim1D; i++)
         {
            CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) );
         }
      }
   }

   {
      int k = 2;
      auto&& s = field.slice(k);
      for(int j = 0; j < k + 2; j++)
      {
         for(int i = 0; i < dim1D/2; i++)
         {
            CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) - (details::fieldValueB<double>(i,j,k)) );
         }
         for(int i = dim1D/2; i < dim1D; i++)
         {
            CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) );
         }
      }
   }

   {
      int k = 4;
      auto&& s = field.slice(k);
      for(int j = 0; j < k + 2; j++)
      {
         for(int i = 0; i < dim1D/2; i++)
         {
            CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) - (details::fieldValueB<double>(i,j,k)) );
         }
         for(int i = dim1D/2; i < dim1D; i++)
         {
            CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) );
         }
      }
   }
}

TEST_CASE("ViewScalarField get data pointer", "[ViewScalarField::data]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   {
      int k = 0;
      auto&& dptr = field.data(k);
      
      CHECK( dptr[0] == 0 );
      CHECK( dptr[3] == 3 );
      CHECK( dptr[1*dim1D + 3] == 1*100 + 3 );
   }

   {
      int k = 2;
      auto&& dptr = field.data(k);
      
      CHECK( dptr[0] == k*100*100 + 0 );
      CHECK( dptr[3] == k*100*100 + 3 );
      CHECK( dptr[1*dim1D + 1] == k*100*100 + 101 );
   }

   {
      int k = 4;
      auto&& dptr = field.data(k);
      
      CHECK( dptr[0] == k*100*100 + 0 );
      CHECK( dptr[3] == k*100*100 + 3 );
      CHECK( dptr[1*dim1D + 2] == k*100*100 + 102 );
   }
}

TEST_CASE("ViewScalarField get data reference", "[ViewScalarField::data_ref]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   {
      auto&& mat = field.data();
      
      CHECK( mat(0, 0, 0) == 0 );
      CHECK( mat(3, 0, 0) == 3 );
      CHECK( mat(2, 1, 0) == 1*100 + 2 );
   }

   {
      auto&& mat = field.data();
      
      CHECK( mat(0, 0, 2) == 2*100*100 + 0 );
      CHECK( mat(3, 0, 2) == 2*100*100 + 3 );
      CHECK( mat(1, 1, 2) == 2*100*100 + 1*100 + 1 );
   }

   {
      auto&& mat = field.data();
      
      CHECK( mat(0, 0, 4) == 4*100*100 + 0 );
      CHECK( mat(3, 0, 4) == 4*100*100 + 3 );
      CHECK( mat(3, 1, 4) == 4*100*100 + 1*100 + 3 );
   }
}

TEST_CASE("ViewScalarField set data", "[ViewScalarField::setData]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   {
      std::vector<std::uint32_t> pointers = {0};
      std::vector<std::uint32_t> indices;

      QuICC::Matrix dMat(dim1D, (dim3D*(dim3D + 3))/2);
      dMat.setZero();
      int j_ = 0;
      for(int k = 0; k < dim3D; k++)
      {
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               dMat(i, j_) = details::fieldValueB<double>(i,j,k);
            }
            j_++;
            indices.push_back(static_cast<std::uint32_t>(j));
         }
         pointers.push_back(static_cast<std::uint32_t>(j_));
      }

      std::array<std::uint32_t, 3> dims{static_cast<std::uint32_t>(dim1D), static_cast<std::uint32_t>(dim3D) - 1 + 2, static_cast<std::uint32_t>(dim3D)};
      QuICC::View::ViewBase<std::uint32_t> vPointers[3];
      QuICC::View::ViewBase<std::uint32_t> vIndices[3];
      vPointers[1] = QuICC::View::ViewBase<std::uint32_t>(pointers.data(), pointers.size());
      vIndices[1] = QuICC::View::ViewBase<std::uint32_t>(indices.data(), indices.size());

      QuICC::View::View<double, QuICC::View::DCCSC3D> nd(dMat.data(), dMat.size(), dims.data(), vPointers, vIndices);

      field.setData(nd);
   }

   {
      std::vector<int> ks = {0, 2, 4};
      for(auto&& k: ks)
      {
         auto&& s = field.slice(k);
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               CHECK( s(i, j) == details::fieldValueB<double>(i,j,k) );
            }
         }
      }
   }
}

TEST_CASE("ViewScalarField set data with flipped sign", "[ViewScalarField::setNegData]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   {
      std::vector<std::uint32_t> pointers = {0};
      std::vector<std::uint32_t> indices;

      QuICC::Matrix dMat(dim1D, (dim3D*(dim3D + 3))/2);
      dMat.setZero();
      int j_ = 0;
      for(int k = 0; k < dim3D; k++)
      {
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               dMat(i, j_) = details::fieldValueB<double>(i,j,k);
            }
            j_++;
            indices.push_back(static_cast<std::uint32_t>(j));
         }
         pointers.push_back(static_cast<std::uint32_t>(j_));
      }

      std::array<std::uint32_t, 3> dims{static_cast<std::uint32_t>(dim1D), static_cast<std::uint32_t>(dim3D) - 1 + 2, static_cast<std::uint32_t>(dim3D)};
      QuICC::View::ViewBase<std::uint32_t> vPointers[3];
      QuICC::View::ViewBase<std::uint32_t> vIndices[3];
      vPointers[1] = QuICC::View::ViewBase<std::uint32_t>(pointers.data(), pointers.size());
      vIndices[1] = QuICC::View::ViewBase<std::uint32_t>(indices.data(), indices.size());

      QuICC::View::View<double, QuICC::View::DCCSC3D> nd(dMat.data(), dMat.size(), dims.data(), vPointers, vIndices);

      field.setNegData(nd);
   }

   {
      std::vector<int> ks = {0, 2, 4};
      for(auto&& k: ks)
      {
         auto&& s = field.slice(k);
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               CHECK( s(i, j) == -(details::fieldValueB<double>(i,j,k)) );
            }
         }
      }
   }
}

TEST_CASE("ViewScalarField add data", "[ViewScalarField::addData]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   {
      std::vector<std::uint32_t> pointers = {0};
      std::vector<std::uint32_t> indices;

      QuICC::Matrix dMat(dim1D, ((dim3D)*(dim3D + 3))/2);
      dMat.setZero();
      int j_ = 0;
      for(int k = 0; k < dim3D; k++)
      {
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               dMat(i, j_) = details::fieldValueB<double>(i,j,k);
            }
            j_++;
            indices.push_back(static_cast<std::uint32_t>(j));
         }
         pointers.push_back(static_cast<std::uint32_t>(j_));
      }

      std::array<std::uint32_t, 3> dims{static_cast<std::uint32_t>(dim1D), static_cast<std::uint32_t>(dim3D) - 1 + 2, static_cast<std::uint32_t>(dim3D)};
      QuICC::View::ViewBase<std::uint32_t> vPointers[3];
      QuICC::View::ViewBase<std::uint32_t> vIndices[3];
      vPointers[1] = QuICC::View::ViewBase<std::uint32_t>(pointers.data(), pointers.size());
      vIndices[1] = QuICC::View::ViewBase<std::uint32_t>(indices.data(), indices.size());

      QuICC::View::View<double, QuICC::View::DCCSC3D> nd(dMat.data(), dMat.size(), dims.data(), vPointers, vIndices);

      field.addData(nd);
   }

   {
      std::vector<int> ks = {0, 2, 4};
      for(auto&& k: ks)
      {
         auto&& s = field.slice(k);
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) + (details::fieldValueB<double>(i,j,k)) );
            }
         }
      }
   }
}

TEST_CASE("ViewScalarField subtract data", "[ViewScalarField::subData]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   {
      std::vector<std::uint32_t> pointers = {0};
      std::vector<std::uint32_t> indices;

      QuICC::Matrix dMat(dim1D, ((dim3D)*(dim3D + 3))/2);
      dMat.setZero();
      int j_ = 0;
      for(int k = 0; k < dim3D; k++)
      {
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               dMat(i, j_) = details::fieldValueB<double>(i,j,k);
            }
            j_++;
            indices.push_back(static_cast<std::uint32_t>(j));
         }
         pointers.push_back(static_cast<std::uint32_t>(j_));
      }

      std::array<std::uint32_t, 3> dims{static_cast<std::uint32_t>(dim1D), static_cast<std::uint32_t>(dim3D) - 1 + 2, static_cast<std::uint32_t>(dim3D)};
      QuICC::View::ViewBase<std::uint32_t> vPointers[3];
      QuICC::View::ViewBase<std::uint32_t> vIndices[3];
      vPointers[1] = QuICC::View::ViewBase<std::uint32_t>(pointers.data(), pointers.size());
      vIndices[1] = QuICC::View::ViewBase<std::uint32_t>(indices.data(), indices.size());

      QuICC::View::View<double, QuICC::View::DCCSC3D> nd(dMat.data(), dMat.size(), dims.data(), vPointers, vIndices);

      field.subData(nd);
   }

   {
      std::vector<int> ks = {0, 2, 4};
      for(auto&& k: ks)
      {
         auto&& s = field.slice(k);
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) - (details::fieldValueB<double>(i,j,k)) );
            }
         }
      }
   }
}

TEST_CASE("ViewScalarField set zeros", "[ViewScalarField::setZeros]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   field.setZeros();

   {
      std::vector<int> ks = {0, 2, 4};
      for(auto&& k: ks)
      {
         auto&& s = field.slice(k);
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               CHECK( s(i, j) == 0 );
            }
         }
      }
   }
}

TEST_CASE("ViewScalarField rescale data by scalar", "[ViewScalarField::rescale]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   double c = -3.1415;
   field.rescale(c);

   std::vector<int> ks = {0, 2, 4};
   for(auto&& k: ks)
   {
      auto&& s = field.slice(k);
      for(int j = 0; j < k + 2; j++)
      {
         for(int i = 0; i < dim1D; i++)
         {
            CHECK( s(i, j) == c*(details::fieldValueA<double>(i,j,k)) );
         }
      }
   }
}

TEST_CASE("ViewScalarField get number of slices", "[ViewScalarField::nSlice]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   CHECK( field.nSlice() == dim3D );
}

TEST_CASE("ViewScalarField set slice data from pointer", "[ViewScalarField::rData]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   {
      std::vector<int> ks = {0, 2, 4};
      for(auto&& k: ks)
      {
         double* dptr = field.rData(k);

         int ii = 0;
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               dptr[ii] = details::fieldValueB<double>(i,j,k);
               ii++;
            }
         }
      }
   }

   {
      std::vector<int> ks = {0, 2, 4};
      for(auto&& k: ks)
      {
         auto&& s = field.slice(k);
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               CHECK( s(i, j) == details::fieldValueB<double>(i,j,k) );
            }
         }
      }
   }

   {
      std::vector<int> ks = {1};
      for(auto&& k: ks)
      {
         auto&& s = field.slice(k);
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               CHECK( s(i, j) == details::fieldValueA<double>(i,j,k) );
            }
         }
      }
   }
}

#if 0
TEST_CASE("ViewScalarField set whole data from reference", "[ViewScalarField::rData_ref]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   {
      QuICC::Matrix nd(dim1D, ((dim3D + 1)*(dim3D + 4))/2);
      nd.setZero();
      int j_ = 0;
      for(int k = 0; k < dim3D; k++)
      {
         for(int j = 0; j < k + 2; j++)
         {
            for(int i = 0; i < dim1D; i++)
            {
               nd(i, j_) = details::fieldValueB<double>(i,j,k);
            }
            j_++;
         }
      }

      field.rData() = nd;
   }

   {
      int k = 0;
      auto&& s = field.slice(k);
      for(int j = 0; j < k + 2; j++)
      {
         for(int i = 0; i < dim1D; i++)
         {
            CHECK( s(i, j) == details::fieldValueB<double>(i,j,k) );
         }
      }
   }

   {
      int k = 2;
      auto&& s = field.slice(k);
      for(int j = 0; j < k + 2; j++)
      {
         for(int i = 0; i < dim1D; i++)
         {
            CHECK( s(i, j) == details::fieldValueB<double>(i,j,k) );
         }
      }
   }

   {
      int k = 4;
      auto&& s = field.slice(k);
      for(int j = 0; j < k + 2; j++)
      {
         for(int i = 0; i < dim1D; i++)
         {
            CHECK( s(i, j) == details::fieldValueB<double>(i,j,k) );
         }
      }
   }
}
#endif

TEST_CASE("ViewScalarField set point data from reference", "[ViewScalarField::rPoint]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   field.rPoint(0, 1, 0) = -700.007;
   field.rPoint(2, 1, 1) = -700.008;
   field.rPoint(7, 3, 3) = -700.009;
   field.rPoint(dim1D-1, 1, 4) = -700.017;

   CHECK( field.point(0, 1, 0) == -700.007);
   CHECK( field.point(2, 1, 1) == -700.008);
   CHECK( field.point(7, 3, 3) == -700.009);
   CHECK( field.point(dim1D-1, 1, 4) == -700.017);
}

TEST_CASE("ViewScalarField set point data from reference from coord", "[ViewScalarField::rPoint_coord]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   std::vector<int> coord = {0, 1, 0};
   field.rPoint(coord) = -700.007;
   coord = {2, 1, 1};
   field.rPoint(coord) = -700.008;
   coord = {7, 3, 3};
   field.rPoint(coord) = -700.009;
   coord = {dim1D-1, 1, 4};
   field.rPoint(coord) = -700.017;

   CHECK( field.point(0, 1, 0) == -700.007);
   CHECK( field.point(2, 1, 1) == -700.008);
   CHECK( field.point(7, 3, 3) == -700.009);
   CHECK( field.point(dim1D-1, 1, 4) == -700.017);
}

TEST_CASE("ViewScalarField copy constructor", "[ViewScalarField::Copy]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spFieldA = details::createFlatScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);
   auto spFieldB = details::createFlatScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);
   auto spFieldC = details::createFlatScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& fieldA = *spFieldA;
   fieldA.setZeros();
   {
      auto ptr = fieldA.data().data();

      for(int i = 0; i < fieldA.data().size(); i++)
      {
         CHECK( *ptr == 0 );
         ptr++;
      }
   }

   auto&& fieldB = *spFieldB;
   auto&& fieldC = *spFieldC;

   fieldA = fieldB;
   {
      auto ptrA = fieldA.data().data();
      auto ptrC = fieldC.data().data();

      for(int i = 0; i < fieldA.data().size(); i++)
      {
         CHECK( *ptrA == *ptrC );
         ptrA++;
         ptrC++;
      }
   }

   fieldB.setZeros();
   {
      auto ptrA = fieldA.data().data();

      for(int i = 0; i < fieldA.data().size(); i++)
      {
         CHECK( *ptrA == 0 );
         ptrA++;
      }
   }
}

TEST_CASE("ViewScalarField get component by reference", "[ViewScalarField::comp]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   CHECK( &(field.comp(QuICC::FieldComponents::Spectral::SCALAR)) == &field );
}

TEST_CASE("ViewScalarField set component by reference", "[ViewScalarField::rComp]")
{
   int dim3D = 5;
   int dim1D = 2*dim3D;
   auto spField = details::createViewScalarField<double>(details::SetupType::UniformUp, dim1D, dim3D);

   auto&& field = *spField;

   CHECK( &(field.rComp(QuICC::FieldComponents::Spectral::SCALAR)) == &field );
}
