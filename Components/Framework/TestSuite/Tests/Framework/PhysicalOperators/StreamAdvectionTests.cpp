   ...
   for(std::uint32_t varBase: variants)
   {
      ...
      for(auto&& c: cs)
      {
         ...
         // Timing FlatScalarField
         {
            ...
            {
               QuICC::Profiler::RegionStart<1>("StreamAdvectionTests::Flat" + ctag + "Set_" + std::to_string(varBase));
               TestOp<R,T>::set(*spScalarA, *spVectorA, *spVectorB, c);
               QuICC::Profiler::RegionStop<1>("StreamAdvectionTests::Flat" + ctag + "Set_" + std::to_string(varBase));

               QuICC::Profiler::RegionStart<1>("StreamAdvectionTests::Flat" + ctag + "Add_" + std::to_string(varBase));
               TestOp<R,T>::add(*spScalarA, *spVectorA, *spVectorB, c);
               QuICC::Profiler::RegionStop<1>("StreamAdvectionTests::Flat" + ctag + "Add_" + std::to_string(varBase));

               QuICC::Profiler::RegionStart<1>("StreamAdvectionTests::Flat" + ctag + "Sub_" + std::to_string(varBase));
               TestOp<R,T>::sub(*spScalarA, *spVectorA, *spVectorB, c);
               QuICC::Profiler::RegionStop<1>("StreamAdvectionTests::Flat" + ctag + "Sub_" + std::to_string(varBase));
            }
         }
         ...
         // Timing ViewScalarField
         {
            ...
            for(std::uint32_t it = 0; it < itMax; it++)
            {
               QuICC::Profiler::RegionStart<1>("StreamAdvectionTests::View" + ctag + "Set_" + std::to_string(varBase));
               TestOp<R,T>::set(*spScalarA, *spVectorA, *spVectorB, c);
               QuICC::Profiler::RegionStop<1>("StreamAdvectionTests::View" + ctag + "Set_" + std::to_string(varBase));

               QuICC::Profiler::RegionStart<1>("StreamAdvectionTests::View" + ctag + "Add_" + std::to_string(varBase));
               TestOp<R,T>::add(*spScalarA, *spVectorA, *spVectorB, c);
               QuICC::Profiler::RegionStop<1>("StreamAdvectionTests::View" + ctag + "Add_" + std::to_string(varBase));

               QuICC::Profiler::RegionStart<1>("StreamAdvectionTests::View" + ctag + "Sub_" + std::to_string(varBase));
               TestOp<R,T>::sub(*spScalarA, *spVectorA, *spVectorB, c);
               QuICC::Profiler::RegionStop<1>("StreamAdvectionTests::View" + ctag + "Sub_" + std::to_string(varBase));
            }
         }
      }
   }
