   ...
   for(std::uint32_t varBase: variants)
   {
      ...
      for(auto&& c: cs)
      {
         ...
         // Timing FlatScalarField
         {
            ...
            {
               QuICC::Profiler::RegionStart<1>("DotTests::Flat" + ctag + "Set_" + std::to_string(varBase));
               TestOp::set(*spScalarA, *spVectorA, *spVectorB, c);
               QuICC::Profiler::RegionStop<1>("DotTests::Flat" + ctag + "Set_" + std::to_string(varBase));

               QuICC::Profiler::RegionStart<1>("DotTests::Flat" + ctag + "Add_" + std::to_string(varBase));
               TestOp::add(*spScalarA, *spVectorA, *spVectorB, c);
               QuICC::Profiler::RegionStop<1>("DotTests::Flat" + ctag + "Add_" + std::to_string(varBase));

               QuICC::Profiler::RegionStart<1>("DotTests::Flat" + ctag + "Sub_" + std::to_string(varBase));
               TestOp::sub(*spScalarA, *spVectorA, *spVectorB, c);
               QuICC::Profiler::RegionStop<1>("DotTests::Flat" + ctag + "Sub_" + std::to_string(varBase));
            }
         }
         ...
         // Timing ViewScalarField
         {
            ...
            for(std::uint32_t it = 0; it < itMax; it++)
            {
               QuICC::Profiler::RegionStart<1>("DotTests::View" + ctag + "Set_" + std::to_string(varBase));
               TestOp::set(*spScalarA, *spVectorA, *spVectorB, c);
               QuICC::Profiler::RegionStop<1>("DotTests::View" + ctag + "Set_" + std::to_string(varBase));

               QuICC::Profiler::RegionStart<1>("DotTests::View" + ctag + "Add_" + std::to_string(varBase));
               TestOp::add(*spScalarA, *spVectorA, *spVectorB, c);
               QuICC::Profiler::RegionStop<1>("DotTests::View" + ctag + "Add_" + std::to_string(varBase));

               QuICC::Profiler::RegionStart<1>("DotTests::View" + ctag + "Sub_" + std::to_string(varBase));
               TestOp::sub(*spScalarA, *spVectorA, *spVectorB, c);
               QuICC::Profiler::RegionStop<1>("DotTests::View" + ctag + "Sub_" + std::to_string(varBase));
            }
         }
      }
   }
