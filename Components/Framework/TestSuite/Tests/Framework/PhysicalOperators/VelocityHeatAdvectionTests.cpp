...
template <QuICC::FieldComponents::Physical::Id TONE, QuICC::FieldComponents::Physical::Id TTWO, QuICC::FieldComponents::Physical::Id TTHREE>
   using TestOp = QuICC::Physical::VelocityHeatAdvection<TONE,TTWO,TTHREE>;

TEST_CASE("VelocityHeatAdvection validation FlatScalarField vs ViewScalarField", "[VelocityHeatAdvectionValidation]")
{
...
TEST_CASE("VelocityHeatAdvection timing", "[VelocityHeatAdvectionTiming]")
{
   constexpr auto R = QuICC::FieldComponents::Physical::R;
   constexpr auto T = QuICC::FieldComponents::Physical::THETA;
   constexpr auto P = QuICC::FieldComponents::Physical::PHI;
   ...
   for(std::uint32_t varBase: variants)
   {
      ...
      for(auto&& c: cs)
      {
         ...
         // Timing FlatScalarField
         {
            ...
            {
               QuICC::Profiler::RegionStart<1>("VelocityHeatAdvectionTests::Flat" + ctag + "Set_" + std::to_string(varBase));
               TestOp<R,T,P>::set(*spScalarA, *spVectorA, *spVectorB, c);
               QuICC::Profiler::RegionStop<1>("VelocityHeatAdvectionTests::Flat" + ctag + "Set_" + std::to_string(varBase));

               QuICC::Profiler::RegionStart<1>("VelocityHeatAdvectionTests::Flat" + ctag + "Add_" + std::to_string(varBase));
               TestOp<R,T,P>::add(*spScalarA, *spVectorA, *spVectorB, c);
               QuICC::Profiler::RegionStop<1>("VelocityHeatAdvectionTests::Flat" + ctag + "Add_" + std::to_string(varBase));

               QuICC::Profiler::RegionStart<1>("VelocityHeatAdvectionTests::Flat" + ctag + "Sub_" + std::to_string(varBase));
               TestOp<R,T,P>::sub(*spScalarA, *spVectorA, *spVectorB, c);
               QuICC::Profiler::RegionStop<1>("VelocityHeatAdvectionTests::Flat" + ctag + "Sub_" + std::to_string(varBase));
            }
         }
         ...
         // Timing ViewScalarField
         {
            ...
            for(std::uint32_t it = 0; it < itMax; it++)
            {
               QuICC::Profiler::RegionStart<1>("VelocityHeatAdvectionTests::View" + ctag + "Set_" + std::to_string(varBase));
               TestOp<R,T,P>::set(*spScalarA, *spVectorA, *spVectorB, c);
               QuICC::Profiler::RegionStop<1>("VelocityHeatAdvectionTests::View" + ctag + "Set_" + std::to_string(varBase));

               QuICC::Profiler::RegionStart<1>("VelocityHeatAdvectionTests::View" + ctag + "Add_" + std::to_string(varBase));
               TestOp<R,T,P>::add(*spScalarA, *spVectorA, *spVectorB, c);
               QuICC::Profiler::RegionStop<1>("VelocityHeatAdvectionTests::View" + ctag + "Add_" + std::to_string(varBase));

               QuICC::Profiler::RegionStart<1>("VelocityHeatAdvectionTests::View" + ctag + "Sub_" + std::to_string(varBase));
               TestOp<R,T,P>::sub(*spScalarA, *spVectorA, *spVectorB, c);
               QuICC::Profiler::RegionStop<1>("VelocityHeatAdvectionTests::View" + ctag + "Sub_" + std::to_string(varBase));
            }
         }
      }
   }
