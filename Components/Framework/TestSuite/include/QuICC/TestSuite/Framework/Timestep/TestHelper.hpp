...
#ifndef QUICC_TESTSUITE_FRAMEWORK_TIMESTEP_TESTHELPER_HPP
#define QUICC_TESTSUITE_FRAMEWORK_TIMESTEP_TESTHELPER_HPP
...
#include "QuICC/TestSuite/Framework/Timestep/Test.hpp"
...
namespace Timestep {
   ...
   template <typename TScheme> std::shared_ptr<typename TScheme::template ImplementationType<SparseMatrix,DecoupledZMatrix,::QuICC::Framework::Selector::SparseSolver> > createTimestepper(Test& test)
   {
      typedef typename TScheme::template ImplementationType<SparseMatrix,DecoupledZMatrix,::QuICC::Framework::Selector::SparseSolver> Timestepper;

      const auto& start = test.start;
      auto spStepper = std::make_shared<Timestepper>(start, SolveTiming::Prognostic::id());
   ...
   template <typename TScheme> void setupTimestepper(Test& test, std::shared_ptr<typename TScheme::template ImplementationType<SparseMatrix,DecoupledZMatrix,::QuICC::Framework::Selector::SparseSolver> > spStepper)
   {
      ...
      auto fieldId = std::make_pair(PhysicalNames::Temperature::id(), FieldComponents::Spectral::SCALAR);
   ...
   template <typename TScheme> void singleStepForward(Test& test, std::shared_ptr<typename TScheme::template ImplementationType<SparseMatrix,DecoupledZMatrix,::QuICC::Framework::Selector::SparseSolver> > spStepper)
   {
      const auto& nN = test.nN;
      const auto& cols = test.cols;
   ...
   template <typename TScheme> void multiStepForward(Test& test, std::shared_ptr<typename TScheme::template ImplementationType<SparseMatrix,DecoupledZMatrix,::QuICC::Framework::Selector::SparseSolver> > spStepper, const int steps)
   {
      const auto& nN = test.nN;
      const auto& cols = test.cols;
      const auto& tEnd = test.tEnd;
...
#endif //QUICC_TESTSUITE_FRAMEWORK_TIMESTEP_TESTHELPER_HPP
