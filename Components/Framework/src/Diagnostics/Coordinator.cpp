...
namespace Diagnostics {
   ...
   void Coordinator::init(const std::vector<Array>& mesh, const std::map<std::size_t, Framework::Selector::VariantSharedScalarVariable>&  scalars, const std::map<std::size_t, Framework::Selector::VariantSharedVectorVariable>&  vectors, const Array& tstep, const std::map<std::size_t,NonDimensional::SharedINumber>& params)
   {
      ...
      } else if(vectors.count(PhysicalNames::Velocity::id()) && vectors.count(PhysicalNames::Magnetic::id()))
      {
         namespace S = SpatialScheme;
         std::shared_ptr<const S::ISpatialScheme> spScheme = std::visit([](auto&& p){return p->dom(0).res().sim().spSpatialScheme();}, vectors.find(PhysicalNames::Velocity::id())->second);

         // Create a toroidal/poloidal spherical shell wrapper
         if(spScheme->has(SpatialScheme::Feature::ShellGeometry) && spScheme->formulation() == VectorFormulation::TORPOL)
         {
            auto spVelocity = std::make_shared<SphericalTorPolWrapper>(vectors.find(PhysicalNames::Velocity::id())->second);
            auto spMagnetic = std::make_shared<SphericalTorPolWrapper>(vectors.find(PhysicalNames::Magnetic::id())->second);

            this->mspCflWrapper = std::make_shared<ShellCflWrapper>(spVelocity, spMagnetic, params);
         // Create a full sphere wrapper
         } else if(spScheme->has(SpatialScheme::Feature::SphereGeometry) && spScheme->formulation() == VectorFormulation::TORPOL)
         {
            auto spVelocity = std::make_shared<SphericalTorPolWrapper>(vectors.find(PhysicalNames::Velocity::id())->second);
            auto spMagnetic = std::make_shared<SphericalTorPolWrapper>(vectors.find(PhysicalNames::Magnetic::id())->second);

            this->mspCflWrapper = std::make_shared<SphereCflWrapper>(spVelocity, spMagnetic, params);
         } else
         {
            throw std::logic_error("Could not setup velocity and magnetic wrappers");
         }
      ...
      } else if(vectors.count(PhysicalNames::Velocity::id()))
      {
         ...
         if(spScheme->has(SpatialScheme::Feature::CartesianGeometry) && spScheme->formulation() == VectorFormulation::TORPOL)
         {
            auto spVelocity = std::make_shared<CartesianTorPolWrapper>(vectors.find(PhysicalNames::Velocity::id())->second);

            this->mspCflWrapper = std::make_shared<CartesianCflWrapper>(spVelocity);

         // Create a toroidal/poloidal spherical shell wrapper
         } else if(spScheme->has(SpatialScheme::Feature::ShellGeometry) && spScheme->formulation() == VectorFormulation::TORPOL)
         {
            auto spVelocity = std::make_shared<SphericalTorPolWrapper>(vectors.find(PhysicalNames::Velocity::id())->second);

            this->mspCflWrapper = std::make_shared<ShellCflWrapper>(spVelocity, params);
         // Create a full sphere wrapper
         } else if(spScheme->has(SpatialScheme::Feature::SphereGeometry) && spScheme->formulation() == VectorFormulation::TORPOL)
         {
            auto spVelocity = std::make_shared<SphericalTorPolWrapper>(vectors.find(PhysicalNames::Velocity::id())->second);

            this->mspCflWrapper = std::make_shared<SphereCflWrapper>(spVelocity, params);
         } else
         {
            throw std::logic_error("Could not setup velocity wrapper for velocity");
         }

         this->mFixedStep = tstep(1);

      // Create a stream function and vertical velocity wrapper
      } else if(scalars.count(PhysicalNames::Streamfunction::id()) && scalars.count(PhysicalNames::VelocityZ::id()))
      {
         namespace S = SpatialScheme;
         std::shared_ptr<const S::ISpatialScheme> spScheme = std::visit([](auto&& p){return p->dom(0).res().sim().spSpatialScheme();}, vectors.find(PhysicalNames::Streamfunction::id())->second);

         if(spScheme->has(SpatialScheme::Feature::CartesianGeometry))
         {
            auto spVelocity = std::make_shared<StreamVerticalWrapper>(scalars.find(PhysicalNames::Streamfunction::id())->second, scalars.find(PhysicalNames::VelocityZ::id())->second);
