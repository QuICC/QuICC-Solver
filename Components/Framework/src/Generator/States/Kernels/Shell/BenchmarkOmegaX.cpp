/**
 * @file BenchmarkOmegaX.cpp
 * @brief Source of benchmark Omega X velocity field generator kernel
 */
...
// Class include
//
#include "QuICC/Generator/States/Kernels/Shell/BenchmarkOmegaX.hpp"
...
namespace Shell {
   ...
   BenchmarkOmegaX::BenchmarkOmegaX()
      : IPhysicalKernel()
   {
   }
   ...
   void BenchmarkOmegaX::init(const MHDFloat ri, const MHDFloat ro)
   {
      this->mRi = ri;
      this->mRo = ro;
   }

   void BenchmarkOmegaX::compute(Framework::Selector::PhysicalScalarField& rNLComp, FieldComponents::Physical::Id id) const
   {
      // Initialize to zero
