...
// Class include
//
#include "QuICC/Generator/States/Kernels/Shell/TorPolHarmonic.hpp"
...
namespace Shell {
   ...
   void TorPolHarmonic::init(const MHDFloat ri, const MHDFloat ro)
   {
      this->mRi = ri;
      this->mRo = ro;
   }
         ...
         for(int iR = 0; iR < nR; ++iR)
         {
            ...
            for(int iTh = 0; iTh < nTh; ++iTh)
            {
               ...
               for(ModeIt it = modeRange.first; it != modeRange.second; ++it)
               {
                  ...
                  {
                     MHDComplex tmp;
                     tmp.real(rIt->second.real()*std::pow(r,rIt->first));
                     tmp.imag(rIt->second.imag()*std::pow(r,rIt->first));
                     ct += tmp;
                  }
