/**
 * @file BenchmarkOmegaY.cpp
 * @brief Source of benchmark Omega Y velocity field generator kernel
 */
...
// Class include
//
#include "QuICC/Generator/States/Kernels/Shell/BenchmarkOmegaY.hpp"
...
namespace Shell {
   ...
   BenchmarkOmegaY::BenchmarkOmegaY()
      : IPhysicalKernel()
   {
   }
   ...
   void BenchmarkOmegaY::init(const MHDFloat ri, const MHDFloat ro)
   {
      this->mRi = ri;
      this->mRo = ro;
   }

   void BenchmarkOmegaY::compute(Framework::Selector::PhysicalScalarField& rNLComp, FieldComponents::Physical::Id id) const
   {
      // Initialize to zero
