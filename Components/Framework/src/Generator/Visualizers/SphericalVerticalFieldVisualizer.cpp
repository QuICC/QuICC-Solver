...
namespace Equations {
   ...
   SphericalVerticalFieldVisualizer::SphericalVerticalFieldVisualizer(SharedEquationParameters spEqParams, SpatialScheme::SharedCISpatialScheme spScheme, std::shared_ptr<Model::IModelBackend> spBackend)
   ...
   void SphericalVerticalFieldVisualizer::setIdentity(const std::size_t vertName, const std::size_t fieldName)
   {
      // Set the name
   ...
   void SphericalVerticalFieldVisualizer::initNLKernel(const bool force)
   {
      // Initialize if empty or forced
      if(force || !this->mspNLKernel)
      {
         // Initialize the physical kernel
         auto spNLKernel = std::make_shared<Physical::Kernel::Spherical::VerticalField>();
   ...
   void SphericalVerticalFieldVisualizer::useNonlinear(const Framework::Selector::PhysicalScalarField& rNLComp, FieldComponents::Physical::Id)
   {
