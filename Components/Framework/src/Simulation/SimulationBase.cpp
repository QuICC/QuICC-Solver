...
SimulationBase::SimulationBase() : mSimRunCtrl()
{
   QuICCTimer().start();
}
...
void SimulationBase::setConfiguration(const int dimension,
...
void SimulationBase::getConfig(std::map<std::string, MHDFloat>& cfg,
...
void SimulationBase::setInitialState(
   Io::Variable::SharedStateFileReader spInitFile)
{
   ...
   for (auto scalIt = this->mPseudospectral.imposedScalarVariables().begin();
        scalIt != this->mPseudospectral.imposedScalarVariables().end();
        scalIt++)
   {
      spInitFile->addScalar((*scalIt));
   }
   ...
   for (auto vectIt = this->mPseudospectral.imposedVectorVariables().begin();
        vectIt != this->mPseudospectral.imposedVectorVariables().end();
        vectIt++)
   {
      spInitFile->addVector((*vectIt));
   }
...
void SimulationBase::addAsciiOutputFile(
...
void SimulationBase::addStatsOutputFile(
   Io::Stats::SharedIStatisticsAsciiWriter spOutFile)
{
...
void SimulationBase::setupOutput()
{
   ...
   for (hdf5It = this->mSimIoCtrl.beginHdf5();
        hdf5It != this->mSimIoCtrl.endHdf5(); ++hdf5It)
   {
      // Loop over all scalars
      for (auto scalIt = this->mPseudospectral.scalarVariables().begin();
           scalIt != this->mPseudospectral.scalarVariables().end(); scalIt++)
   ...
   for (statsIt = this->mSimIoCtrl.beginStats();
        statsIt != this->mSimIoCtrl.endStats(); ++statsIt)
   {
      // Loop over all scalars
      for (auto scalIt = this->mPseudospectral.scalarVariables().begin();
           scalIt != this->mPseudospectral.scalarVariables().end(); scalIt++)
...
SimulationBase::spSpatialScheme() const
{
...
void SimulationBase::addConfigurationNode(
