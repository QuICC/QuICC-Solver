...
ArrayI SimulationConfig::dimension() const
{
   ...
   // Get truncation map
   std::map<std::string, int> trunc =
      this->mspCfgFile->spFramework()
         ->spNode(Io::Config::Framework::TRUNCATION)
         ->iTags()
         .map();
...
SimulationConfig::transformSetup() const
{
   // Safety assert for non NULL pointer
   assert(this->mspCfgFile);
   ...
   for (auto sit = range.first; sit != range.second; ++sit)
   {
      ...
      std::transform(sit->second.cbegin(), sit->second.cend(), tag.begin(),
...
Array SimulationConfig::boxScale() const
{
   ...
   auto cfgTrunc = this->mspCfgFile->spFramework()->spNode(
      Io::Config::Framework::TRUNCATION);
...
int SimulationConfig::nCpu() const
{
   ...
   return this->mspCfgFile->spFramework()
      ->spNode(Io::Config::Framework::PARALLEL)
      ->iTags()
      .value("cpus");
}
...
Splitting::Algorithms::Id SimulationConfig::algorithm() const
{
   ...
   auto tag = this->mspCfgFile->spFramework()
                 ->spNode(Io::Config::Framework::PARALLEL)
                 ->sTags()
                 .value("algorithm");
...
std::list<int> SimulationConfig::cpuFactors() const
{
   ...
   auto tag = this->mspCfgFile->spFramework()
                 ...
                 ->sTags()
                 .value("decomposition");
   std::transform(tag.cbegin(), tag.cend(), tag.begin(),
      [](unsigned char c) { return std::tolower(c); });
      ...
      do
      {
         ...
         catch (const std::invalid_argument& e)
         {
            throw std::logic_error(
               "Decomposition format is wrong. It should be 'auto' or a comma "
               "separated list of factors");
         }
...
const std::map<std::string, MHDFloat>& SimulationConfig::physical() const
{
   ...
   return this->mspCfgFile->spSimulation()
      ->spNode(Io::Config::Simulation::PHYSICAL)
      ->fTags()
      .map();
}
...
std::map<std::string, MHDFloat>& SimulationConfig::rPhysical()
{
   ...
   return this->mspCfgFile->rspSimulation()
      ->rspNode(Io::Config::Simulation::PHYSICAL)
      ->fTags()
      .rMap();
}
...
std::map<std::string, std::size_t> SimulationConfig::boundary() const
{
   ...
   auto s = this->mspCfgFile->spSimulation()
               ->spNode(Io::Config::Simulation::BOUNDARY)
               ->sTags()
               .map();
...
Array SimulationConfig::run() const
{
   ...
   auto spRun =
      this->mspCfgFile->spFramework()->spNode(Io::Config::Framework::RUN);
...
Array SimulationConfig::timestepping() const
{
   ...
   auto spTime = this->mspCfgFile->spFramework()->spNode(
      Io::Config::Framework::TIMESTEPPING);
...
std::size_t SimulationConfig::timestepper() const
{
   ...
   auto s = this->mspCfgFile->spFramework()
               ->spNode(Io::Config::Framework::TIMESTEPPING)
               ->sTags()
               .value("scheme");
   std::string tag = s;
   std::transform(s.cbegin(), s.cend(), tag.begin(),
   ...
   // equations
   if (this->splitEquation() && !((id == Timestep::Id::ImexPc2::id()) || (id == Timestep::Id::ImexPc2b::id()) || (id == Timestep::Id::ImexEuler::id()) || (id == Timestep::Id::ImexEulerb::id())))
   {
      throw std::logic_error(
...
std::size_t SimulationConfig::bcScheme() const
{
   ...
   auto s = this->mspCfgFile->spSetup()
               ->spNode(Io::Config::Setup::BOUNDARY)
               ->sTags()
               .value("scheme");
   std::string tag = s;
   std::transform(s.cbegin(), s.cend(), tag.begin(),
...
bool SimulationConfig::splitEquation() const
{
   ...
   auto s = this->mspCfgFile->spSetup()
               ->spNode(Io::Config::Setup::MODEL)
               ->sTags()
               .value("split_equation");
   std::string tag = s;
   std::transform(s.cbegin(), s.cend(), tag.begin(),
      [](unsigned char c) { return std::tolower(c); });
   ...
   {
      throw std::logic_error(
         "on/off tag for split_equation was not recognized");
   }
