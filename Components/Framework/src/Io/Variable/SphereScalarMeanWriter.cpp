 ...
 * @brief Source of the implementation of the scalar field mean value writer in
 * a sphere
 */
...
SphereScalarMeanWriter::SphereScalarMeanWriter(const std::string& prefix,
   const std::string& type) :
    IVariableAsciiWriter(prefix + Tags::Mean::BASENAME, Tags::Mean::EXTENSION,
       prefix + Tags::Mean::HEADER, type, Tags::Mean::VERSION,
       Dimensions::Space::SPECTRAL, EXTEND),
    mHasMOrdering(false),
    mMean(std::numeric_limits<MHDFloat>::quiet_NaN()),
...
void SphereScalarMeanWriter::init()
{
   ...
   if (m0 == 0 && l0 == 0)
   {
      ...
      wnl.compute<Internal::MHDFloat>(ipoly, nN, 0, igrid,
         (iweights.array() * igrid.array().pow(2)).matrix(), ev::Set());
...
void SphereScalarMeanWriter::writeContent()
{
   ...
   if (this->mProjector.size() > 0)
   {
      this->mMean = std::visit(
         [&](auto&& p)
         {
            MHDComplex val = (this->mProjector.transpose() *
                              (p->dom(0).total().profile(0, 0) + this->mBg))
                                .array()
                                .sum();
            return val.real();
         },
...
// Get the "global" mean value from MPI code
#ifdef QUICC_MPI
   MPI_Allreduce(MPI_IN_PLACE, &this->mMean, 1, MPI_DOUBLE, MPI_SUM,
      MPI_COMM_WORLD);
#endif // QUICC_MPI
   ...
   if (QuICCEnv().allowsIO())
   {
      ...
      this->mFile << std::setprecision(ioPrec) << ioFW(ioPrec) << this->mTime
                  << "\t" << ioFW(ioPrec) << this->mMean << std::endl;
   }
