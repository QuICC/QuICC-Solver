...
VisualizationFileWriter::VisualizationFileWriter(std::string type) :
    IVariableHdf5NWriter(Tags::VisualizationFile::BASENAME,
       Tags::VisualizationFile::EXTENSION, Tags::VisualizationFile::HEADER,
       type, Tags::VisualizationFile::VERSION, Dimensions::Space::PHYSICAL,
       true)
{}
...
void VisualizationFileWriter::write()
{
   ...
   {
      if (std::visit([&](auto&& p) -> bool { return (p->dom(0).hasPhys()); },
             sit->second))
      {
         std::visit(
            [&](auto&& p)
            {
               this->writePhysicalScalar(
                  PhysicalNames::Coordinator::tag(sit->first),
                  p->dom(0).phys());
            },
            sit->second);
      }

      if (std::visit([&](auto&& p) -> bool { return (p->dom(0).hasGrad()); },
             sit->second))
      {
         std::visit(
            [&](auto&& p)
            {
               this->writePhysicalVector(
      ...
      if (std::visit([&](auto&& p) -> bool { return (p->dom(0).hasGrad2()); },
             sit->second))
      {
         std::visit(
            [&](auto&& p)
            {
               this->writePhysicalTensor(
   ...
   for (vit = vRange.first; vit != vRange.second; ++vit)
   {
         ...
         for (unsigned int i = 0; i < fId.size(); i++)
         {
            if (fId.at(i) != FieldComponents::Spectral::NOTUSED)
            {
               std::visit(
                  ...
                  {
                     this->writePhysicalVector(
                        PhysicalNames::Coordinator::tag(vit->first) + "_" +
                           Tools::IdToHuman::toTag(fId.at(i)) + "_grad",
                        p->dom(0).grad(fId.at(i)).data());
                  },
                  vit->second);
            }
         }
      ...
      if (std::visit([&](auto&& p) -> bool { return (p->dom(0).hasCurl()); },
             vit->second))
      {
         std::visit(
            [&](auto&& p)
...
void VisualizationFileWriter::writeMesh()
{
   ...
   hid_t group = H5Gcreate(this->file(), Tags::VisualizationFile::MESH.c_str(),
   ...
   for (int i = 0; i < this->res().cpu()->nDim(); i++)
   {
      ...
      {
         this->writeArray(group,
            Tags::VisualizationFile::GRID + "_" +
               Tools::IdToHuman::toTag(gridId.at(i)),
            this->mMesh.at(i));
      }
   }
...
void VisualizationFileWriter::writePhysicalScalar(const std::string& name,
   const Framework::Selector::PhysicalScalarField& scalar)
{
   ...
   std::vector<std::tuple<int, int,
      const Framework::Selector::PhysicalScalarField::PointType*>>
      fieldInfo = Datatypes::FieldTools::createInfo(scalar);
...
void VisualizationFileWriter::writePhysicalVector(const std::string& name,
   const std::map<FieldComponents::Physical::Id,
      Framework::Selector::PhysicalScalarField>& vector,
   const std::string& joint)
{
   ...
   std::map<FieldComponents::Physical::Id,
      Framework::Selector::PhysicalScalarField>::const_iterator it;
   for (it = vector.begin(); it != vector.end(); ++it)
   {
      ...
      this->writeRegularField(group,
         name + joint + Tools::IdToHuman::toTag(it->first), fieldInfo);
   }
...
void VisualizationFileWriter::writePhysicalTensor(const std::string& name,
   const std::map<
      std::pair<FieldComponents::Physical::Id, FieldComponents::Physical::Id>,
      Framework::Selector::PhysicalScalarField>& tensor,
   const std::string& joint)
{
   // Create the Field group
   hid_t group = H5Gcreate(this->file(), name.c_str(), H5P_DEFAULT, H5P_DEFAULT,
   ...
   for (it = tensor.begin(); it != tensor.end(); ++it)
   {
      ...
      this->writeRegularField(group,
         name + joint + Tools::IdToHuman::toTag(it->first.first) +
            Tools::IdToHuman::toTag(it->first.second),
         fieldInfo);
   }
