 ...
 * @brief Source of the implementation of the ASCII Chebyshev for
...
namespace Variable {
ICartesian1DTorPolEnergyBaseWriter::ICartesian1DTorPolEnergyBaseWriter(
...
void ICartesian1DTorPolEnergyBaseWriter::prepareInput(
...
void ICartesian1DTorPolEnergyBaseWriter::compute(
   Transform::TransformCoordinatorType& coord)
{
   constexpr auto TId = Dimensions::Transform::TRA1D;
   Matrix spectrum;
   ...
   const int nK = this->res().sim().dim(Dimensions::Simulation::SIM2D,
   ...
   for (int k = 0; k < tRes.dim<Dimensions::Data::DAT3D>(); ++k)
   {
      ...
      for (int j = 0; j < tRes.dim<Dimensions::Data::DAT2D>(k); ++j)
      {
         ...
         if (j_ == 0)
         {
            ...
            else if (k_ <= this->res().sim().dim(Dimensions::Simulation::SIM2D,
                              Dimensions::Space::SPECTRAL) /
                              2)
            {
               factor = 2.0;
   ...
   std::visit(
      ...
      {
         coord.transform1D().reduce(spectrum, p->data(),
            Transform::Reductor::EnergyD1::id());
      },
      pInVarPolS);
