 ...
 * energy spectrum calculation for toroidal/poloidal field in a spherical
 * geometry
 */
...
ISphericalTorPolModeSpectrumWriter::ISphericalTorPolModeSpectrumWriter(
   const std::string& prefix, const std::string& type) :
    ISphericalTorPolEnergyBaseWriter(prefix + Tags::Spectrum::MODEBASENAME,
...
void ISphericalTorPolModeSpectrumWriter::storeQEnergy(const int l, const int m,
...
void ISphericalTorPolModeSpectrumWriter::storeSEnergy(const int l, const int m,
   const MHDFloat energy)
{
   if (m <= l)
   {
...
void ISphericalTorPolModeSpectrumWriter::storeTEnergy(const int l, const int m,
   const MHDFloat energy)
{
   if (m <= l)
   {
...
#ifdef QUICC_MPI
   MPI_Allreduce(MPI_IN_PLACE, mTorEnergy.data(), mTorEnergy.size(), MPI_DOUBLE,
      MPI_SUM, MPI_COMM_WORLD);
   MPI_Allreduce(MPI_IN_PLACE, mPolEnergy.data(), mPolEnergy.size(), MPI_DOUBLE,
      MPI_SUM, MPI_COMM_WORLD);
#endif // QUICC_MPI
