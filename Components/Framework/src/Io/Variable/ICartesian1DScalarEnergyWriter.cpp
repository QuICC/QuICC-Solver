 ...
 * for scalar field in a plane layer
 */
...
ICartesian1DScalarEnergyWriter::ICartesian1DScalarEnergyWriter(
   const std::string& prefix, const std::string& type) :
    ICartesian1DScalarEnergyBaseWriter(prefix + Tags::Energy::BASENAME,
       Tags::Energy::EXTENSION, prefix + Tags::Energy::HEADER, type,
       Tags::Energy::VERSION, Dimensions::Space::SPECTRAL,
       ICartesian1DScalarEnergyBaseWriter::EXTEND),
    mEnergy(2)
{
...
void ICartesian1DScalarEnergyWriter::storeEnergy(const int kx, const int ky,
...
// Get the "global" Kinetic energy from MPI code
   ...
   if (QuICCEnv().allowsIO())
   {
      ...
      this->mFile << std::setprecision(ioPrec) << ioFW(ioPrec) << this->mTime
                  << "\t" << ioFW(ioPrec) << this->mEnergy.sum();
      if (this->mShowParity)
      {
         this->mFile << std::setprecision(ioPrec) << "\t" << ioFW(ioPrec)
                     << this->mEnergy(0) << "\t" << ioFW(ioPrec)
                     << this->mEnergy(1);
      }
