...
void ISphericalTorPolMSpectrumWriter::init()
{
   ...
   this->mTorEnergy = Array::Zero(this->res().sim().dim(
      Dimensions::Simulation::SIM3D, Dimensions::Space::SPECTRAL));
   this->mPolEnergy = Array::Zero(this->res().sim().dim(
      Dimensions::Simulation::SIM3D, Dimensions::Space::SPECTRAL));

   ISphericalTorPolEnergyBaseWriter::init();
}

void ISphericalTorPolMSpectrumWriter::resetEnergy()
{
   this->mTorEnergy.setZero();
   this->mPolEnergy.setZero();
}

void ISphericalTorPolMSpectrumWriter::storeQEnergy(const int l, const int m,
   const MHDFloat energy)
{
   this->mPolEnergy(m) += energy;
}

void ISphericalTorPolMSpectrumWriter::storeSEnergy(const int l, const int m,
   const MHDFloat energy)
{
   this->mPolEnergy(m) += energy;
}

void ISphericalTorPolMSpectrumWriter::storeTEnergy(const int l, const int m,
   const MHDFloat energy)
{
   this->mTorEnergy(m) += energy;
}

void ISphericalTorPolMSpectrumWriter::writeContent()
{
...
// Get the "global" Kinetic energy from MPI code
   ...
   energy.segment(this->mTorEnergy.size(), this->mPolEnergy.size()) =
      this->mPolEnergy;

   MPI_Allreduce(MPI_IN_PLACE, energy.data(), energy.size(), MPI_DOUBLE,
   ...
   if (QuICCEnv().allowsIO())
   {
      ...
      this->mFile << "# characteristic M: " << std::setprecision(ioPrec);
      Array w = Array::LinSpaced(this->mTorEnergy.size(), 0,
         this->mTorEnergy.size() - 1);
      MHDFloat eT = (w.array() * this->mTorEnergy.array()).sum() / tT;
      MHDFloat eP = (w.array() * this->mPolEnergy.array()).sum() / tP;
      MHDFloat e =
         (w.array() * (this->mTorEnergy.array() + this->mPolEnergy.array()))
            .sum() /
         t;
      MHDFloat vT =
         ((w.array() - eT).array().pow(2) * this->mTorEnergy.array()).sum() /
         tT;
      MHDFloat vP =
         ((w.array() - eP).array().pow(2) * this->mPolEnergy.array()).sum() /
         tP;
      MHDFloat v = ((w.array() - e).array().pow(2) *
                      (this->mTorEnergy.array() + this->mPolEnergy.array()))
                      .sum() /
                   t;
      ...
      for (int i = 0; i < this->mTorEnergy.size(); i++)
      {
         ...
         this->mFile << ioFW(ioPrec)
                     << this->mTorEnergy(i) + this->mPolEnergy(i) << "\t";
   ...
   {
      QuICCEnv().abort("Toroidal/Poloidal L energy spectrum is NaN!");
   }
}
