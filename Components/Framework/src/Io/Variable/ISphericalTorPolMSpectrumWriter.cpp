 ...
 * @brief Source of the implementation of the ASCII spherical harmonics M energy spectrum calculation for toroidal/poloidal field in a spherical geometry
 */
...
namespace Variable {
   ...
   ISphericalTorPolMSpectrumWriter::ISphericalTorPolMSpectrumWriter(const std::string& prefix, const std::string& type, std::vector<std::shared_ptr<QuICC::DenseSM::Chebyshev::LinearMap::RadialTorPolFunction>> pF)
      : ISphericalTorPolEnergyBaseWriter(prefix + Tags::Spectrum::MBASENAME, Tags::Spectrum::EXTENSION, prefix + Tags::Spectrum::HEADER, type, Tags::Spectrum::VERSION, Dimensions::Space::SPECTRAL, OVERWRITE, pF), mTorEnergy(0), mPolEnergy(0)
   {
   }
