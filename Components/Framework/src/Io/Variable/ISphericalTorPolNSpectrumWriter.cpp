 ...
 * @brief Source of the implementation of the ASCII spherical harmonics N power
...
ISphericalTorPolNSpectrumWriter::ISphericalTorPolNSpectrumWriter(
   const std::string& prefix, const std::string& type) :
    ISphericalTorPolPowerBaseWriter(prefix + Tags::Spectrum::NBASENAME,
...
void ISphericalTorPolNSpectrumWriter::storeQPower(const int n, const int l,
   const int m, const MHDFloat power)
{
   this->mPolPower(n, l) += power;
}

void ISphericalTorPolNSpectrumWriter::storeSPower(const int n, const int l,
   const int m, const MHDFloat power)
{
   this->mPolPower(n, l) += power;
}

void ISphericalTorPolNSpectrumWriter::storeTPower(const int n, const int l,
   const int m, const MHDFloat power)
{
...
// Get the "global" Kinetic power from MPI code
   ...
   if (QuICCEnv().allowsIO())
   {
      ...
      Array w = Array::LinSpaced(this->mTorPower.rows(), 0,
         this->mTorPower.rows() - 1);
