...
VisualizationFileReader::VisualizationFileReader(std::string name,
   std::string type) :
    IVariableHdf5Reader(Tags::VisualizationFile::BASENAME + name,
   ...
   for (sit = sRange.first; sit != sRange.second; ++sit)
   {
             ...
             sit->second))
      {
         std::visit(
            [&](auto&& p)
            {
               this->readPhysicalScalar(
                  PhysicalNames::Coordinator::tag(sit->first),
                  p->rDom(0).rPhys(), this->isRequired(sit->first));
            },
            sit->second);
      }
...
void VisualizationFileReader::readPhysicalScalar(const std::string& name,
   Framework::Selector::PhysicalScalarField& rScalar, const bool isRequired)
{
   ...
   if (group >= 0)
   {
      ...
      std::vector<std::tuple<int, int,
         Framework::Selector::PhysicalScalarField::PointType*>>
         fieldInfo = Datatypes::FieldTools::createInfo(rScalar);
...
void VisualizationFileReader::readPhysicalVector(const std::string& name,
   std::map<FieldComponents::Physical::Id,
      Framework::Selector::PhysicalScalarField>& rVector,
...
void VisualizationFileReader::readPhysicalComponent(const std::string& name,
   FieldComponents::Physical::Id id,
   Framework::Selector::PhysicalScalarField& rComp, const bool isRequired)
{
   // Open the vector field group
   hid_t group = H5Gopen(this->file(), name.c_str(), H5P_DEFAULT);
   ...
   if (group >= 0)
   {
      ...
      std::vector<std::tuple<int, int,
         Framework::Selector::PhysicalScalarField::PointType*>>
         fieldInfo = Datatypes::FieldTools::createInfo(rComp);

      // Read the field component
      this->readRegularField(group, name + "_" + Tools::IdToHuman::toTag(id),
         fieldInfo);
...
void VisualizationFileReader::readPhysicalTensor(const std::string&,
   std::map<
      std::pair<FieldComponents::Physical::Id, FieldComponents::Physical::Id>,
      Framework::Selector::PhysicalScalarField>&,
   const bool)
{}

void VisualizationFileReader::adaptData(
   Framework::Selector::PhysicalScalarField&)
{}
