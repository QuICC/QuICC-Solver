...
StateFileWriter::StateFileWriter(std::string type, const bool isRegular) :
    IVariableHdf5NWriter(Tags::StateFile::BASENAME, Tags::StateFile::EXTENSION,
       Tags::StateFile::HEADER, type, Tags::StateFile::VERSION,
       Dimensions::Space::SPECTRAL, isRegular)
{}

StateFileWriter::StateFileWriter(std::string name, std::string type,
   const bool isRegular) :
    IVariableHdf5NWriter(Tags::StateFile::BASENAME + name,
       Tags::StateFile::EXTENSION, Tags::StateFile::HEADER, type,
       Tags::StateFile::VERSION, Dimensions::Space::SPECTRAL, isRegular)
{}
...
void StateFileWriter::write()
{
   ...
   for (sit = sRange.first; sit != sRange.second; ++sit)
   {
      std::visit(
         [&](auto&& p)
         {
            this->writeSpectralScalar(
               PhysicalNames::Coordinator::tag(sit->first),
               p->dom(0).perturbation());
         },
         sit->second);
   }
   ...
   for (vit = vRange.first; vit != vRange.second; ++vit)
   {
      std::visit(
         [&](auto&& p)
         {
            this->writeSpectralVector(
               PhysicalNames::Coordinator::tag(vit->first),
               p->dom(0).perturbation().data());
         },
         vit->second);
   }
