...
StateFileReader::StateFileReader(std::string name, std::string type,
   const bool isRegular) :
    IVariableHdf5Reader(Tags::StateFile::BASENAME + name,
...
void StateFileReader::read()
{
   ...
   for (sit = sRange.first; sit != sRange.second; ++sit)
   {
      ...
      std::visit(
         [&](auto&& p)
         {
            this->readSpectralScalar(
               PhysicalNames::Coordinator::tag(sit->first),
               p->rDom(0).rPerturbation(), this->isRequired(sit->first));
         },
         sit->second);
   }
...
void StateFileReader::readRun()
{
   ...
   hid_t group =
      H5Gopen(this->file(), Tags::VariableHdf5::RUN.c_str(), H5P_DEFAULT);
