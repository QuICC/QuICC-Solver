 ...
 * enstrophy M spectrum calculation for toroidal/poloidal field in a spherical
 * geometry
 */
...
ISphericalTorPolEnstrophyMSpectrumWriter::
   ISphericalTorPolEnstrophyMSpectrumWriter(const std::string& prefix,
      const std::string& type) :
    ISphericalTorPolEnstrophyBaseWriter(prefix +
                                           Tags::EnstrophySpectrum::BASENAME +
                                           Tags::EnstrophySpectrum::MBASENAME,
...
void ISphericalTorPolEnstrophyMSpectrumWriter::init()
{
   ...
   this->mTorEnstrophy = Array::Zero(this->res().sim().dim(
      Dimensions::Simulation::SIM3D, Dimensions::Space::SPECTRAL));
...
void ISphericalTorPolEnstrophyMSpectrumWriter::storeQEnstrophy(const int l,
   const int m, const MHDFloat enstrophy)
{
   this->mTorEnstrophy(m) += enstrophy;
}

void ISphericalTorPolEnstrophyMSpectrumWriter::storeSEnstrophy(const int l,
   const int m, const MHDFloat enstrophy)
{
   this->mTorEnstrophy(m) += enstrophy;
}

void ISphericalTorPolEnstrophyMSpectrumWriter::storeTEnstrophy(const int l,
   const int m, const MHDFloat enstrophy)
{
