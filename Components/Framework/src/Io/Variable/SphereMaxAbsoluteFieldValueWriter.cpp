 ...
 * @brief Source of the implementation of the ASCII absolute field value in a
 * sphere
 */
...
SphereMaxAbsoluteFieldValueWriter::SphereMaxAbsoluteFieldValueWriter(
   const std::string& prefix, const std::string& type) :
    IVariableAsciiWriter(prefix + Tags::MaxAbsoluteFieldValue::BASENAME,
       Tags::MaxAbsoluteFieldValue::EXTENSION,
       prefix + Tags::MaxAbsoluteFieldValue::HEADER, type,
       Tags::MaxAbsoluteFieldValue::VERSION, Dimensions::Space::SPECTRAL,
       EXTEND),
    mHasMOrdering(false),
    mMaxAbsoluteFieldValue(std::numeric_limits<MHDFloat>::quiet_NaN())
{}
...
void SphereMaxAbsoluteFieldValueWriter::compute(
   Transform::TransformCoordinatorType& coord)
{
   ...
   this->mMaxAbsoluteFieldValue = std::visit(
      [&](auto&& p)
      {
         auto val = (p->dom(0)
                        .phys()
                        .comp(FieldComponents::Physical::R)
                        .data()
                        .array()
                        .pow(2) +
                     p->dom(0)
                        .phys()
                        .comp(FieldComponents::Physical::THETA)
                        .data()
                        .array()
                        .pow(2) +
                     p->dom(0)
                        .phys()
                        .comp(FieldComponents::Physical::PHI)
                        .data()
                        .array()
                        .pow(2))
                       .maxCoeff();
...
// Get the "global" max of the absolute field value from MPI code
#ifdef QUICC_MPI
   MPI_Allreduce(MPI_IN_PLACE, &this->mMaxAbsoluteFieldValue, 1, MPI_DOUBLE,
   ...
   if (QuICCEnv().allowsIO())
   {
      ...
      this->mFile << std::setprecision(ioPrec) << ioFW(ioPrec) << this->mTime
                  << "\t" << ioFW(ioPrec) << this->mMaxAbsoluteFieldValue
                  << std::endl;
   }
