...
ICartesian1DScalarEnergyBaseWriter::ICartesian1DScalarEnergyBaseWriter(
...
void ICartesian1DScalarEnergyBaseWriter::prepareInput(
...
void ICartesian1DScalarEnergyBaseWriter::compute(
   Transform::TransformCoordinatorType& coord)
{
   ...
   for (int k = 0; k < tRes.dim<Dimensions::Data::DAT3D>(); ++k)
   {
      ...
      for (int j = 0; j < tRes.dim<Dimensions::Data::DAT2D>(k); j++)
      {
         ...
         if (ky == 0)
         {
            ...
            else if (kx < this->res().sim().dim(Dimensions::Simulation::SIM2D,
