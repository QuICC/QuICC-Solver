...
ISphericalScalarEnergyWriter::ISphericalScalarEnergyWriter(
   const std::string& prefix, const std::string& type) :
    ISphericalScalarEnergyBaseWriter(prefix + Tags::Energy::BASENAME,
       Tags::Energy::EXTENSION, prefix + Tags::Energy::HEADER, type,
       Tags::Energy::VERSION, Dimensions::Space::SPECTRAL,
       ISphericalScalarEnergyBaseWriter::EXTEND),
    mEnergy(2)
{
...
void ISphericalScalarEnergyWriter::storeEnergy(const int l, const int m,
