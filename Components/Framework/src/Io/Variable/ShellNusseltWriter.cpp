 ...
 * @brief Source of the implementation of the ASCII Nusselt number in a
 * spherical shell
 */
...
ShellNusseltWriter::ShellNusseltWriter(const std::string& prefix,
...
void ShellNusseltWriter::init()
{
   ...
   if (m0 == 0 && l0 == 0)
   {
      ...
      int flag =
         this->mPhysical.find(NonDimensional::Heating::id())->second->value();
      // Internal heating
      if (flag == 0)
      {
      ...
      else if (flag == 2 || flag == 3)
      {
         ...
         auto beta =
            this->mPhysical.find(NonDimensional::Beta::id())->second->value();
         auto chi = ri / ro;
         auto bg1 = ri / ri;
         if (flag == 2)
         {
      ...
      else
      {
         throw std::logic_error(
            "Unknown background profile for spherical shell Nusselt writer");
      }
      ...
      this->mNusselt = std::visit(
         [&](auto&& p) -> Array
         {
            return (this->mBackground +
                      this->mBoundary.transpose() *
                         p->dom(0).total().profile(0, 0).real())
                      .array() /
                   this->mBackground.array();
         },
         sRange.first->second);
   }
   else
...
// Get the "global" Kinetic energy from MPI code
#ifdef QUICC_MPI
   MPI_Allreduce(MPI_IN_PLACE, this->mNusselt.data(), this->mNusselt.size(),
   ...
   if (QuICCEnv().allowsIO())
   {
      ...
      this->mFile << std::setprecision(ioPrec) << ioFW(ioPrec) << this->mTime
                  << "\t" << ioFW(ioPrec) << this->mNusselt(0) << "\t"
                  << ioFW(ioPrec) << this->mNusselt(1) << std::endl;
   }
   ...
   {
      QuICCEnv().abort("Spherical shell Nusselt is NaN!");
   }
}
