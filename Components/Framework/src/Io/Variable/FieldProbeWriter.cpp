...
FieldProbeWriter::FieldProbeWriter(const std::string& prefix,
   const std::string& type, const std::vector<MHDFloat>& position) :
    IVariableAsciiWriter(prefix + Tags::FieldProbe::BASENAME,
       Tags::FieldProbe::EXTENSION, prefix + Tags::FieldProbe::HEADER, type,
       Tags::FieldProbe::VERSION, Dimensions::Space::PHYSICAL, EXTEND),
    mPosition(position)
{}
...
void FieldProbeWriter::init()
{
   if (this->mMesh.size() != this->mPosition.size())
   {
      throw std::logic_error(
         "Mesh and position do not have the same dimensionality");
   }
...
void FieldProbeWriter::writeContent()
{
   ...
   assert(std::distance(sRange.first, sRange.second) +
             std::distance(vRange.first, vRange.second) ==
          1);
      ...
      if (isScalar)
      {
         auto v = std::visit(
            [&](auto&& p)
            {
               return p->dom(0)
                  .phys()
                  .comp(FieldComponents::Physical::SCALAR)
                  .point(i, j, k);
            },
            sRange.first->second);
      ...
      if (isVector)
      {
         auto v = std::visit(
            [&](auto&& p)
            {
               return p->dom(0)
                  .phys()
                  .comp(this->res().sim().ss().physical().ONE())
                  .point(i, j, k);
            },
            vRange.first->second);
         this->mValue.push_back(v);
         v = std::visit(
            [&](auto&& p)
            {
               return p->dom(0)
                  .phys()
                  .comp(this->res().sim().ss().physical().TWO())
                  .point(i, j, k);
            },
            vRange.first->second);
         this->mValue.push_back(v);
         v = std::visit(
            [&](auto&& p)
            {
               return p->dom(0)
                  .phys()
                  .comp(this->res().sim().ss().physical().THREE())
                  .point(i, j, k);
            },
            vRange.first->second);
         this->mValue.push_back(v);
...
#ifdef QUICC_MPI
   MPI_Allreduce(MPI_IN_PLACE, this->mValue.data(), this->mValue.size(),
