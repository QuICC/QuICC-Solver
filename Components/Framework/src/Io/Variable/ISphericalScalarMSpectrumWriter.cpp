...
ISphericalScalarMSpectrumWriter::ISphericalScalarMSpectrumWriter(
   const std::string& prefix, const std::string& type) :
    ISphericalScalarEnergyBaseWriter(prefix + Tags::Spectrum::MBASENAME,
...
{
   this->mEnergy = Array::Zero(this->res().sim().dim(
...
void ISphericalScalarMSpectrumWriter::storeEnergy(const int l, const int m,
   const MHDFloat energy)
{
   this->mEnergy(m) += energy;
}

void ISphericalScalarMSpectrumWriter::writeContent()
{
...
// Get the "global" Kinetic energy from MPI code
   ...
   if (QuICCEnv().allowsIO())
   {
      ...
      Array w =
         Array::LinSpaced(this->mEnergy.size(), 0, this->mEnergy.size() - 1);
      MHDFloat e = (w.array() * this->mEnergy.array()).sum() / t;
      MHDFloat v =
         ((w.array() - e).array().pow(2) * this->mEnergy.array()).sum() / t;
