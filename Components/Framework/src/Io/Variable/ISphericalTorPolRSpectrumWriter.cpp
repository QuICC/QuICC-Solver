 ...
 * spectrum calculation for toroidal/poloidal field in a spherical geometry
 */
...
ISphericalTorPolRSpectrumWriter::ISphericalTorPolRSpectrumWriter(
   const std::string& prefix, const std::string& type) :
    IVariableAsciiWriter(prefix + Tags::Spectrum::RBASENAME,
...
void ISphericalTorPolRSpectrumWriter::init()
{
   ...
   this->mTorPower =
      Matrix::Zero(this->res().sim().dim(Dimensions::Simulation::SIM1D,
...
void ISphericalTorPolRSpectrumWriter::prepareInput(
...
void ISphericalTorPolRSpectrumWriter::compute(
   Transform::TransformCoordinatorType& coord)
{
   constexpr auto TId = Dimensions::Transform::TRA1D;
   Matrix spectrum;
   ...
   // Size of spectrum
   auto size = std::visit([](auto&& p) -> std::pair<int, int>
      { return std::make_pair(0, p->data().cols()); }, pInVarTor);
   ...
   std::visit(
      [&](auto&& p)
      {
         coord.transform1D().reduce(spectrum, p->data(),
            Transform::Reductor::RadialPower::id());
      },
      pInVarTor);
   ...
   if (this->mHasMOrdering)
   {
      ...
      for (int k = 0; k < tRes.dim<Dimensions::Data::DAT3D>(); ++k)
      {
         ...
         for (int j = 0; j < tRes.dim<Dimensions::Data::DAT2D>(k); j++)
         {
            ...
            {
               this->storeTPower(i, l_, m_,
                  factor * lfactor * spectrum(i, idx));
            }
            idx += 1;
         }
      }
   ...
   std::visit(
      ...
      {
         coord.transform1D().reduce(spectrum, p->data(),
            Transform::Reductor::RadialPowerOverr1::id());
      },
      pInVarPolQ);
   ...
   std::visit(
      ...
      {
         coord.transform1D().reduce(spectrum, p->data(),
            Transform::Reductor::RadialPowerOverr1D1R1::id());
      },
      pInVarPolS);
...
void ISphericalTorPolRSpectrumWriter::storeQPower(const int n, const int l,
   const int m, const MHDFloat power)
{
   this->mPolPower(n, l) += power;
}

void ISphericalTorPolRSpectrumWriter::storeSPower(const int n, const int l,
   const int m, const MHDFloat power)
{
   this->mPolPower(n, l) += power;
}

void ISphericalTorPolRSpectrumWriter::storeTPower(const int n, const int l,
   const int m, const MHDFloat power)
{
...
// Get the "global" Kinetic power from MPI code
   ...
   MPI_Allreduce(MPI_IN_PLACE, power.data(), power.size(), MPI_DOUBLE, MPI_SUM,
      MPI_COMM_WORLD);
   ...
   if (QuICCEnv().allowsIO())
   {
      ...
      for (int l = 1; l < this->mTorPower.cols(); l++)
      {
         ...
         MHDFloat e = (w.array() * (this->mTorPower.col(l).array() +
                                      this->mPolPower.col(l).array()))
                         .sum() /
                      p;
         MHDFloat vT =
            ((w.array() - eT).array().pow(2) * this->mTorPower.col(l).array())
               .sum() /
            pT;
         MHDFloat vP =
            ((w.array() - eP).array().pow(2) * this->mPolPower.col(l).array())
               .sum() /
            pP;
         MHDFloat v = ((w.array() - e).array().pow(2) *
                         (this->mTorPower.col(l).array() +
                            this->mPolPower.col(l).array()))
                         .sum() /
                      p;
         this->mFile << "# l = " << ioIW() << l << "\t" << std::setprecision(2)
                     << std::fixed;
         this->mFile << e << " (SD=" << std::sqrt(v)
                     << ",pc=" << std::setprecision(1) << std::right
                     << std::setw(4) << (p / t) * 100 << ")" << std::left
                     << "\t";
         this->mFile << eT << " (SD=" << std::sqrt(vT)
                     << ",pc=" << std::setprecision(1) << std::right
                     << std::setw(4) << (pT / tT) * 100 << ")" << std::left
                     << "\t";
         this->mFile << eP << " (SD=" << std::sqrt(vP)
                     << ",pc=" << std::setprecision(1) << std::right
                     << std::setw(4) << (pP / tP) * 100 << ")" << std::left;
         this->mFile << std::endl;
         this->mFile << std::scientific;
      }
      ...
      {
         this->mFile << std::left << ioIW() << std::setprecision(ioPrec)
                     << this->mGrid(i) << "\t";
         for (int j = 0; j < this->mTorPower.cols(); j++)
         {
            this->mFile << ioFW(ioPrec)
                        << this->mTorPower(i, j) + this->mPolPower(i, j)
                        << "\t";
         }
         this->mFile << std::endl;
      }
