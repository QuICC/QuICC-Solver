 ...
 * @brief Source of the implementation of the ASCII angular momentum number in a
 * sphere
 */
...
SphereAngularMomentumWriter::SphereAngularMomentumWriter(
   const std::string& prefix, const std::string& type) :
    IVariableAsciiWriter(prefix + Tags::AngularMomentum::BASENAME,
       Tags::AngularMomentum::EXTENSION, prefix + Tags::AngularMomentum::HEADER,
       type, Tags::AngularMomentum::VERSION, Dimensions::Space::SPECTRAL,
       EXTEND),
    mHasMOrdering(false),
    mHasM0(false),
    mHasM1(false),
...
void SphereAngularMomentumWriter::compute(
   Transform::TransformCoordinatorType& coord)
{
   ...
   if (this->mHasM1)
   {
      ...
      std::visit(
         ...
         {
            mom =
               (this->mOp.transpose() * f->dom(0)
                                           .total()
                                           .comp(FieldComponents::Spectral::TOR)
                                           .profile(this->mM1j, this->mM1k));
         },
         field);
...
// Get the "global" value from MPI code
#ifdef QUICC_MPI
   MPI_Allreduce(MPI_IN_PLACE, this->mMomentum.data(), this->mMomentum.size(),
   ...
   if (QuICCEnv().allowsIO())
   {
      ...
      this->mFile << std::setprecision(ioPrec) << ioFW(ioPrec) << this->mTime
                  << "\t" << ioFW(ioPrec) << this->mMomentum.norm();
