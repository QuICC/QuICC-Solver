    ...
    mShowParity(false)
{
   ...
   std::vector<std::size_t> ops = {Transform::Reductor::PowerR2::id(),
      Transform::Reductor::Power::id(), Transform::Reductor::PowerD1R1::id()};
...
void ISphericalTorPolPowerBaseWriter::prepareInput(
...
void ISphericalTorPolPowerBaseWriter::compute(
   Transform::TransformCoordinatorType& coord)
{
   constexpr auto TId = Dimensions::Transform::TRA1D;
   Matrix spectrum;
   ...
   if (this->mPath2Op.count(this->mPathId) == 0)
   {
      throw std::logic_error("Unknown power transform reductor path (" +
                             ...
                             ") requested for Toroidal/Poloidal");
   }
