 ...
 * @brief Source of the implementation of the ASCII Chebyshev energy calculation
 * for toroidal/poloidal field in a plane layer
 */
...
ICartesian1DTorPolEnergyWriter::ICartesian1DTorPolEnergyWriter(
   const std::string& prefix, const std::string& type) :
    ICartesian1DTorPolEnergyBaseWriter(prefix + Tags::Energy::BASENAME,
...
void ICartesian1DTorPolEnergyWriter::storeQEnergy(const int kx, const int ky,
...
void ICartesian1DTorPolEnergyWriter::storeSEnergy(const int kx, const int ky,
   const MHDFloat energy)
{
   if (kx == 0 && ky == 0)
   {
...
void ICartesian1DTorPolEnergyWriter::storeTEnergy(const int kx, const int ky,
   const MHDFloat energy)
{
   if (kx == 0 && ky == 0)
   {
...
// Get the "global" Kinetic energy from MPI code
   ...
   if (QuICCEnv().allowsIO())
   {
      ...
      this->mFile << std::setprecision(ioPrec) << ioFW(ioPrec) << this->mTime
                  << "\t" << ioFW(ioPrec)
                  << this->mTorEnergy.sum() + this->mPolEnergy.sum();
      if (this->mShowParity)
      {
         this->mFile << std::setprecision(ioPrec) << "\t" << ioFW(ioPrec)
                     << this->mTorEnergy(0) + this->mPolEnergy(0) << "\t"
                     << ioFW(ioPrec)
                     << this->mTorEnergy(1) + this->mPolEnergy(1);
      }
