/**
 * @file ISphericalScalarNCoeffsPowerBaseWriter.cpp
...
#include "QuICC/Io/Variable/ISphericalScalarNCoeffsPowerBaseWriter.hpp"
...
ISphericalScalarNCoeffsPowerBaseWriter::ISphericalScalarNCoeffsPowerBaseWriter(
    ...
    mShowParity(false)
{
   ...
   std::vector<std::size_t> ops = {Transform::Reductor::Spectrum::id()};
   this->addPath2Op(Transform::Path::Scalar::id(), ops);
}

void ISphericalScalarNCoeffsPowerBaseWriter::showParity()
{
   this->mShowParity = true;
}

void ISphericalScalarNCoeffsPowerBaseWriter::prepareInput(
...
void ISphericalScalarNCoeffsPowerBaseWriter::compute(
   Transform::TransformCoordinatorType& coord)
{
   constexpr auto TId = Dimensions::Transform::TRA1D;
   ...
   {
      throw std::logic_error("Unknown power transform reductor path (" +
   ...
   std::visit([&](auto&& p)
      { coord.transform1D().reduce(spectrum, p->data(), powerR2Id); }, pInVar);
   ...
   if (this->mHasMOrdering)
   {
      ...
      for (int k = 0; k < tRes.dim<Dimensions::Data::DAT3D>(); ++k)
      {
         ...
         for (int j = 0; j < tRes.dim<Dimensions::Data::DAT2D>(k); j++)
         {
            ...
            for (int i = 0;
               i < this->res().counter().dim(Dimensions::Simulation::SIM1D,
                      Dimensions::Space::SPECTRAL, l_);
               i++)
            {
