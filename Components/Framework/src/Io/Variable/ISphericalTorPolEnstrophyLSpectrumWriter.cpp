 ...
 * enstrophy L spectrum calculation for toroidal/poloidal field in a spherical
 * geometry
 */
...
ISphericalTorPolEnstrophyLSpectrumWriter::
   ISphericalTorPolEnstrophyLSpectrumWriter(const std::string& prefix,
      const std::string& type) :
    ISphericalTorPolEnstrophyBaseWriter(prefix +
                                           Tags::EnstrophySpectrum::BASENAME +
                                           Tags::EnstrophySpectrum::LBASENAME,
       Tags::EnstrophySpectrum::EXTENSION,
       prefix + Tags::EnstrophySpectrum::HEADER, type,
       Tags::EnstrophySpectrum::VERSION, Dimensions::Space::SPECTRAL),
    mTorEnstrophy(0),
    mPolEnstrophy(0)
{}
...
void ISphericalTorPolEnstrophyLSpectrumWriter::init()
{
   ...
   this->mTorEnstrophy = Array::Zero(this->res().sim().dim(
      Dimensions::Simulation::SIM2D, Dimensions::Space::SPECTRAL));
   this->mPolEnstrophy = Array::Zero(this->res().sim().dim(
      Dimensions::Simulation::SIM2D, Dimensions::Space::SPECTRAL));

   ISphericalTorPolEnstrophyBaseWriter::init();
}

void ISphericalTorPolEnstrophyLSpectrumWriter::resetEnstrophy()
{
   this->mTorEnstrophy.setZero();
   this->mPolEnstrophy.setZero();
}

void ISphericalTorPolEnstrophyLSpectrumWriter::storeQEnstrophy(const int l,
   const int m, const MHDFloat enstrophy)
{
   this->mTorEnstrophy(l) += enstrophy;
}

void ISphericalTorPolEnstrophyLSpectrumWriter::storeSEnstrophy(const int l,
   const int m, const MHDFloat enstrophy)
{
   this->mTorEnstrophy(l) += enstrophy;
}

void ISphericalTorPolEnstrophyLSpectrumWriter::storeTEnstrophy(const int l,
   const int m, const MHDFloat enstrophy)
{
...
// Get the "global" Kinetic enstrophy from MPI code
   ...
   enstrophy.segment(this->mTorEnstrophy.size(), this->mPolEnstrophy.size()) =
      this->mPolEnstrophy;

   MPI_Allreduce(MPI_IN_PLACE, enstrophy.data(), enstrophy.size(), MPI_DOUBLE,
   ...
   if (QuICCEnv().allowsIO())
   {
      ...
      this->mFile << ioFW(ioPrec)
                  << this->mTorEnstrophy.sum() + this->mPolEnstrophy.sum()
                  << "\t";
      ...
      {
         this->mFile << std::left << ioIW() << i << "\t"
                     << std::setprecision(ioPrec);
         this->mFile << ioFW(ioPrec)
                     << this->mTorEnstrophy(i) + this->mPolEnstrophy(i) << "\t";
   ...
   if (std::isnan(this->mTorEnstrophy.sum()) ||
       std::isnan(this->mPolEnstrophy.sum()))
   {
