...
IVariableHdf5Reader::IVariableHdf5Reader(std::string name, std::string ext,
...
bool IVariableHdf5Reader::isFull() const
{
   ...
   bool sizeStatus =
      (this->mScalars.size() + this->mVectors.size() == this->mExpected.size());
...
void IVariableHdf5Reader::addScalar(const std::pair<std::size_t,
...
void IVariableHdf5Reader::addVector(const std::pair<std::size_t,
...
#ifdef QUICC_MPI
   MPI_Allreduce(MPI_IN_PLACE, &this->mCollIoRead, 1, MPI_INT, MPI_MIN,
      MPI_COMM_WORLD);
#endif // QUICC_MPI
}
...
SharedSimulationResolution IVariableHdf5Reader::getFileTruncation() const
{
   ...
   hid_t base = H5Gopen(this->file(), Tags::VariableHdf5::TRUNCATION.c_str(),
      H5P_DEFAULT);

   // Open the spectral truncation subgroup
   hid_t subGroup =
      H5Gopen(base, Tags::VariableHdf5::TRUNCSPECTRAL.c_str(), H5P_DEFAULT);
   ...
   subGroup =
      H5Gopen(base, Tags::VariableHdf5::TRUNCTRANSFORM.c_str(), H5P_DEFAULT);
   ...
   subGroup =
      H5Gopen(base, Tags::VariableHdf5::TRUNCPHYSICAL.c_str(), H5P_DEFAULT);
...
void IVariableHdf5Reader::getParameters(
   std::map<std::string, MHDFloat>& rParams) const
{
   std::ostringstream oss;

   // Open the truncation parameters group
   hid_t base =
      H5Gopen(this->file(), Tags::VariableHdf5::PHYSICAL.c_str(), H5P_DEFAULT);
   ...
   for (hsize_t i = 0; i < nParams; i++)
   {
      ...
      size = H5Lget_name_by_idx(base, ".", H5_INDEX_NAME, H5_ITER_INC,
         static_cast<hsize_t>(i), NULL, 0, H5P_DEFAULT);
      dsname.resize(size);
      size = H5Lget_name_by_idx(base, ".", H5_INDEX_NAME, H5_ITER_INC,
         static_cast<hsize_t>(i), &dsname[0], size + 1, H5P_DEFAULT);
      ...
      else
      {
         throw std::logic_error(
            "Tried to read unimplemented parameter data type");
      }
      H5Tclose(type);
      H5Dclose(dataset);
...
void IVariableHdf5Reader::checkTruncation()
{
   ...
   {
      if (this->res().sim().dim(static_cast<Dimensions::Simulation::Id>(i),
             this->mSpaceId) !=
          this->mspFileRes->dim(static_cast<Dimensions::Simulation::Id>(i),
             this->mSpaceId))
      {
         ...
         if (this->res().sim().dim(static_cast<Dimensions::Simulation::Id>(i),
             ...
             this->mspFileRes->dim(static_cast<Dimensions::Simulation::Id>(i),
                this->mSpaceId))
         {
            Tools::Formatter::printCentered(std::cout,
               " ---> Zeros have been added!", ' ');
         }
         else
         {
            Tools::Formatter::printCentered(std::cout,
               "---> File data has been truncated!", ' ');
         }
         Tools::Formatter::printLine(std::cout, '-');
