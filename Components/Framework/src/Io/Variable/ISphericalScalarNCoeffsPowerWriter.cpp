/**
 * @file ISphericalScalarNCoeffsPowerWriter.cpp
 * @brief Source of the implementation of the ASCII spherical harmonics L power
 * spectrum calculation for scalar field in a spherical geometry
 */
...
ISphericalScalarNCoeffsPowerWriter::ISphericalScalarNCoeffsPowerWriter(
   const std::string& prefix, const std::string& type) :
    ISphericalScalarNCoeffsPowerBaseWriter(prefix + Tags::Spectrum::NBASENAME,
       Tags::Spectrum::EXTENSION, prefix + Tags::Spectrum::HEADER, type,
       Tags::Spectrum::VERSION, Dimensions::Space::SPECTRAL, OVERWRITE),
    mPower(0, 0)
{}

void ISphericalScalarNCoeffsPowerWriter::init()
{
   this->mPower =
   ...
   ISphericalScalarNCoeffsPowerBaseWriter::init();
}

void ISphericalScalarNCoeffsPowerWriter::resetPower()
{
   this->mPower.setZero();
}

void ISphericalScalarNCoeffsPowerWriter::storePower(const int n, const int l,
   const int m, const MHDFloat power)
{
   this->mPower(n, l) += power;
}

void ISphericalScalarNCoeffsPowerWriter::writeContent()
{
...
#ifdef QUICC_MPI
   MPI_Allreduce(MPI_IN_PLACE, this->mPower.data(), this->mPower.size(),
