/**
 * @file ISphericalTorPolNScalarSpectrumWriter.cpp
 * @brief Source of the implementation of the ASCII spherical harmonics N spectrum calculation for toroidal/poloidal scalar fields in a spherical geometry
 */
...
namespace Variable {
   ...
   ISphericalTorPolNScalarSpectrumWriter::ISphericalTorPolNScalarSpectrumWriter(const std::string& prefix, const std::string& type, std::vector<std::shared_ptr<QuICC::DenseSM::Chebyshev::LinearMap::RadialTorPolFunction>> pF)
      : ISphericalTorPolScalarSpectrumBaseWriter(prefix + Tags::Spectrum::NBASENAME, Tags::Spectrum::EXTENSION, prefix + Tags::Spectrum::HEADER, type, Tags::Spectrum::VERSION, Dimensions::Space::SPECTRAL, OVERWRITE), mTorPower(0,0), mPolPower(0,0)
   {
   }

   ISphericalTorPolNScalarSpectrumWriter::~ISphericalTorPolNScalarSpectrumWriter()
   {
   }

   void ISphericalTorPolNScalarSpectrumWriter::init()
   {
      ...
      ISphericalTorPolScalarSpectrumBaseWriter::init();
   }

   void ISphericalTorPolNScalarSpectrumWriter::resetPower()
   {
      this->mTorPower.setZero();
      this->mPolPower.setZero();
   }

   void ISphericalTorPolNScalarSpectrumWriter::storeSPower(const int n, const int l, const int m, const MHDFloat power)
   {
      this->mPolPower(n, l) += power;
   }

   void ISphericalTorPolNScalarSpectrumWriter::storeTPower(const int n, const int l, const int m, const MHDFloat power)
   {
      this->mTorPower(n, l) += power;
   }

   void ISphericalTorPolNScalarSpectrumWriter::writeContent()
   {
      // Normalize by the volume
      this->mTorPower /= this->mVolume;
      this->mPolPower /= this->mVolume;
