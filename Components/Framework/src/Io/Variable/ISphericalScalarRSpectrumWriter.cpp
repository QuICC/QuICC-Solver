 ...
 * @brief Source of the implementation of the ASCII spherical harmonics power on
 * radial grid calculation for scalar field in a spherical geometry
 */
...
void ISphericalScalarRSpectrumWriter::storePower(const int n, const int l,
   const int m, const MHDFloat power)
{
   this->mPower(n, l) += power;
}

void ISphericalScalarRSpectrumWriter::prepareInput(
...
void ISphericalScalarRSpectrumWriter::compute(
   Transform::TransformCoordinatorType& coord)
{
   constexpr auto TId = Dimensions::Transform::TRA1D;
   ...
   auto size = std::visit([](auto&& p) -> std::pair<int, int>
      { return std::make_pair(0, p->data().cols()); }, pInVar);
