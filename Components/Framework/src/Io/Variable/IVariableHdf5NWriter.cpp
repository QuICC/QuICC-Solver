 ...
 * @brief Source of the implementation of the generic variable to HDF5 file
 * writer
 */
...
IVariableHdf5NWriter::IVariableHdf5NWriter(std::string name, std::string ext,
   std::string header, std::string type, std::string version,
   const Dimensions::Space::Id id, const bool isRegular) :
    IHdf5NWriter(name, ext, header, type, version),
    mTime(-1.0),
    mTimestep(-1.0),
   ...
   const std::map<std::string, std::size_t>& boundary)
{
   ...
   for (auto it = parameters.cbegin(); it != parameters.cend(); ++it)
   {
      ...
      this->mPhysical.insert(std::make_pair(nd,
         NonDimensional::Coordinator::map().find(nd)->second->create(
            it->second)));
   }
...
void IVariableHdf5NWriter::updatePhysical(
   const std::map<std::string, MHDFloat>& parameters)
{
   // Convert parameters to NonDimensional numbers
   for (auto it = parameters.cbegin(); it != parameters.cend(); ++it)
...
void IVariableHdf5NWriter::setSimTime(const MHDFloat time,
   const MHDFloat timestep)
{
   this->mTime = time;
...
bool IVariableHdf5NWriter::isFull() const
{
   ...
   status = status && (this->mScalars.size() + this->mVectors.size() ==
                         this->mExpected.size());
   ...
   {
      status = status && (this->mMesh.size() ==
                            static_cast<size_t>(this->res().cpu()->nDim()));
   }

   return status;
}

void IVariableHdf5NWriter::addScalar(const std::pair<std::size_t,
   Framework::Selector::VariantSharedScalarVariable>& scalar)
{
   ...
   if (this->mExpected.count(scalar.first))
   {
      ...
      if (!this->mspRes)
      {
         std::visit([&](auto&& p) { this->setResolution(p->dom(0).spRes()); },
            scalar.second);
      }
   }
}

void IVariableHdf5NWriter::addVector(const std::pair<std::size_t,
...
void IVariableHdf5NWriter::setDatasetOffsets()
{
   ...
   if (this->mSpaceId == Dimensions::Space::SPECTRAL)
   {
      this->res().counter().computeOffsets(this->mBlock, this->mFileOffsets,
         this->mSpaceId);
   }
   else
   {
...
void IVariableHdf5NWriter::setCollIo()
{
   ...
   if (this->mSpaceId == Dimensions::Space::SPECTRAL ||
       this->mSpaceId == Dimensions::Space::TRANSFORM)
   {
      transId = Dimensions::Transform::TRA1D;
   }
   else
   {
      transId = static_cast<Dimensions::Transform::Id>(
         this->res().sim().ss().dimension() - 1);
   }

   // Get the maximum number of slowest directions over all CPUs
   for (int i = 0; i < this->res().nCpu(); ++i)
   {
      if (this->mCollIoWrite <
          this->res().cpu(i)->dim(transId)->dim<Dimensions::Data::DAT3D>())
      {
         this->mCollIoWrite =
            this->res().cpu(i)->dim(transId)->dim<Dimensions::Data::DAT3D>();
      }
...
void IVariableHdf5NWriter::writeTruncation()
{
   ...
   hid_t base = H5Gcreate(this->file(), Tags::VariableHdf5::TRUNCATION.c_str(),
      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

   // Create the spectral truncation subgroup
   hid_t subGroup = H5Gcreate(base, Tags::VariableHdf5::TRUNCSPECTRAL.c_str(),
      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
   ...
   for (int i = 0; i < this->res().cpu()->nDim(); i++)
   {
      ...
      this->writeScalar(subGroup, oss.str(),
         this->res().sim().dim(static_cast<Dimensions::Simulation::Id>(i),
   ...
   subGroup = H5Gcreate(base, Tags::VariableHdf5::TRUNCTRANSFORM.c_str(),
      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
   ...
   subGroup = H5Gcreate(base, Tags::VariableHdf5::TRUNCPHYSICAL.c_str(),
      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
void IVariableHdf5NWriter::writePhysical()
{
   ...
   hid_t group = H5Gcreate(this->file(), Tags::VariableHdf5::PHYSICAL.c_str(),
      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
   ...
   for (auto it = this->mBoundary.cbegin(); it != this->mBoundary.cend(); ++it)
   {
      ...
      this->writeString(group, it->first,
         Bc::Name::Coordinator::tag(it->second));
   }
...
void IVariableHdf5NWriter::writeRun()
{
   ...
   hid_t group = H5Gcreate(this->file(), Tags::VariableHdf5::RUN.c_str(),
      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
