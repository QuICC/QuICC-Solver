 ...
 * enstrophy calculation for toroidal/poloidal field in a spherical geometry
 */
...
ISphericalTorPolEnstrophyWriter::ISphericalTorPolEnstrophyWriter(
   const std::string& prefix, const std::string& type) :
    ISphericalTorPolEnstrophyBaseWriter(prefix + Tags::Enstrophy::BASENAME,
       Tags::Enstrophy::EXTENSION, prefix + Tags::Enstrophy::HEADER, type,
       Tags::Enstrophy::VERSION, Dimensions::Space::SPECTRAL),
    mTorEnstrophy(2),
    mPolEnstrophy(2)
{
...
void ISphericalTorPolEnstrophyWriter::resetEnstrophy()
{
   this->mTorEnstrophy.setZero();
   this->mPolEnstrophy.setZero();
}

void ISphericalTorPolEnstrophyWriter::storeQEnstrophy(const int l, const int m,
   const MHDFloat enstrophy)
{
   if ((l - m) % 2 == 0)
   {
...
void ISphericalTorPolEnstrophyWriter::storeSEnstrophy(const int l, const int m,
   const MHDFloat enstrophy)
{
   if ((l - m) % 2 == 0)
   {
...
void ISphericalTorPolEnstrophyWriter::storeTEnstrophy(const int l, const int m,
   const MHDFloat enstrophy)
{
   if ((l - m) % 2 == 1)
   {
...
// Get the "global" Kinetic enstrophy from MPI code
   ...
   if (QuICCEnv().allowsIO())
   {
      ...
      if (this->mShowParity)
      {
         this->mFile << std::setprecision(ioPrec) << "\t" << ioFW(ioPrec)
                     << this->mTorEnstrophy(0) + this->mPolEnstrophy(0) << "\t"
                     << ioFW(ioPrec)
                     << this->mTorEnstrophy(1) + this->mPolEnstrophy(1);
      }

      // Toroidal
      this->mFile << std::setprecision(ioPrec) << "\t" << ioFW(ioPrec)
                  << this->mTorEnstrophy.sum();
      if (this->mShowParity)
      {
         this->mFile << std::setprecision(ioPrec) << "\t" << ioFW(ioPrec)
                     << this->mTorEnstrophy(0) << "\t" << ioFW(ioPrec)
                     << this->mTorEnstrophy(1);
      }

      // Poloidal
      this->mFile << std::setprecision(ioPrec) << "\t" << ioFW(ioPrec)
                  << this->mPolEnstrophy.sum();
      if (this->mShowParity)
      {
         this->mFile << std::setprecision(ioPrec) << "\t" << ioFW(ioPrec)
                     << this->mPolEnstrophy(0) << "\t" << ioFW(ioPrec)
                     << this->mPolEnstrophy(1);
      }
   ...
   {
      QuICCEnv().abort("Toroidal/Poloidal enstrophy is NaN!");
   }
}
