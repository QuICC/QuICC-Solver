/**
 * @file ISphericalTorPolScalarSpectrumBaseWriter.cpp
 * @brief Source of the implementation of the ASCII spherical harmonics spectrum calculation for toroidal/poloidal scalar field in a spherical geometry
 */
...
// Class include
//
#include "QuICC/Io/Variable/ISphericalTorPolScalarSpectrumBaseWriter.hpp"
...
namespace Variable {
   ISphericalTorPolScalarSpectrumBaseWriter::ISphericalTorPolScalarSpectrumBaseWriter(std::string name, std::string ext, std::string header, std::string type, std::string version, const Dimensions::Space::Id id, const IAsciiWriter::WriteMode mode, std::vector<std::shared_ptr<QuICC::DenseSM::IGenericProfile>> pF)
   ...
   ISphericalTorPolScalarSpectrumBaseWriter::~ISphericalTorPolScalarSpectrumBaseWriter()
   {
   }

   void ISphericalTorPolScalarSpectrumBaseWriter::showParity()
   {
      this->mShowParity = true;
   }

   void ISphericalTorPolScalarSpectrumBaseWriter::prepareInput(const FieldComponents::Spectral::Id sId, Transform::TransformCoordinatorType& coord)
   {
   ...
   void ISphericalTorPolScalarSpectrumBaseWriter::compute(Transform::TransformCoordinatorType& coord)
   {
      constexpr auto TId = Dimensions::Transform::TRA1D;
      Matrix spectrum;
      ...
      if(this->mHasMOrdering)
      {
         ...
         for(int k = 0; k < tRes.dim<Dimensions::Data::DAT3D>(); ++k)
         {
            ...
            for(int j = 0; j < tRes.dim<Dimensions::Data::DAT2D>(k); j++)
            {
               ...
               {
                  this->storeTPower(i, l_, m_, factor*spectrum(i, idx));
               }
               idx += 1;
            }
         }
      ...
      if(this->mHasMOrdering)
      {
         ...
         for(int k = 0; k < tRes.dim<Dimensions::Data::DAT3D>(); ++k)
         {
            ...
            for(int j = 0; j < tRes.dim<Dimensions::Data::DAT2D>(k); j++)
            {
               ...
               for(int i = 0; i < this->res().counter().dim(Dimensions::Simulation::SIM1D, Dimensions::Space::SPECTRAL, l_); i++)
               {
                  this->storeSPower(i, l_, m_, factor*spectrum(i, idx));
               }
               idx += 1;
            }
         }
