...
void MpiConverterSendRecv::setupCommunication(const int packs,
   const TransformDirection::Id direction)
{
...
void MpiConverterSendRecv::prepareForwardReceive()
{
   ...
   if (this->mPacks > 0)
   {
      ...
      int flag;
      int ierr = MPI_Testall(this->nFCpu(), this->pRecvFRequests(this->mPacks),
      ...
      {
         QuICCEnv().abort(
            "Setting up MPI requests for forward received failed for ID = " +
            std::to_string(static_cast<int>(this->mTraId)));
      }
...
void MpiConverterSendRecv::initiateBackwardSend()
{
   ...
   if (this->mPacks > 0)
   {
      ...
      // Make sure calls are posted at the right moment
      int flag;
      int ierr = MPI_Testall(this->nBCpu(), this->pSendBRequests(this->mPacks),
         &flag, MPI_STATUSES_IGNORE);
      ...
      if (!flag)
      {
         QuICCEnv().abort("Starting MPI communicationr for forward send "
                          "requests failed for ID = " +
                          std::to_string(static_cast<int>(this->mTraId)));
      }
...
void MpiConverterSendRecv::prepareBackwardReceive()
{
   ...
   if (this->mPacks > 0)
   {
      ...
      // Make sure calls are posted at the right moment
      int flag;
      int ierr = MPI_Testall(this->nBCpu(), this->pRecvBRequests(this->mPacks),
         &flag, MPI_STATUSES_IGNORE);
      ...
      {
         QuICCEnv().abort(
            "Setting up MPI requests for backward received failed for ID = " +
            std::to_string(static_cast<int>(this->mTraId)));
      }
...
void MpiConverterSendRecv::initiateForwardSend()
{
   ...
   if (this->mPacks > 0)
   {
      ...
      // Make sure calls are posted at the right moment
      int flag;
      int ierr = MPI_Testall(this->nFCpu(), this->pSendFRequests(this->mPacks),
         &flag, MPI_STATUSES_IGNORE);
...
{
   if (this->mActiveDirection == TransformDirection::FORWARD &&
       this->mDirection == TransformDirection::BACKWARD)
   {
      this->syncFwdRecvBuffer();
   }
   else if (this->mActiveDirection == TransformDirection::BACKWARD &&
            this->mDirection == TransformDirection::FORWARD)
   {
      this->syncFwdSendBuffer();
   }
   else if (this->mActiveDirection == TransformDirection::FORWARD &&
            this->mDirection == this->mActiveDirection)
   {
      this->syncFwdRecvBuffer();
   }
   else if (this->mActiveDirection == TransformDirection::BACKWARD &&
...
void MpiConverterSendRecv::syncFwdRecvBuffer()
{
   ...
   {
      int flag;
      // (depending on MPI implementation the double test (test+wait) is
      // required for the expected result)
      int ierr =
         MPI_Testall(this->nFCpu(), this->pRecvFRequests(this->mActiveReceive),
            &flag, MPI_STATUSES_IGNORE);
      ...
      if (!flag)
      {
         ierr = MPI_Waitall(this->nFCpu(),
            this->pRecvFRequests(this->mActiveReceive), MPI_STATUSES_IGNORE);
...
void MpiConverterSendRecv::syncBwdRecvBuffer()
{
   ...
   if (this->mActiveReceive > 0)
   {
      ...
      // required for the expected result)
      int ierr =
         MPI_Testall(this->nBCpu(), this->pRecvBRequests(this->mActiveReceive),
            &flag, MPI_STATUSES_IGNORE);
      ...
      if (!flag)
      {
         ierr = MPI_Waitall(this->nBCpu(),
            this->pRecvBRequests(this->mActiveReceive), MPI_STATUSES_IGNORE);
...
void MpiConverterSendRecv::syncFwdSendBuffer()
{
   ...
   if (this->mActiveSend > 0)
   {
      ...
      // (depending on MPI implementation the double test (test+wait) is
      // required for the expected result)
      int ierr = MPI_Testall(this->nBCpu(),
         this->pSendFRequests(this->mActiveSend), &flag, MPI_STATUSES_IGNORE);
      ...
      {
         ierr = MPI_Waitall(this->nBCpu(),
            this->pSendFRequests(this->mActiveSend), MPI_STATUSES_IGNORE);
...
void MpiConverterSendRecv::syncBwdSendBuffer()
{
   ...
   if (this->mActiveSend > 0)
   {
      ...
      // (depending on MPI implementation the double test (test+wait) is
      // required for the expected result)
      int ierr = MPI_Testall(this->nFCpu(),
         this->pSendBRequests(this->mActiveSend), &flag, MPI_STATUSES_IGNORE);
      ...
      {
         ierr = MPI_Waitall(this->nFCpu(),
            this->pSendBRequests(this->mActiveSend), MPI_STATUSES_IGNORE);
...
int MpiConverterSendRecv::sendDest(const int id, const int ref,
...
int MpiConverterSendRecv::recvSrc(const int id, const int ref,
   const int size) const
{
   // Create recv ring
...
void MpiConverterSendRecv::setupRequests()
{
   ...
   for (int k = 0; k < this->mForwardPacks.size(); ++k)
   {
      ...
      this->mRecvFRequests.insert(
         std::make_pair(packs, std::vector<MPI_Request>()));
      ...
      for (int id = 0; id < this->nFCpu(); ++id)
      {
         ...
         grpMe = (*std::find(this->mFCpuGroup.begin(), this->mFCpuGroup.end(),
            QuICCEnv().id(this->mTraId)));
         ...
         assert(static_cast<size_t>(grpSrc) <
                this->mRecvFRequests.at(packs).size());

// initialise the Recv request
#if defined QUICC_MPIPACK_MANUAL
         ierr = MPI_Recv_init(this->mspFBuffers->at(grpSrc),
            packs * this->mFSizes.at(grpSrc),
            MpiTypes::type<FwdBufferType::DataType>(), src, tag,
            QuICCEnv().comm(this->mTraId),
            &(this->mRecvFRequests.at(packs).at(grpSrc)));
         QuICCEnv().check(ierr, 981);
#else
         ierr = MPI_Recv_init(this->mspFBuffers->at(grpSrc),
            packs * this->mFSizes.at(grpSrc), MPI_PACKED, src, tag,
            QuICCEnv().comm(this->mTraId),
            &(this->mRecvFRequests.at(packs).at(grpSrc)));
         QuICCEnv().check(ierr, 981);
      ...
      {
         // Get CPU group index of local node
         grpMe = (*std::find(this->mBCpuGroup.begin(), this->mBCpuGroup.end(),
            QuICCEnv().id(this->mTraId)));
         ...
         assert(static_cast<size_t>(grpDest) <
                this->mSendBRequests.at(packs).size());

// initialise the Send request
#if defined QUICC_MPIPACK_MANUAL
         ierr = MPI_Send_init(this->mspBBuffers->at(grpDest),
            packs * this->mBSizes.at(grpDest),
            MpiTypes::type<BwdBufferType::DataType>(), dest, tag,
            QuICCEnv().comm(this->mTraId),
            &(this->mSendBRequests.at(packs).at(grpDest)));
         QuICCEnv().check(ierr, 982);
#else
         ierr = MPI_Send_init(this->mspBBuffers->at(grpDest),
            packs * this->mBSizes.at(grpDest), MPI_PACKED, dest, tag,
            QuICCEnv().comm(this->mTraId),
            &(this->mSendBRequests.at(packs).at(grpDest)));
         QuICCEnv().check(ierr, 982);
#endif // defined QUICC_MPIPACK_MANUAL
...
// initialise the Recv request
#if defined QUICC_MPIPACK_MANUAL
         ierr = MPI_Recv_init(this->mspBBuffers->at(grpSrc),
            packs * this->mBSizes.at(grpSrc),
            MpiTypes::BwdBufferType::DataType > (), src, tag,
            QuICCEnv().comm(this->mTraId),
            &(this->mRecvBRequests.at(packs).at(grpSrc)));
#else
         ierr = MPI_Recv_init(this->mspBBuffers->at(grpSrc),
            packs * this->mBSizes.at(grpSrc), MPI_PACKED, src, tag,
            QuICCEnv().comm(this->mTraId),
            &(this->mRecvBRequests.at(packs).at(grpSrc)));
#endif // defined QUICC_MPIPACK_MANUAL
...
// initialise the Send request
#if defined QUICC_MPIPACK_MANUAL
         ierr = MPI_Send_init(this->mspFBuffers->at(grpDest),
            packs * this->mFSizes.at(grpDest),
            MpiTypes::type<FwdBufferType::DataType>(), dest, tag,
            QuICCEnv().comm(this->mTraId),
            &(this->mSendFRequests.at(packs).at(grpDest)));
#else
         ierr = MPI_Send_init(this->mspFBuffers->at(grpDest),
            packs * this->mFSizes.at(grpDest), MPI_PACKED, dest, tag,
            QuICCEnv().comm(this->mTraId),
            &(this->mSendFRequests.at(packs).at(grpDest)));
...
void MpiConverterSendRecv::cleanupRequests()
{
   ...
   for (auto it = this->mRecvBRequests.begin();
        it != this->mRecvBRequests.end(); it++)
   {
   ...
   for (auto it = this->mRecvFRequests.begin();
        it != this->mRecvFRequests.end(); it++)
   {
      for (unsigned int i = 0; i < (*it).second.size(); ++i)
      {
         MPI_Request_free(&((*it).second.at(i)));
   ...
   for (auto it = this->mSendBRequests.begin();
        it != this->mSendBRequests.end(); it++)
   {
      for (unsigned int i = 0; i < (*it).second.size(); ++i)
      {
         MPI_Request_free(&((*it).second.at(i)));
   ...
   for (auto it = this->mSendFRequests.begin();
        it != this->mSendFRequests.end(); it++)
   {
      for (unsigned int i = 0; i < (*it).second.size(); ++i)
      {
         MPI_Request_free(&((*it).second.at(i)));
...
void MpiConverterSendRecv::profileStorage() const
{
   ...
   MHDFloat memReq =
      (Debug::MemorySize<int>::BYTES + Debug::MemorySize<int>::BYTES);
