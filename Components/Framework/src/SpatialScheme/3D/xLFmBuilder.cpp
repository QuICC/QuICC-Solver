/**
 * @file xLFmBuilder.cpp
 * @brief Source of the generic radial + Spherical Harmonics (Associated Legendre(poly) + Fourrier) scheme implementation with spectral m ordering
 */
...
namespace SpatialScheme {
   ...
   void xLFmBuilder::addTransformSetups(SharedResolution spRes) const
   {
   ...
   Transform::SharedTransformSetup xLFmBuilder::spSetup2D(SharedResolution spRes) const
   {
   ...
   Transform::SharedTransformSetup xLFmBuilder::spSetup3D(SharedResolution spRes) const
   {
      // Check options consistency
   ...
   xLFmBuilder::xLFmBuilder(const ArrayI& dim, const GridPurpose::Id purpose, const std::map<std::size_t,std::vector<std::size_t>>& options)
      : IRegularSHmlBuilder(dim, purpose, options)
   {
   }

   bool xLFmBuilder::sameSpectralOrdering() const
   {
      return false;
   }
