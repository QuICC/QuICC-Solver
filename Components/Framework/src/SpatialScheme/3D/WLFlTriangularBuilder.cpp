/** 
 * @file WLFlTriangularBuilder.cpp
...
namespace SpatialScheme {
   ...
   void WLFlTriangularBuilder::addTransformSetups(SharedResolution spRes) const
   {
   ...
   Transform::Poly::SharedSetup WLFlTriangularBuilder::spSetup1D(SharedResolution spRes) const
   {
      ...
      {
         int l = spRes->cpu()->dim(Dimensions::Transform::TRA1D)->idx<Dimensions::Data::DAT3D>(i);
         int nN = spRes->counter().dimensions(Dimensions::Space::SPECTRAL, l)(0);
         spSetup->addIndex(l, spRes->cpu()->dim(Dimensions::Transform::TRA1D)->dim<Dimensions::Data::DAT2D>(i), nN);
      }
   ...
   Transform::Poly::ALegendre::SharedSetup WLFlTriangularBuilder::spSetup2D(SharedResolution spRes) const
   {
      // Get physical size of polynomial transform
   ...
   Transform::Fft::Fourier::Mixed::SharedSetup WLFlTriangularBuilder::spSetup3D(SharedResolution spRes) const
   {
   ...
   WLFlTriangularBuilder::WLFlTriangularBuilder(const ArrayI& dim, const GridPurpose::Id purpose)
      : ITriangularSHlBuilder(dim, purpose)
   {
   }

   WLFlTriangularBuilder::~WLFlTriangularBuilder()
   {
   }

   void WLFlTriangularBuilder::setDimensions()
   {
      // Check if resolution is compatible with triangular truncation
      if(!Tools::Triangular::isOptimal(this->mI+1,this->mL))
      {
      ...
      int nR = (2*(Tools::Triangular::truncation(this->mI+1,0)-1) + 1)/2;
      // Check against max L requirements
      nR = std::max(nR, (2*(Tools::Triangular::truncation(this->mI+1,this->mL)-1) + this->mL + 1)/2);
      // Dealias
      nR = Transform::Poly::Tools::dealias(nR);
   ...
   void WLFlTriangularBuilder::setCosts()
   {
   ...
   void WLFlTriangularBuilder::setScalings()
   {
   ...
   void WLFlTriangularBuilder::setMemoryScore()
   {
