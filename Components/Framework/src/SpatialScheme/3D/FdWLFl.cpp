/** 
 * @file FdWLFl.cpp
 * @brief Source of the ID for a Finite Differences sphere + Spherical Harmonics (Associated Legendre(poly) + Fourrier) scheme implementation with spectral l ordering
 */
...
namespace SpatialScheme {
   ...
   const std::string FdWLFl::sTag = "FdWLFl";

   const std::string FdWLFl::sFormatted = "FdWLFl";

   const std::size_t FdWLFl::sId = Hasher::makeId(FdWLFl::sTag);

   FdWLFl::FdWLFl(const VectorFormulation::Id formulation, const GridPurpose::Id purpose)
      : xLF(formulation, purpose, FdWLFl::sId, FdWLFl::sTag, FdWLFl::sFormatted)
   {
   ...
   void FdWLFl::setImplementation(const std::map<std::size_t,std::vector<std::size_t>>& type)
   {
   ...
   std::shared_ptr<IBuilder> FdWLFl::createBuilder(ArrayI& dim, const bool needInterpretation) const
   {
      auto spBuilder = makeBuilder<FdWLFlBuilder>(dim, this->purpose(), needInterpretation, this->mImplType, this->mspCustomMesher);

      return spBuilder;
   }

   std::shared_ptr<Transform::ITransform> FdWLFl::createTransform(const Dimensions::Transform::Id id, std::shared_ptr<Transform::TransformSetup> spSetup) const
   {
   ...
   std::shared_ptr<Parallel::IIndexConv> FdWLFl::createIndexConv(const Dimensions::Transform::Id id) const
   {
   ...
   std::shared_ptr<Equations::Tools::ICoupling> FdWLFl::createCouplingTools(const Equations::CouplingIndexType indexType) const
   {
