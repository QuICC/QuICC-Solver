/**
 * @file JLFmBuilder.cpp
 * @brief Source of the sphere Bessel(poly) + Spherical Harmonics (Associated Legendre(poly) + Fourrier) scheme implementation with spectral m ordering
 */
...
namespace SpatialScheme {
   ...
   void JLFmBuilder::tuneResolution(SharedResolution spRes, const Parallel::SplittingDescription& descr)
   {
   ...
   void JLFmBuilder::addTransformSetups(SharedResolution spRes) const
   {
   ...
   Transform::SharedTransformSetup JLFmBuilder::spSetup1D(SharedResolution spRes) const
   {
   ...
   Transform::SharedTransformSetup JLFmBuilder::spSetup2D(SharedResolution spRes) const
   {
   ...
   Transform::SharedTransformSetup JLFmBuilder::spSetup3D(SharedResolution spRes) const
   {
      // Check options consistency
   ...
   JLFmBuilder::JLFmBuilder(const ArrayI& dim, const GridPurpose::Id purpose, const std::map<std::size_t,std::vector<std::size_t>>& options)
      : IRegularSHmlBuilder(dim, purpose, options)
   {
   }

   bool JLFmBuilder::sameSpectralOrdering() const
   {
      return false;
   }

   void JLFmBuilder::setDimensions()
   {
      // Set default mesher
