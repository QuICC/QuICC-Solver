/** 
 * @file ITriangularSHlBuilder.cpp
 * @brief Source of the triangular basis + Spherical Harmonics scheme implementation with l spectral ordering
 */
...
// Class include
//
#include "QuICC/SpatialScheme/3D/ITriangularSHlBuilder.hpp"
...
namespace SpatialScheme {
   ...
   ITriangularSHlBuilder::ITriangularSHlBuilder(const ArrayI& dim, const GridPurpose::Id purpose)
      : IBuilder(dim.size(), purpose), mI(dim(0)), mL(dim(1)), mM(dim(2))
   {
      assert(dim.size() == 3);
   }

   ITriangularSHlBuilder::~ITriangularSHlBuilder()
   {
   }

   ArrayI ITriangularSHlBuilder::resolution() const
   {
   ...
   void ITriangularSHlBuilder::addIndexCounter(SharedResolution spRes)
   {
      auto spCounter = std::make_shared<TriangularSHlIndexCounter>(spRes->spSim(), spRes->spCpu());

      spRes->setIndexCounter(spCounter);
   }

   int ITriangularSHlBuilder::fillIndexes(const Dimensions::Transform::Id transId, std::vector<ArrayI>& fwd1D, std::vector<ArrayI>& bwd1D, std::vector<ArrayI>& idx2D, ArrayI& idx3D, const ArrayI& id, const ArrayI& bins, const ArrayI& n0, const ArrayI& nN, const Splitting::Locations::Id flag)
   {
      ...
      {
         Tools::Triangular::fillIndexes1D(fwd1D, bwd1D, idx3D, this->dim(transId, Dimensions::Data::DATF1D), this->dim(transId, Dimensions::Data::DATB1D));
   ...
   int ITriangularSHlBuilder::splittableTotal(const Dimensions::Transform::Id transId, Splitting::Locations::Id flag)
   {
   ...
   void ITriangularSHlBuilder::splitSerial(std::multimap<int,int>& modes, const Dimensions::Transform::Id transId)
   {
   ...
   void ITriangularSHlBuilder::splitSingle1D(std::multimap<int,int>& modes, const ArrayI& n0, const ArrayI& nN, const Dimensions::Transform::Id transId)
   {
      // Create index list for first transform
      if(transId == Dimensions::Transform::TRA1D)
      {
   ...
   void ITriangularSHlBuilder::splitSingle2D(std::multimap<int,int>& modes, const ArrayI& id, const ArrayI& bins, const ArrayI& n0, const ArrayI& nN, const Dimensions::Transform::Id transId)
   {
      // Create index list for first transform
      if(transId == Dimensions::Transform::TRA1D)
      {
   ...
   void ITriangularSHlBuilder::splitTubular(std::multimap<int,int>& modes, const ArrayI& id, const ArrayI& bins, const ArrayI& n0, const ArrayI& nN, const Dimensions::Transform::Id transId)
   {
      // Create index list for first transform
      if(transId == Dimensions::Transform::TRA1D)
      {
   ...
   void ITriangularSHlBuilder::buildMLMap(std::multimap<int,int>& harmonics, const int id, const int bins)
   {
