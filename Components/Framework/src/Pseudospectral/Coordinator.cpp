   ...
   const std::size_t eqId, const int it)
{
   DebuggerMacro_showValue("Adding scalar equation for " +
                              ...
                              " to pseudospectral coordinator iteration ",
      1, it);
   ...
   if (this->mScalarEquations.count(it) == 0)
   {
      this->mScalarEquations.insert(
         std::make_pair(it, std::vector<SharedIScalarEquation>()));
   }
   ...
   {
      this->mScalarEqMap.insert(
         std::pair(key, std::vector<SharedIScalarEquation>()));
   }
   ...
   const std::size_t eqId, const int it)
{
   ...
   if (this->mVectorEquations.count(it) == 0)
   {
      this->mVectorEquations.insert(
         std::make_pair(it, std::vector<SharedIVectorEquation>()));
   }
   ...
   {
      this->mVectorEqMap.insert(
         std::pair(key, std::vector<SharedIVectorEquation>()));
   }
...
Coordinator::VectorEquation_range Coordinator::vectorRange(
   const std::size_t eqId, const int it)
{
   auto key = std::make_pair(eqId, it);
...
Coordinator::SharedIScalarEquation Coordinator::scalarEq(const std::size_t eqId,
   const int it, const int i)
{
   auto key = std::make_pair(eqId, it);
   assert(this->mScalarEqMap.count(key) > 0);
   auto eqIt = this->mScalarEqMap.find(key);
...
Coordinator::SharedIVectorEquation Coordinator::vectorEq(const std::size_t eqId,
   const int it, const int i)
{
   auto key = std::make_pair(eqId, it);
   assert(this->mVectorEqMap.count(key) > 0);
...
void Coordinator::initTransformCoordinator()
{
   ...
   Transform::TransformCoordinatorTools::computePacks(packs,
      this->mspFwdGrouper, this->mspBwdGrouper, this->mFwdTree, mBwdTree,
      this->it(), this->mspRes);
   ...
   const Parallel::SplittingDescription& descr)
{
   ...
   Parallel::setGrouper(descr, this->mspImposedFwdGrouper,
...
void Coordinator::updatePhysical(const int it)
{
   Profiler::RegionFixture<1> fix(
      "Pseudospectral::Coordinator::updatePhysical");

   // Define transform trees
   this->mTransformCoordinator.defineBwdTransforms(this->mBwdTree.at(it));

   // Compute physical values
   this->mspBwdGrouper->transform(this->mScalarVariables,
      this->mVectorVariables, this->mTransformCoordinator);
}
...
void Coordinator::updateEquations(const int it, const bool isFinished) const
{
   ...
   for (auto scalEqIt = this->mScalarEquations.at(it).begin();
        scalEqIt < this->mScalarEquations.at(it).end(); ++scalEqIt)
   {
      Profiler::RegionFixture<2> fixId(
         profRegion + "-" +
         PhysicalNames::Coordinator::tag((*scalEqIt)->name()));
      (*scalEqIt)->updateConstraintKernel(this->time(), this->timestep(),
         isFinished);
   }

   // Loop over all vector equations
   assert(this->mVectorEquations.count(it) == 1);
   for (auto vectEqIt = this->mVectorEquations.at(it).begin();
        vectEqIt < this->mVectorEquations.at(it).end(); ++vectEqIt)
   {
      Profiler::RegionFixture<2> fixId(
         profRegion + "-" +
         PhysicalNames::Coordinator::tag((*vectEqIt)->name()));
      (*vectEqIt)->updateConstraintKernel(this->time(), this->timestep(),
         isFinished);
   }
}

void Coordinator::writeDiagnostics(const bool isAsciiTime,
   const bool isHdf5Time) const
{
   Profiler::RegionFixture<1> fix(
      "Pseudospectral::Coordinator::writeDiagnostics");
   ...
   ScalarEquation_range scalarEq_range, VectorEquation_range vectorEq_range)
{
   ...
   {
      DebuggerMacro_msg("Explicit term for trivial equations for operator " +
                           ModelOperator::Coordinator::tag(opId),
         5);
      this->mTrivialCoordinator.getExplicitInput(opId, scalarEq_range,
         vectorEq_range, this->mScalarVariables, this->mVectorVariables);
   }
}
   ...
   ScalarEquation_range scalarEq_range, VectorEquation_range vectorEq_range)
{
   if (this->atLeastOne(scalarEq_range, vectorEq_range))
   {
      DebuggerMacro_msg("Explicit term for diagnostic equations for operator " +
                           ModelOperator::Coordinator::tag(opId),
         5);
      this->mLinearCoordinator.getExplicitInput(opId, scalarEq_range,
         vectorEq_range, this->mScalarVariables, this->mVectorVariables);
   }
}
   ...
   ScalarEquation_range scalarEq_range, VectorEquation_range vectorEq_range)
{
   if (this->atLeastOne(scalarEq_range, vectorEq_range))
   {
      DebuggerMacro_msg("Explicit term for prognostic equations for operator " +
                           ModelOperator::Coordinator::tag(opId),
         5);
      this->mTimestepCoordinator.getExplicitInput(opId, scalarEq_range,
         vectorEq_range, this->mScalarVariables, this->mVectorVariables);
   }
}
   ...
   ScalarEquation_range scalarEq_range, VectorEquation_range vectorEq_range)
{
   if (this->atLeastOne(scalarEq_range, vectorEq_range))
   {
      DebuggerMacro_msg("Solve trivial equations at time " +
                           SolveTiming::Coordinator::tag(timeId),
         5);
      this->mTrivialCoordinator.setSolveTime(timeId);
      this->mTrivialCoordinator.solve(scalarEq_range, vectorEq_range,
         this->mScalarVariables, this->mVectorVariables);
   }
}
   ...
   ScalarEquation_range scalarEq_range, VectorEquation_range vectorEq_range)
{
   if (this->atLeastOne(scalarEq_range, vectorEq_range))
   {
      DebuggerMacro_msg("Solve diagnostic equations at time " +
                           SolveTiming::Coordinator::tag(timeId),
         5);
      this->mLinearCoordinator.setSolveTime(timeId);
      this->mLinearCoordinator.solve(scalarEq_range, vectorEq_range,
         this->mScalarVariables, this->mVectorVariables);
   }
}
...
{
   auto sP = const_cast<Coordinator*>(this)->scalarRange(
      PseudospectralTag::Prognostic::id(), it);
   auto vP = const_cast<Coordinator*>(this)->vectorRange(
      PseudospectralTag::Prognostic::id(), it);
   return this->atLeastOne(sP, vP);
}

void Coordinator::solvePrognosticEquations(ScalarEquation_range scalarEq_range,
   VectorEquation_range vectorEq_range)
{
   // Only step forward if equations are present
   if (this->atLeastOne(scalarEq_range, vectorEq_range))
   {
      DebuggerMacro_msg("Timestep prognostic equations", 5);
      this->mTimestepCoordinator.stepForward(scalarEq_range, vectorEq_range,
         this->mScalarVariables, this->mVectorVariables);
   }
}
...
{
   Profiler::RegionFixture<1> fix(
      "Pseudospectral::Coordinator::solveEquationsBefore");

   // Solve trivial equations
   Profiler::RegionStart<2>(
      "Pseudospectral::Coordinator::solveEquations-trivialBefore");
   ...
   Profiler::RegionStart<2>(
      "Pseudospectral::Coordinator::solveEquations-diagnosticBefore");
...
{
   Profiler::RegionFixture<1> fix(
      "Pseudospectral::Coordinator::solveEquationsAfter");

   // Solve diagnostic equations
   Profiler::RegionStart<2>(
      "Pseudospectral::Coordinator::solveEquations-diagnosticAfter");
   ...
   // Solve trivial equations
   Profiler::RegionStart<2>(
      "Pseudospectral::Coordinator::solveEquations-trivialAfter");
...
{
   Profiler::RegionFixture<1> fix(
      "Pseudospectral::Coordinator::finalizeTimestep");
...
void Coordinator::setupEquations()
{
   ...
   auto spMesh =
      std::make_shared<std::vector<Array>>(this->mTransformCoordinator.mesh());
   ...
   for (auto k: this->it())
   {
      for (auto sEqIt = this->mScalarEquations.at(k).begin();
           sEqIt < this->mScalarEquations.at(k).end(); ++sEqIt)
      {
         for (auto j: this->it())
         {
            // Loop over all scalar equations
      ...
      for (auto vEqIt = this->mVectorEquations.at(k).begin();
           vEqIt < this->mVectorEquations.at(k).end(); ++vEqIt)
      {
         for (auto j: this->it())
         {
            // Loop over all scalar equations
...
void Coordinator::addToMap(const std::size_t eqId, const int it,
   const ScalarEquation_range& r)
{
   auto key = std::make_pair(eqId, it);
   ...
   {
      this->mScalarEqMap.insert(
         std::make_pair(key, std::vector<SharedIScalarEquation>()));
   }

   if (this->mScalarEqMap.at(key).size() > 0)
   {
      throw std::logic_error(
         "Manually added Prognostic equation not supported");
   }
   else
   {
      ...
      for (auto eqIt = r.first; eqIt != r.second; eqIt++)
      {
         ...
         auto ur =
            this->scalarRange(PseudospectralTag::Uninitialized::id(), it);
   ...
   const VectorEquation_range& r)
{
   ...
   {
      this->mVectorEqMap.insert(
         std::make_pair(key, std::vector<SharedIVectorEquation>()));
   }
   ...
   else
   {
      ...
      for (auto eqIt = r.first; eqIt != r.second; eqIt++)
      {
         ...
         auto ur =
            this->vectorRange(PseudospectralTag::Uninitialized::id(), it);
         auto uit = std::find(ur.first, ur.second, *eqIt);
         if (uit != ur.second)
         {
...
void Coordinator::sortEquations()
{
   ...
   for (auto j: this->it())
   {
      ...
      // Identifiy the solver indexes by analysing the coupling between the
      // equations
      DebuggerMacro_msg(
         "Identifying solver for Prognostic equations sub-iteration " +
            std::to_string(j),
         1);
      pStart =
         Equations::Tools::identifySolver(sP, vP, pStart.first, pStart.second);
      DebuggerMacro_msg("... done", 1);
      DebuggerMacro_msg(
         "Identifying solver for Diagnostic equations sub-iteration " +
            std::to_string(j),
         1);
      dStart =
         Equations::Tools::identifySolver(sD, vD, dStart.first, dStart.second);
      DebuggerMacro_msg("... done", 1);
      DebuggerMacro_msg(
         "Identifying solver for Trivial equations sub-iteration " +
            std::to_string(j),
         1);
      tStart =
         Equations::Tools::identifySolver(sT, vT, tStart.first, tStart.second);
      DebuggerMacro_msg("... done", 1);
   }
}

bool Coordinator::atLeastOne(ScalarEquation_range s,
   VectorEquation_range v) const
{
   bool r = ((std::distance(s.first, s.second) +
                std::distance(v.first, v.second)) > 0);
