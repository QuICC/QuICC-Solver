   ...
   const Eigen::MatrixBase<DERIVED>& eig, const TransformResolution& res)
{
   ...
   tempOnHostMemorySpace Converter(viewb,
      TransferMode::write | TransferMode::block);
   ...
   for (std::uint32_t p = 0; p < nLayers; ++p)
   {
      ...
      const Eigen::Ref<const Eigen::Matrix<SCALAR, -1, -1>> inB =
         eig.block(0, start, inRows, cols);
      ...
      for (std::int64_t j = 0; j < inB.cols(); ++j)
      {
         for (std::int64_t i = 0; i < inB.rows(); ++i)
         {
            if constexpr (std::is_same_v<VIEWATT, View::DCCSC3DJIK>)
            {
               ...
               viewb[offSet + i * widthView + j] =
                  inB.data()[i + j * eig.rows()];
            }
            else if constexpr (std::is_same_v<VIEWATT, View::DCCSC3D>)
            {
               // copy padded to flattened column
               viewb[offSet + i + j * heightView] =
                  inB.data()[i + j * eig.rows()];
            }
            else
            {
               throw std::logic_error(
                  "copy 2 view not implemented for this layout");
            }
         }
      }
      offSet += inB.size();
   ...
   const TransformResolution& res)
{
   ...
   for (std::uint32_t p = 0; p < nLayers; p++)
   {
      ...
      Eigen::Ref<Eigen::Matrix<SCALAR, -1, -1>> outB =
         eig.block(0, start, outRows, cols);
      ...
      for (std::int64_t j = 0; j < outB.cols(); ++j)
      {
         for (std::int64_t i = 0; i < outB.rows(); ++i)
         {
            ...
            {
               throw std::logic_error(
                  "copy from view not implemented for this layout");
            }
         }
      }
   ...
   const TransformResolution& res)
{
   ...
   std::visit(
      ...
      {
         auto& ptrTor =
            p->rDom(0).rPerturbation().rComp(FieldComponents::Spectral::TOR);
         details::copyEig2View(Torv, ptrTor.data(), res);
         auto& ptrPol =
            p->rDom(0).rPerturbation().rComp(FieldComponents::Spectral::POL);
   ...
   const TransformResolution& res)
{
   ...
   std::visit(
      [&](auto&& p, auto& Urv, auto& Uthetav, auto& Uphiv)
      {
         ...
         auto& ptrUtheta =
            p->rDom(0).rPhys().rComp(FieldComponents::Physical::THETA);
         details::copyView2Eig(ptrUtheta.rData(), Uthetav, res);
         auto& ptrUphi =
            p->rDom(0).rPhys().rComp(FieldComponents::Physical::PHI);
