...
void Coordinator::computeNonlinear(const int it, const bool saveOutput)
{
   ...
   else
   {
      ...
      // Physical velocity
      std::size_t hVelR = hash_combine(PhysicalNames::Velocity::id(),
      ...
      if (std::holds_alternative<C_DCCSC3D_t>(tempVarv))
      {
         ...
         if (!mIsMag)
         {
            mJitter->apply(Tv, TorVelv, PolVelv, Urv, Uthetav, Uphiv, Tv,
               TorVelv, PolVelv);
         }
         else
         {
            ...
            mJitter->apply(Tv, TorVelv, PolVelv, TorMagv, PolMagv, Urv, Uthetav,
               Uphiv, Brv, Bthetav, Bphiv, Tv, TorVelv, PolVelv, TorMagv,
               PolMagv);
         }
      }
