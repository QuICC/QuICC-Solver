 ...
 * @brief Source of the implementation of the physical <-> spectral transform
...
#include "QuICC/Transform/Backward/OverrSq.hpp"
#include "QuICC/Transform/Backward/OverrSqD1R1.hpp"
...
#include "QuICC/Transform/Forward/OversinLaplhm1Dphi.hpp"
#include "QuICC/Transform/Forward/D1Laplhm1.hpp"
...
ShellTransformSteps::ShellTransformSteps(
   std::shared_ptr<const SpatialScheme::ISpatialScheme> spScheme) :
    ITransformSteps(spScheme)
{}

bool ShellTransformSteps::applicable(
   std::shared_ptr<const SpatialScheme::ISpatialScheme> spScheme)
{
   return spScheme->has(SpatialScheme::Feature::ShellGeometry);
}

std::vector<TransformPath> ShellTransformSteps::forwardScalar(
   const std::vector<PathId>& components) const
{
   ...
   {
      transform.push_back(
         TransformPath(FieldComponents::Physical::SCALAR, FieldType::SCALAR));
      transform.back().addEdge(Forward::P::id());
      transform.back().addEdge(Forward::P::id());
      transform.back().addEdge(Forward::P::id(), scalId,
         Arithmetics::Add::id());
   }
   else
   {
...
std::vector<TransformPath> ShellTransformSteps::forwardNLScalar(
   const std::vector<PathId>& components) const
{
   assert(components.size() == 1);
   std::vector<TransformPath> transform;
   ...
   else
   {
      throw std::logic_error(
         "Requested an unknown nonlinear scalar forward transform");
   }

   return transform;
}

std::vector<TransformPath> ShellTransformSteps::forwardVector(
   const std::vector<PathId>& components) const
{
   std::vector<TransformPath> transform;
   ...
   if (this->ss().formulation() == VectorFormulation::TORPOL)
   {
      ...
      if (curlFlag == Path::TorPol::id() && curlcurlFlag == Path::TorPol::id())
      {
         ...
         transform.push_back(
            TransformPath(FieldComponents::Physical::THETA, FieldType::VECTOR));
         transform.back().addEdge(Forward::P::id());
         transform.back().addEdge(Forward::OverlaplhOversinDphi::id());
         transform.back().addEdge(Forward::T::id(), curlId,
            Arithmetics::Add::id());

         transform.push_back(
            TransformPath(FieldComponents::Physical::PHI, FieldType::VECTOR));
         transform.back().addEdge(Forward::P::id());
         ...
         transform.push_back(
            TransformPath(FieldComponents::Physical::R, FieldType::VECTOR));
         transform.back().addEdge(Forward::P::id());
         transform.back().addEdge(Forward::Overlaplh::id());
         transform.back().addEdge(Forward::Pol::id(), curlcurlId,
            Arithmetics::Add::id());
      }
      else
      {
         throw std::logic_error(
            "Requested an unknown vector forward transform");
      }
   }
   else
   {
      ...
      transform.back().addEdge(Forward::P::id());
      transform.back().addEdge(Forward::P::id());
      transform.back().addEdge(Forward::P::id(), FieldComponents::Spectral::R,
         Arithmetics::Add::id());
      ...
      transform.back().addEdge(Forward::P::id());
      transform.back().addEdge(Forward::P::id(),
         FieldComponents::Spectral::THETA, Arithmetics::Add::id());

      transform.push_back(
         TransformPath(FieldComponents::Physical::PHI, FieldType::VECTOR));
      transform.back().addEdge(Forward::P::id());
      transform.back().addEdge(Forward::P::id());
      transform.back().addEdge(Forward::P::id(), FieldComponents::Spectral::PHI,
         Arithmetics::Add::id());
   }
...
std::vector<TransformPath> ShellTransformSteps::forwardNLVector(
   const std::vector<PathId>& components) const
{
   std::vector<TransformPath> transform;
   ...
   if (this->ss().formulation() == VectorFormulation::TORPOL)
   {
      ...
      if (curlFlag == Path::I2CurlNl::id())
      {
         ...
         transform.back().addEdge(Forward::P::id());
         transform.back().addEdge(Forward::OverlaplhOversinDphi::id());
         transform.back().addEdge(Forward::I2T::id(), curlId,
            Arithmetics::Add::id());
      ...
      {
         throw std::logic_error(
            "Requested an unknown curl nonlinear vector forward transform");
      }

      // Integrate for standard fourth order spherical equation with negative
      // sign
      if (curlcurlFlag == Path::NegI4CurlCurlNl::id())
      {
         ...
         transform.back().addEdge(Forward::P::id());
         transform.back().addEdge(Forward::P::id());
         transform.back().addEdge(Forward::I4Q::id(), curlcurlId,
            Arithmetics::Sub::id());
         ...
         transform.back().addEdge(Forward::P::id());
         transform.back().addEdge(Forward::OverlaplhD1::id());
         transform.back().addEdge(Forward::I4S::id(), curlcurlId,
            Arithmetics::Add::id());
      ...
      // Integrate for standard second order spherical equation with negative
      // sign
      else if (curlcurlFlag == Path::NegI2CurlCurlNl::id())
      {
         ...
         transform.back().addEdge(Forward::P::id());
         transform.back().addEdge(Forward::P::id());
         transform.back().addEdge(Forward::I2Q::id(), curlcurlId,
            Arithmetics::Sub::id());
         ...
         transform.back().addEdge(Forward::P::id());
         transform.back().addEdge(Forward::OverlaplhD1::id());
         transform.back().addEdge(Forward::I2S::id(), curlcurlId,
            Arithmetics::Add::id());
      ...
      // Integrate for standard second order spherical equation with negative
      // sign and an additional factor of r
      else if (curlcurlFlag == Path::NegI2rCurlCurlNl::id())
      {
         ...
         transform.back().addEdge(Forward::P::id());
         transform.back().addEdge(Forward::P::id());
         transform.back().addEdge(Forward::I2rQ::id(), curlcurlId,
            Arithmetics::Sub::id());
         ...
         transform.back().addEdge(Forward::P::id());
         transform.back().addEdge(Forward::OverlaplhD1::id());
         transform.back().addEdge(Forward::I2rS::id(), curlcurlId,
            Arithmetics::Add::id());
   ...
   const PhysPathId& req) const
{
   ...
   transform.push_back(
      TransformPath(FieldComponents::Spectral::SCALAR, FieldType::SCALAR));
   transform.back().addEdge(Backward::P::id());
   transform.back().addEdge(Backward::P::id());
   transform.back().addEdge(Backward::P::id(),
      FieldComponents::Physical::SCALAR, Arithmetics::Add::id());
   ...
   const PhysPathId& req) const
{
   ...
   if (req.find(FieldComponents::Physical::R)->second != Path::Empty::id())
   {
      ...
      transform.back().addEdge(Backward::P::id());
      transform.back().addEdge(Backward::P::id(), FieldComponents::Physical::R,
         Arithmetics::Add::id());
   }
   ...
   if (req.find(FieldComponents::Physical::PHI)->second != Path::Empty::id())
   {
      ...
      transform.back().addEdge(Backward::P::id(),
         FieldComponents::Physical::PHI, Arithmetics::Add::id());
   }
   ...
   std::vector<TransformPath>  ShellTransformSteps::backwardGradient(const std::map<std::pair<FieldComponents::Physical::Id,FieldComponents::Physical::Id>,bool>& req) const
   {
      ...
      if(this->ss().formulation() == VectorFormulation::TORPOL)
      {
         ...
         {  //du_r/dr
            ...
            transform.back().addEdge(Backward::Overr1D1::id());
         ...
         {  // (1/r)du_r/dt - u_t/r
            ...
            // Poloidal part
            /*
            // version 1: (1/r^2)d( L^2 S)/dt - (1/r^2)d(r dS/dt )/dr
            // -> the presence of repeated paths breaks optimisation...
            // part 1: (1/r^2)d( L^2 S)/dt -> this is already present in the curl
            transform.push_back(TransformPath(FieldComponents::Spectral::POL, FieldType::GRADIENT));
            transform.back().addEdge(Backward::Overr2::id());
            transform.back().addEdge(Backward::D1Laplh::id());
            transform.back().addEdge(Backward::P::id(), pairId, Arithmetics::Add::id());
            // part 2 : - (1/r^2)d(r dS/dt )/dr
            transform.push_back(TransformPath(FieldComponents::Spectral::POL, FieldType::GRADIENT));
            transform.back().addEdge(Backward::Overr2D1R1::id());
            transform.back().addEdge(Backward::D1::id());
            transform.back().addEdge(Backward::P::id(), pairId, Arithmetics::Sub::id());
            */
            // version 2: (1/r^2)L^2 dS/dt - (1/r^2)dS/dt - (1/r)d( dS/dt )/dr
            // -> the presence of repeated paths breaks optimisation...
            /*
            transform.push_back(TransformPath(FieldComponents::Spectral::POL, FieldType::GRADIENT));
            transform.back().addEdge(Backward::Overr2::id());
            ...
            transform.back().addEdge(Backward::P::id(), pairId, Arithmetics::Sub::id());
            */
            // looks like whatever contains a L^2 is doomed to break optimisation....
            // version 3:  grad(u)_{tr} - culr_phi
            // -culr_phi : (already a problem)
            /*
            transform.push_back(TransformPath(FieldComponents::Spectral::POL, FieldType::GRADIENT));
            ...
            */
            ...
            // version 4: (1/r^2)(L^2-1) dS/dt - (1/r)d( dS/dt )/dr
            // works
            //(1/r^2)(L^2-1) dS/dt
            transform.push_back(TransformPath(FieldComponents::Spectral::POL, FieldType::GRADIENT));
            transform.back().addEdge(Backward::Overr2::id());
            transform.back().addEdge(Backward::D1Laplhm1::id());
            transform.back().addEdge(Backward::P::id(), pairId, Arithmetics::Add::id());
            //- (1/r)d( dS/dt )/dr
            transform.push_back(TransformPath(FieldComponents::Spectral::POL, FieldType::GRADIENT));
            transform.back().addEdge(Backward::Overr1D1::id());
         ...
         {// 1/(r sin t) * d u_r/dp - u_p/r
            ...
            // Poloidal part
            /*
            // variant 1: breaks optimisation
            // exists in curl_theta
            transform.push_back(TransformPath(FieldComponents::Spectral::POL, FieldType::GRADIENT));
            transform.back().addEdge(Backward::Overr2::id());
            ...
            */
            ...
            transform.push_back(TransformPath(FieldComponents::Spectral::POL, FieldType::GRADIENT));
            transform.back().addEdge(Backward::Overr2::id());
            transform.back().addEdge(Backward::OversinLaplhm1Dphi::id());
            transform.back().addEdge(Backward::P::id(), pairId, Arithmetics::Add::id());
         ...
         {  //du_t/dr
            ...
            /*
            transform.push_back(TransformPath(FieldComponents::Spectral::POL, FieldType::GRADIENT));
            transform.back().addEdge(Backward::SLaplr::id()); // this exact path is already used in curl_phi. Breaks optimisation
            transform.back().addEdge(Backward::D1::id());
            transform.back().addEdge(Backward::P::id(), pairId, Arithmetics::Add::id());
            */
            // variant 1: works
            ...
            transform.push_back(TransformPath(FieldComponents::Spectral::POL, FieldType::GRADIENT));
            transform.back().addEdge(Backward::Overr1D2R1::id()); // I just renamed SLaplr to Overr1D2R1 (it's the same operation).
            transform.back().addEdge(Backward::D1::id());
            transform.back().addEdge(Backward::P::id(), pairId, Arithmetics::Add::id());
         ...
         { // (1/r) * du_t/t + u_r/r
            ...
            // Poloidal part   
            // this bit breaks optimisation
            /*
            transform.push_back(TransformPath(FieldComponents::Spectral::POL, FieldType::GRADIENT));
            transform.back().addEdge(Backward::Overr2::id()); // rename this operator?
            transform.back().addEdge(Backward::Laplh::id());
            transform.back().addEdge(Backward::P::id(), pairId, Arithmetics::Add::id());
            */
         ...
         {  //du_p/dr
            ...
            transform.push_back(TransformPath(FieldComponents::Spectral::POL, FieldType::GRADIENT));
            transform.back().addEdge(Backward::Overr1D2R1::id()); // it's Slaplr
            transform.back().addEdge(Backward::OversinDphi::id());
            transform.back().addEdge(Backward::P::id(), pairId, Arithmetics::Add::id());
         ...
         {// 1/(r sin t) * du_p/dp + cot t * u_t/r + u_r/r
            ...
            //2 x the same operator. Not good
            transform.push_back(TransformPath(FieldComponents::Spectral::POL, FieldType::GRADIENT));
            transform.back().addEdge(Backward::Overr2::id());
            transform.back().addEdge(Backward::Laplh::id());
            transform.back().addEdge(Backward::P::id(), pairId, Arithmetics::Add::id());
   ...
   const Grad2PathId& req) const
{
   ...
   if (req.find(pairId)->second != Path::Empty::id())
   {
      ...
      transform.back().addEdge(Backward::P::id());
      transform.back().addEdge(Backward::P::id());
      transform.back().addEdge(Backward::P::id(), pairId,
         Arithmetics::Add::id());
   }
   ...
   const PhysPathId& req) const
{
   ...
   if (this->ss().formulation() == VectorFormulation::TORPOL)
   {
      ...
      if (req.find(FieldComponents::Physical::PHI)->second != Path::Empty::id())
      {
         ...
         transform.back().addEdge(Backward::D1::id());
         transform.back().addEdge(Backward::P::id(),
            FieldComponents::Physical::PHI, Arithmetics::Sub::id());
   ...
   const PhysPathId& req) const
{
   ...
   if (this->ss().formulation() == VectorFormulation::TORPOL)
   {
          ...
          Path::Empty::id())
      {
         ...
         // Poloidal part 1
         transform.push_back(
            TransformPath(FieldComponents::Spectral::POL, FieldType::CURL));
         transform.back().addEdge(Backward::Slaplr::id());
         transform.back().addEdge(Backward::OversinDphi::id());
         transform.back().addEdge(Backward::P::id(),
   ...
   else
   {
      if (req.find(FieldComponents::Physical::R)->second != Path::Empty::id())
      {
         ...
         transform.back().addEdge(Backward::OversinDphi::id());
         transform.back().addEdge(Backward::P::id(),
            FieldComponents::Physical::R, Arithmetics::Sub::id());

         transform.push_back(
            TransformPath(FieldComponents::Spectral::PHI, FieldType::CURL));
         transform.back().addEdge(Backward::Overr1::id());
...
std::vector<TransformPath> ShellTransformSteps::backwardDivergence() const
{
   ...
   if (this->ss().formulation() == VectorFormulation::TORPOL)
   {
      ...
      throw std::logic_error(
         "Divergence should not be used in Toroidal/Poloidal expansion");
   }
   else
   {
      transform.push_back(
         TransformPath(FieldComponents::Spectral::R, FieldType::DIVERGENCE));
      transform.back().addEdge(Backward::D1::id());
      transform.back().addEdge(Backward::P::id());
      transform.back().addEdge(Backward::P::id(),
         FieldComponents::Physical::SCALAR, Arithmetics::Add::id());

      transform.push_back(TransformPath(FieldComponents::Spectral::THETA,
         FieldType::DIVERGENCE));
      transform.back().addEdge(Backward::Overr1::id());
      transform.back().addEdge(Backward::OversinD1Sin::id());
      transform.back().addEdge(Backward::P::id(),
         FieldComponents::Physical::SCALAR, Arithmetics::Add::id());
      ...
      transform.push_back(
         TransformPath(FieldComponents::Spectral::PHI, FieldType::DIVERGENCE));
      transform.back().addEdge(Backward::Overr1::id());
      transform.back().addEdge(Backward::OversinDphi::id());
      transform.back().addEdge(Backward::P::id(),
         FieldComponents::Physical::SCALAR, Arithmetics::Add::id());
