...
void BackwardConfigurator::prepareSpectral(const TransformTree& tree,
   Framework::Selector::VariantSharedScalarVariable& rScalar,
   TransformCoordinatorType& coord)
{
   Profiler::RegionFixture<2> fix(
      "Transform::BackwardConfigurator::prepareSpectral");

   // Debugger message
   DebuggerMacro_msg("prepareSpectral (scalar)", 4);

   // Safety assert
   assert(tree.comp<FieldComponents::Spectral::Id>() ==
   ...
   std::visit(
      [&](auto&& p) {
         coord.communicator().transferForward(traId, p->rDom(0).rTotal(),
            false);
      },
      rScalar);
}
...
void BackwardConfigurator::prepareSpectral(const TransformTree& tree,
   Framework::Selector::VariantSharedVectorVariable& rVector,
   TransformCoordinatorType& coord)
{
   // Debugger message
   ...
   std::visit(
      [&](auto&& p)
      {
         coord.communicator().transferForward(traId,
            p->rDom(0).rTotal().rComp(
               tree.comp<FieldComponents::Spectral::Id>()),
            false);
      },
      rVector);
}

void BackwardConfigurator::preparePhysical(const TransformTree&,
   const TransformTreeEdge& edge,
   Framework::Selector::VariantSharedScalarVariable& rScalar,
   TransformCoordinatorType& coord)
{
   Profiler::RegionFixture<2> fix(
      "Transform::BackwardConfigurator::preparePhysical");
   ...
   {
      std::visit([&](auto&& p)
         { coord.communicator().holdPhysical(p->rDom(0).rPhys()); },
         rScalar);
   }
   ...
   else if (edge.fieldId() == FieldType::GRADIENT)
   {
      std::visit(
         [&](auto&& p)
         {
            coord.communicator().holdPhysical(p->rDom(0).rGrad().rComp(
               edge.outId<FieldComponents::Physical::Id>()));
         },
         rScalar);
   }
   ...
   else if (edge.fieldId() == FieldType::GRADIENT2)
   {
      std::visit(
         [&](auto&& p)
         {
            coord.communicator().holdPhysical(p->rDom(0).rGrad2().rComp(
               edge.outId<FieldComponents::Physical::Id>(0),
               edge.outId<FieldComponents::Physical::Id>(1)));
         },
         rScalar);
   }
}

void BackwardConfigurator::preparePhysical(const TransformTree& tree,
   const TransformTreeEdge& edge,
   Framework::Selector::VariantSharedVectorVariable& rVector,
   TransformCoordinatorType& coord)
{
   ...
   if (edge.fieldId() == FieldType::VECTOR)
   {
      std::visit(
         [&](auto&& p)
         {
            coord.communicator().holdPhysical(p->rDom(0).rPhys().rComp(
               edge.outId<FieldComponents::Physical::Id>()));
         },
         rVector);
   }
   ...
   {  
      ...
      {// tensor gradient form         
         std::visit([&](auto&& p)
         {
            coord.communicator().holdPhysical(
               p->rDom(0)
                  .rGrad()
                  .rComp(edge.outId<FieldComponents::Physical::Id>(0),edge.outId<FieldComponents::Physical::Id>(1)));
               }, 
               rVector);
      }
      else
      {// vector gradient form:
         std::visit(
            [&](auto&& p)
            {
               coord.communicator().holdPhysical(
                  p->rDom(0)
                     .rGrad(tree.comp<FieldComponents::Spectral::Id>())
                     .rComp(edge.outId<FieldComponents::Physical::Id>()));
            },
            rVector);      
      }
   }
   ...
   else if (edge.fieldId() == FieldType::CURL)
   {
      std::visit(
         [&](auto&& p)
         {
            coord.communicator().holdPhysical(p->rDom(0).rCurl().rComp(
               edge.outId<FieldComponents::Physical::Id>()));
         },
         rVector);
   }
...
void BackwardConfigurator::project1D(const TransformTreeEdge& edge,
   TransformCoordinatorType& coord)
{
   // Debugger message
   DebuggerMacro_msg("Project 1D with operator (" +
                        Backward::Coordinator::tag(edge.opId()) + ")",
      6);
   ...
   BackwardConfigurator::genericProjection(edge, coord, traId, processOutput,
      profRegion);
}

void BackwardConfigurator::project2D(const TransformTreeEdge& edge,
   TransformCoordinatorType& coord)
{
   // Debugger message
   DebuggerMacro_msg("Project 2D with operator (" +
                        Backward::Coordinator::tag(edge.opId()) + ")",
      6);
...
void BackwardConfigurator::projectND(const TransformTreeEdge& edge,
   TransformCoordinatorType& coord)
{
   // Debugger message
   DebuggerMacro_msg("Project ND with operator (" +
                        Backward::Coordinator::tag(edge.opId()) + ")",
      6);

   const std::string profRegion = "Transform::BackwardConfigurator::projectND";
   const auto traId =
      static_cast<Dimensions::Transform::Id>(coord.ss().dimension() - 1);
   const bool processOutput = false;
   BackwardConfigurator::genericProjection(edge, coord, traId, processOutput,
      profRegion);
}

void BackwardConfigurator::genericProjection(const TransformTreeEdge& edge,
   TransformCoordinatorType& coord, const Dimensions::Transform::Id traId,
   const bool processOutput, const std::string profRegion)
{
   ...
   {
      if (edge.arithId() == Arithmetics::Set::id() ||
          edge.arithId() == Arithmetics::SetNeg::id())
      {
   ...
   std::visit(
      [&](auto&& pOut, auto&& pIn) {
         coord.transform(traId).backward(pOut->rData(), pIn->data(),
            edge.opId());
      },
      pOutVar, pInVar);
   ...
   if (processOutput)
   {
      ...
      if (std::visit([](auto&& pRecOut) { return (pRecOut != 0); }, pRecOutVar))
      {
         ...
         std::visit(
            [&](auto&& pRecOut, auto&& pOut) {
               Datatypes::FieldTools::combine(*pRecOut, *pOut,
                  edge.combinedArithId());
            },
            pRecOutVar, pOutVar);
         ...
         {
            coord.communicator().storage(traId).holdFwd(pRecOutVar,
               edge.combinedOutId());
         }
         else
         {
      ...
      else if (edge.combinedOutId() >= 0)
      {
         ...
         {
            std::visit([](auto&& pOut)
               { Datatypes::FieldTools::negative(*pOut); },
               pOutVar);
         }
         ...
         coord.communicator().storage(traId).holdFwd(pOutVar,
            edge.combinedOutId());
      ...
      if (edge.arithId() != Arithmetics::None::id())
      {
         ...
         Profiler::RegionStop<4>(profRegion + "-post" + "-comm_d");
               ...
               std::visit([](auto&& pRecOut) { return (pRecOut != 0); },
                  pRecOutVar))
      {
   ...
   else
   {
      ...
      if (std::visit([](auto&& pRecOut) { return (pRecOut != 0); }, pRecOutVar))
      {
         ...
         std::visit(
            [&](auto&& pRecOut, auto&& pOut) {
               Datatypes::FieldTools::combine(*pRecOut, *pOut, edge.arithId());
            },
            pRecOutVar, pOutVar);
