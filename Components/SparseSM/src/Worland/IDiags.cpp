...
IDiags::ACoeff_t IDiags::normalizeDiag(const ACoeff_t& n, const int k,
   const int p) const
{
   if (n(0) + k < 0)
   {
   ...
   if (this->mNorm.count(this->l()) > 0 &&
       this->mNorm.count(this->l() + dp) > 0)
   {
      // Check sizes are correct
      assert(this->mNorm.at(this->l()).size() >
             static_cast<int>(n.bottomRows(1)(0)));
      assert(this->mNorm.at(this->l() + dp).size() >
             static_cast<int>(n.bottomRows(1)(0) + k));

      auto n0 = static_cast<int>(n(0));
      norm = (this->mNorm.at(this->l()).segment(n0, n.size()) -
              this->mNorm.at(this->l() + dp).segment(n0 + k, n.size()))
                .exp();
   }
   else
   {
      norm =
         (this->lnorm(n, this->l()) - this->lnorm(n + k, this->l() + dp)).exp();
   }

   return norm;
}

IDiags::ACoeff_t IDiags::lnorm_chebyshev(const ACoeff_t& n,
   const Scalar_t l) const
{
   ACoeff_t norm;

   if (l == 0)
   {
      norm = -Internal::Math::log(MHD_MP(2.0)) + (n + MHD_MP(0.5)).lgamma() -
             (n + MHD_MP(1.0)).lgamma();
      if (n(0) == 0)
      {
         norm(0) = MHD_MP(0.5) * (Internal::Math::log(Internal::Math::PI) -
                                    Internal::Math::log(MHD_MP(2.0)));
      }
   }
   else
   {
      norm = -(MHD_MP(2.0) * (MHD_MP(2.0) * n + l)).log() +
             (n + MHD_MP(0.5)).lgamma() + (n + l + MHD_MP(0.5)).lgamma() -
             (n + l).lgamma() - (n + MHD_MP(1.0)).lgamma();
      norm *= MHD_MP(0.5);
   }
...
IDiags::ACoeff_t IDiags::lnorm_legendre(const ACoeff_t& n,
   const Scalar_t l) const
{
   Scalar_t b = this->beta(l);
   ACoeff_t norm =
      -MHD_MP(0.5) * (MHD_MP(2.0) * (MHD_MP(2.0) * n + b + MHD_MP(1.0))).log();
