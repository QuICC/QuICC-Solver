...
#include "QuICC/SparseSM/Worland/I3D2p.hpp"
...
I3D2p::I3D2p(const int rows, const int cols, const Scalar_t alpha,
    ...
    IWorlandOperator(rows, cols, alpha, dBeta)
{
   ...
   case WorlandKind::CHEBYSHEV:
      this->mpImpl = std::make_shared<Chebyshev::I3D2pDiags>(alpha, l, q);
      break;
...
void I3D2p::buildTriplets(TripletList_t& list) const
{
   const int dShift = 2;
...
void I3D2p::buildBanded(Internal::Matrix& bd, unsigned int& kL,
