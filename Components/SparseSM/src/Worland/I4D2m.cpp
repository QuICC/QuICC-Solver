...
#include "QuICC/SparseSM/Worland/I4D2m.hpp"
...
I4D2m::I4D2m(const int rows, const int cols, const Scalar_t alpha,
    ...
    IWorlandOperator(rows, cols, alpha, dBeta)
{
   ...
   case WorlandKind::CHEBYSHEV:
      this->mpImpl = std::make_shared<Chebyshev::I4D2mDiags>(alpha, l, q);
      break;
...
void I4D2m::buildTriplets(TripletList_t& list) const
{
   const int dShift = 2;
...
void I4D2m::buildBanded(Internal::Matrix& bd, unsigned int& kL,
