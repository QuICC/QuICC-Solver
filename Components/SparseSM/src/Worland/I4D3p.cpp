...
#include "QuICC/SparseSM/Worland/I4D3p.hpp"
...
I4D3p::I4D3p(const int rows, const int cols, const Scalar_t alpha,
    ...
    IWorlandOperator(rows, cols, alpha, dBeta)
{
   ...
   case WorlandKind::CHEBYSHEV:
      this->mpImpl = std::make_shared<Chebyshev::I4D3pDiags>(alpha, l, q);
      break;
...
void I4D3p::buildTriplets(TripletList_t& list) const
{
   const int dShift = 2;
...
void I4D3p::buildBanded(Internal::Matrix& bd, unsigned int& kL,
