/**
 * @file I3D2pDiags.cpp
 * @brief Source of the implementation of the full sphere Worland I3D2pDiags
 * sparse operator
 *
 ...
 * no correctQ1/zeroLast (TRUNCATE_QI=OFF).
 */
...
// Project includes
//
#include "QuICC/SparseSM/Worland/Chebyshev/I3D2pDiags.hpp"
...
I3D2pDiags::I3D2pDiags(const Scalar_t alpha, const int l, const int q) :
    QuICC::SparseSM::Worland::I3D2pDiags(alpha, MHD_MP(-0.5), l, q)
{
   if (q > 0)
   {
      throw std::logic_error(
         "I3D2p: truncation (q>0) not supported - build with TRUNCATE_QI=OFF");
   }
}

I3D2pDiags::ACoeff_t I3D2pDiags::d_3(const ACoeff_t& n) const
{
   ACoeff_t val;
   val = 8.0_mp * (l<1>() + n - 1.0_mp) * (2.0_mp * l<1>() + 2.0_mp * n - 3.0_mp) *
...
I3D2pDiags::ACoeff_t I3D2pDiags::d_2(const ACoeff_t& n) const
{
   ACoeff_t val;
   val = -8.0_mp * (2.0_mp * l<1>() + 2.0_mp * n - 1.0_mp) *
         (2.0_mp * l<2>() - 2.0_mp * l<1>() * n + 3.0_mp * l<1>() -
            4.0_mp * n * n + 4.0_mp * n + 3.0_mp) /
...
I3D2pDiags::ACoeff_t I3D2pDiags::d_1(const ACoeff_t& n) const
{
   ACoeff_t val;
   val = -4.0_mp * (2.0_mp * l<1>() + 2.0_mp * n + 1.0_mp) *
...
I3D2pDiags::ACoeff_t I3D2pDiags::d0(const ACoeff_t& n) const
{
   ACoeff_t val;
   val = -2.0_mp * (2.0_mp * n + 1.0_mp) * (2.0_mp * l<1>() + 2.0_mp * n + 1.0_mp) *
         (2.0_mp * l<1>() + 2.0_mp * n + 3.0_mp) *
         (6.0_mp * l<1>() * n + l<1>() + 4.0_mp * n * n + 4.0_mp * n - 3.0_mp) /
...
I3D2pDiags::ACoeff_t I3D2pDiags::d1(const ACoeff_t& n) const
{
   ACoeff_t val;
