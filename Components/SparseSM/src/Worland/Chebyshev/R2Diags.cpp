/**
 * @file R2Diags.cpp
 * @brief Source of the implementation of the full sphere Worland R2Diags sparse
 * operator
 */
...
R2Diags::ACoeff_t R2Diags::d_1(const ACoeff_t& n) const
{
   ...
   {
      val = n * (l1 + n - MHD_MP(1.0)) /
            ((l1 + MHD_MP(2.0) * n - MHD_MP(2.0)) *
               (l1 + MHD_MP(2.0) * n - MHD_MP(1.0)));
   }
...
R2Diags::ACoeff_t R2Diags::d0(const ACoeff_t& n) const
{
   ...
   {
      auto l2 = l1 * l1;
      val = (MHD_MP(2.0) * l2 + MHD_MP(4.0) * l1 * n - l1 +
               MHD_MP(4.0) * n.pow(2) - MHD_MP(1.0)) /
            (MHD_MP(2.0) * (l1 + MHD_MP(2.0) * n - MHD_MP(1.0)) *
               (l1 + MHD_MP(2.0) * n + MHD_MP(1.0)));
   }
   ...
   {
      val.head(1) =
         (MHD_MP(2.0) * l1 + MHD_MP(1.0)) / (MHD_MP(2.0) * (l1 + MHD_MP(1.0)));
   }
...
R2Diags::ACoeff_t R2Diags::d1(const ACoeff_t& n) const
{
   ...
   val = (MHD_MP(2.0) * n + MHD_MP(1.0)) *
         (MHD_MP(2.0) * l1 + MHD_MP(2.0) * n + MHD_MP(1.0)) /
         (MHD_MP(4.0) * (l1 + MHD_MP(2.0) * n + MHD_MP(1.0)) *
            (l1 + MHD_MP(2.0) * n + MHD_MP(2.0)));
