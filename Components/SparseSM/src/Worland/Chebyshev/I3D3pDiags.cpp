/**
 * @file I3D3pDiags.cpp
 * @brief Source of the implementation of the full sphere Worland I3D3pDiags
 * sparse operator
 *
...
I3D3pDiags::I3D3pDiags(const Scalar_t alpha, const int l, const int q) :
    QuICC::SparseSM::Worland::I3D3pDiags(alpha, MHD_MP(-0.5), l, q)
{
   if (q > 0)
   {
      throw std::logic_error(
         "I3D3p: truncation (q>0) not supported - build with TRUNCATE_QI=OFF");
   }
}

I3D3pDiags::ACoeff_t I3D3pDiags::d_3(const ACoeff_t& n) const
{
   ACoeff_t val;
...
I3D3pDiags::ACoeff_t I3D3pDiags::d_2(const ACoeff_t& n) const
{
   ACoeff_t val;
   val = 12.0_mp * (2.0_mp * n - 1.0_mp) *
         (2.0_mp * l<1>() + 2.0_mp * n - 1.0_mp) *
         (2.0_mp * l<1>() + 2.0_mp * n + 1.0_mp) *
         (2.0_mp * l<1>() + 2.0_mp * n + 1.0_mp) /
...
I3D3pDiags::ACoeff_t I3D3pDiags::d_1(const ACoeff_t& n) const
{
   ACoeff_t val;
   val =
      6.0_mp * (2.0_mp * n - 1.0_mp) * (2.0_mp * n + 1.0_mp) *
      (2.0_mp * l<1>() + 2.0_mp * n + 1.0_mp) *
      (2.0_mp * l<1>() + 2.0_mp * n + 1.0_mp) *
...
I3D3pDiags::ACoeff_t I3D3pDiags::d0(const ACoeff_t& n) const
{
   ACoeff_t val;
