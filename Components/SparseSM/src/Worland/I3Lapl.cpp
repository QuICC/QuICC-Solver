/**
 * @file I3Lapl.cpp
 * @brief Source of the implementation of the full sphere Worland I3Lapl sparse operator
 */
...
// Project includes
//
#include "QuICC/SparseSM/Worland/I3Lapl.hpp"
#include "QuICC/SparseSM/Worland/Chebyshev/I3LaplDiags.hpp"
//#include "QuICC/SparseSM/Worland/Legendre/I3LaplDiags.hpp"
//#include "QuICC/SparseSM/Worland/CylEnergy/I3LaplDiags.hpp"
...
namespace Worland {
   ...
   I3Lapl::I3Lapl(const int rows, const int cols, const Scalar_t alpha, const Scalar_t dBeta, const int l, const int q)
      : IWorlandOperator(rows, cols, alpha, dBeta)
   {
      switch(this->type())
      {
         case WorlandKind::CHEBYSHEV:
            this->mpImpl = std::make_shared<Chebyshev::I3LaplDiags>(alpha, l, q);
            break;
         case WorlandKind::LEGENDRE:
            //this->mpImpl = std::make_shared<Legendre::I3LaplDiags>(alpha, l);
            throw std::logic_error("Not yet implemented");
            break;
         case WorlandKind::CYLENERGY:
            //this->mpImpl = std::make_shared<CylEnergy::I3LaplDiags>(alpha, l);
            throw std::logic_error("Not yet implemented");
            break;
         case WorlandKind::SPHENERGY:
            this->mpImpl = std::make_shared<SphEnergy::I3LaplDiags>(alpha, l, q);
            break;
      }
   }

   void I3Lapl::buildTriplets(TripletList_t& list) const
   {
      const int dShift = 2;
   ...
   void I3Lapl::buildBanded(Internal::Matrix& bd, unsigned int& kL, unsigned int &kU) const
   {
      kL = 1;
      kU = 7;
