 ...
 * @brief Source of the implementation of boundary value of r D2 - r D1(Log(rho))D1
 */
...
// Project includes
//
#include "QuICC/SparseSM/Worland/Boundary/StressFreePolAnelastic.hpp"
...
StressFreePolAnelastic::StressFreePolAnelastic(const Scalar_t alpha, const Scalar_t dBeta, const int l, const MHDFloat Fb) :
    ICondition(alpha, dBeta, l, 0),
    mBCk0(alpha, dBeta, l),
    mBCk1(alpha, dBeta, l + 1),
    mBCk2(alpha, dBeta, l + 2),
    mD1(alpha,dBeta,l),
    mFb(Fb)
{}
...
StressFreePolAnelastic::ACoeff_t StressFreePolAnelastic::compute(const int maxN)
{
   ...
   if (this->l() > 0)
   {
      if (maxN > 0)
      {
         ...
         val.bottomRows(maxN) +=
            MHD_MP(2.0) * (ab1 + n) * (MHD_MP(1.0) + d2l) * bcVal;
      }
   }
   else
   {
   ...
   //anelastic bit
   auto bcD1 = this->mD1.compute(maxN);

   n = (ACoeffI::LinSpaced(maxN + 1, 0, maxN)).cast<Scalar_t>();

   return this->invnorm(n) * val - this->mFb * bcD1;
}
