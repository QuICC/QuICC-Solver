...
InsulatingSphere::InsulatingSphere(const int rows, const int cols,
    ...
    IWorlandOperator(rows, cols, alpha, dBeta)
{
   ...
   case WorlandKind::CHEBYSHEV:
      this->mpImpl =
         std::make_shared<Chebyshev::InsulatingSphereDiags>(alpha, l);
      break;
   case WorlandKind::LEGENDRE:
      // this->mpImpl = std::make_shared<Legendre::InsulatingSphereDiags>(alpha,
      // l);
      throw std::logic_error("Not yet implemented");
      break;
   case WorlandKind::CYLENERGY:
      // this->mpImpl =
      // std::make_shared<CylEnergy::InsulatingSphereDiags>(alpha, l);
      throw std::logic_error("Not yet implemented");
      break;
   case WorlandKind::SPHENERGY:
      this->mpImpl =
         std::make_shared<SphEnergy::InsulatingSphereDiags>(alpha, l);
      break;
   }
}
