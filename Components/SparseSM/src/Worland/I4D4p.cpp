/**
 * @file I4D4p.cpp
 * @brief Source of the implementation of the full sphere Worland I4D4p sparse
 * operator
 */
...
#include "QuICC/SparseSM/Worland/I4D4p.hpp"
...
I4D4p::I4D4p(const int rows, const int cols, const Scalar_t alpha,
    ...
    IWorlandOperator(rows, cols, alpha, dBeta)
{
   ...
   case WorlandKind::CHEBYSHEV:
      this->mpImpl = std::make_shared<Chebyshev::I4D4pDiags>(alpha, l, q);
      break;
...
void I4D4p::buildTriplets(TripletList_t& list) const
{
   const int dShift = 2;
...
void I4D4p::buildBanded(Internal::Matrix& bd, unsigned int& kL,
