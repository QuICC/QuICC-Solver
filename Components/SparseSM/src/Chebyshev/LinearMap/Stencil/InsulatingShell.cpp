...
InsulatingShell::InsulatingShell(const int rows, const int cols,
   const Scalar_t lower, const Scalar_t upper, const Scalar_t l) :
...
InsulatingShell::ACoeff_t InsulatingShell::d_2(const ACoeff_t& n) const
{
   ...
   const auto val_num =
      a2 * (2.0 * l1 * (l1 + 1.0) -
              2.0 * (n - 3.0) * n * ((n - 3.0) * n + 5.0) - 13.0) +
      a1 * b1 * (2.0 * l1 + 1.0) * (2.0 * (n - 3.0) * n + 5.0) +
      2.0 * b2 * (n.pow(2) - 3.0 * n + 2.0).pow(2);
   const auto val_den =
      a2 * (2.0 * l1 * (l1 + 1.0) -
              2.0 * (n - 1.0) * n * ((n - 1.0) * n + 1.0) - 1.0) +
      a1 * b1 * (2.0 * l1 + 1.0) * (2.0 * (n - 1.0) * n + 1.0) +
      2.0 * b2 * (n - 1.0).pow(2) * n.pow(2);
   ACoeff_t val = -val_num / val_den;

   const auto corr_num =
      a1 * (a1 * (2.0 * l2 + 2.0 * l1 - 1.0) + 2.0 * b1 * l1 + b1);
   const auto corr_den = 2.0 * (a2 * (2.0 * l2 + 2.0 * l1 - 13.0) +
                                  5.0 * a1 * (2.0 * b1 * l1 + b1) + 8.0 * b2);
...
InsulatingShell::ACoeff_t InsulatingShell::d_1(const ACoeff_t& n) const
{
   ...
   const auto val_den =
      a2 * (2.0 * l1 * (l1 + 1.0) - 2.0 * n * (n + 1.0) * (n.pow(2) + n + 1.0) -
              1.0) +
      a1 * b1 * (2.0 * l1 + 1.0) * (2.0 * n * (n + 1.0) + 1.0) +
      2.0 * b2 * n.pow(2) * (n + 1.0).pow(2);
   ACoeff_t val = val_num / val_den;

   const auto corr_num = 2.0 * a1 * (2.0 * a1 * l1 + a1 - b1);
   const auto corr_den = a2 * (2.0 * l2 + 2.0 * l1 - 13.0) +
                         5.0 * a1 * (2.0 * b1 * l1 + b1) + 8.0 * b2;
   val(0) = corr_num / corr_den;
