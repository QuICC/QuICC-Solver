/**
 * @file I4Y1.cpp
 * @brief Source of the implementation of the I^4 Y sparse operator, with y = ax
 * + b
 */
...
I4Y1::I4Y1(const int rows, const int cols, const Scalar_t lower,
...
{
   return -Internal::Math::pow(this->a(), 4) * this->b() /
          (4.0 * n * (n - 3.0) * (n - 1.0) * (n + 1.0));
}

I4Y1::ACoeff_t I4Y1::d_1(const ACoeff_t& n) const
{
   return 2.0 * Internal::Math::pow(this->a() / 2.0, 5) * (n - 8.0) /
          (n * (n - 3.0) * (n - 2.0) * (n + 1.0) * (n + 2.0));
}

I4Y1::ACoeff_t I4Y1::d0(const ACoeff_t& n) const
{
   return 6.0 * Internal::Math::pow(this->a() / 2.0, 4) * this->b() /
          ((n - 2.0) * (n - 1.0) * (n + 1.0) * (n + 2.0));
}

I4Y1::ACoeff_t I4Y1::d1(const ACoeff_t& n) const
{
   return 2.0 * Internal::Math::pow(this->a() / 2.0, 5) * (n + 8.0) /
          (n * (n - 2.0) * (n - 1.0) * (n + 2.0) * (n + 3.0));
}

I4Y1::ACoeff_t I4Y1::d2(const ACoeff_t& n) const
{
   return -4.0 * Internal::Math::pow(this->a() / 2.0, 4) * this->b() /
          (n * (n - 1.0) * (n + 1.0) * (n + 3.0));
}
