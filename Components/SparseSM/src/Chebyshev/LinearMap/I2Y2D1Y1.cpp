...
I2Y2D1Y1::I2Y2D1Y1(const int rows, const int cols, const Scalar_t lower,
...
{
   return Internal::Math::pow(this->a(), 3) * (this->b()) * (3 * n - 7.0) /
          (8 * n * (n - 1.0));
}
...
I2Y2D1Y1::ACoeff_t I2Y2D1Y1::d_2(const ACoeff_t& n) const
{
   ...
   return a2 * (a2 * n * n - 3 * a2 + 6 * b2 * n * n - 4 * b2 * n - 10 * b2) /
          (8 * n * (n - 1) * (n + 1));
}
...
I2Y2D1Y1::ACoeff_t I2Y2D1Y1::d_1(const ACoeff_t& n) const
{
   ...
   return (this->a()) * (this->b()) *
          (3 * a2 * n + 7 * a2 + 4 * b2 * n + 4 * b2) / (8 * n * (n + 1));
}
