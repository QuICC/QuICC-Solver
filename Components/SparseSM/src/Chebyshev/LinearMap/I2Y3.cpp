/**
 * @file I2Y3.cpp
 * @brief Source of the implementation of the I^2 Y^3 sparse operator, with y =
 * ax + b
 */
...
I2Y3::I2Y3(const int rows, const int cols, const Scalar_t lower,
...
I2Y3::ACoeff_t I2Y3::d_3(const ACoeff_t& n) const
{
   ...
   return c * (a2 * (n + 3_mp) + 12_mp * b2 * (n + 1_mp)) /
          (4_mp * n * (n - 1_mp) * (n + 1_mp));
}
...
I2Y3::ACoeff_t I2Y3::d_2(const ACoeff_t& n) const
{
   ...
   return c * b1 * (3_mp * a2 + 2_mp * b2 * (n + 1_mp)) /
          (2_mp * n * (n - 1_mp) * (n + 1_mp));
}
...
I2Y3::ACoeff_t I2Y3::d2(const ACoeff_t& n) const
{
   ...
   const auto a2 = this->a() * this->a();
   const auto b2 = this->b() * this->b();
   return -a2 * b1 * (3_mp * a2 - 2_mp * b2 * (n - 1_mp)) /
          (8_mp * n * (n - 1_mp) * (n + 1_mp));
}
