 ...
 * @brief Source of the implementation of the I^2 Y^4 spherical laplacian sparse
...
I2Y4SphLapl::I2Y4SphLapl(const int rows, const int cols, const Scalar_t lower,
...
I2Y4SphLapl::ACoeff_t I2Y4SphLapl::d_3(const ACoeff_t& n) const
{
   ...
   const auto a2 = a1 * a1;
   const auto a3 = a1 * a2;
   return -(a3 * b1 * (l2 + l1 - 2.0 * n * n + 11.0 * n - 15.0)) /
          (4.0 * n * (n - 1.0));
}
...
I2Y4SphLapl::ACoeff_t I2Y4SphLapl::d_2(const ACoeff_t& n) const
{
   ...
   const auto n3 = n2 * n;
   return -a2 *
          (a2 * l2 + a2 * l1 - 2 * a2 * n3 + 6 * a2 * n2 + 2 * a2 * n -
             12 * a2 + 2 * b2 * l2 * n + 2 * b2 * l2 + 2 * b2 * l1 * n +
             2 * b2 * l1 - 12 * b2 * n3 + 36 * b2 * n2 - 48 * b2) /
          (8 * n * (n - 1) * (n + 1));
}
...
I2Y4SphLapl::ACoeff_t I2Y4SphLapl::d_1(const ACoeff_t& n) const
{
   ...
   const auto n2 = n * n;
   return a1 * b1 *
          (a2 * l2 + a2 * l1 + 6 * a2 * n2 - 3 * a2 * n - 15 * a2 +
             8 * b2 * n2 - 4 * b2 * n - 12 * b2) /
          (4 * n * (n + 1));
}
...
I2Y4SphLapl::ACoeff_t I2Y4SphLapl::d0(const ACoeff_t& n) const
{
   ...
   const auto b4 = b2 * b2;
   const auto n2 = n * n;
   return (a4 * l2 + a4 * l1 + 3 * a4 * n2 - 9 * a4 + 4 * a2 * b2 * l2 +
             4 * a2 * b2 * l1 + 24 * a2 * b2 * n2 - 48 * a2 * b2 + 8 * b4 * n2 -
             8 * b4) /
          (8 * (n - 1) * (n + 1));
}
