 ...
 * @brief Source of the implementation of the I^2 Y^3 spherical laplacian sparse
...
I2Y3SphLapl::I2Y3SphLapl(const int rows, const int cols, const Scalar_t lower,
...
I2Y3SphLapl::ACoeff_t I2Y3SphLapl::d_2(const ACoeff_t& n) const
{
   ...
   return -c * b1 * (l2 + l1 - 3.0 * n.pow(2) + 11.0 * n - 10.0) /
          (n * (n - 1.0));
}
...
I2Y3SphLapl::ACoeff_t I2Y3SphLapl::d_1(const ACoeff_t& n) const
{
   ...
   return a1 *
          (a2 * l2 + a2 * l1 + 3.0 * a2 * n.pow(2) - a2 * n - 6.0 * a2 +
             12.0 * b2 * n.pow(2) - 4.0 * b2 * n - 16.0 * b2) /
          (8.0 * n * (n + 1.0));
}
...
I2Y3SphLapl::ACoeff_t I2Y3SphLapl::d0(const ACoeff_t& n) const
{
   ...
   const auto b2 = b1 * b1;
   return b1 *
          (a2 * l2 + a2 * l1 + 3.0 * a2 * n.pow(2) - 5.0 * a2 +
             2.0 * b2 * n.pow(2) - 2.0 * b2) /
          (2.0 * (n - 1.0) * (n + 1.0));
}
