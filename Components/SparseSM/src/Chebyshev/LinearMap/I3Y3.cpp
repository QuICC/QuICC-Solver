/**
 * @file I3Y3.cpp
 * @brief Source of the implementation of the I^3 Y^3 sparse operator, with y =
 * ax + b
 */
...
I3Y3::I3Y3(const int rows, const int cols, const Scalar_t lower,
...
{
   return -a<3>() * b<1>() *
          (3.0_mp * a<2>() * n - 15.0_mp * a<2>() - 4.0_mp * b<2>() * n -
             4.0_mp * b<2>()) /
          (32.0_mp * n * (n - 2.0_mp) * (n - 1.0_mp) * (n + 1.0_mp));
}
...
{
   return -3.0_mp * a<3>() * b<1>() *
          (a<2>() * n + 4.0_mp * a<2>() + 2.0_mp * b<2>() * n +
             4.0_mp * b<2>()) /
...
{
   return -3.0_mp * a<4>() * (a<2>() + 6.0_mp * b<2>()) /
