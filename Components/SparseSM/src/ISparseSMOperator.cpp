 ...
 * @brief Source of the implementation of generic interface to the sparse
 * operator
 */
...
void ISparseSMOperator::buildOpImpl(Internal::SparseMatrix& mat, const int r,
   const int c) const
{
   // Build triplets
   TripletList_t list;
...
void ISparseSMOperator::buildOpImpl(Internal::Matrix& bd, const int r,
   const int c) const
{
   unsigned int kL, kU;
   this->buildBanded(bd, kL, kU);
...
void ISparseSMOperator::buildBanded(Internal::Matrix& bd, unsigned int& kL,
   unsigned int& kU) const
{
   throw std::logic_error("Banded operator not yet implemented!");
}

void ISparseSMOperator::convertToTriplets(TripletList_t& list, const int d,
   const ACoeffI& rowIdx, const ACoeff_t& diag) const
{
...
void ISparseSMOperator::leftOutOfMatrix(TripletList_t&, const int, const int,
   const Scalar_t) const
{
   // Ignore entry
}

void ISparseSMOperator::rightOutOfMatrix(TripletList_t&, const int, const int,
   const Scalar_t) const
{
   // Ignore entry
}
