...
namespace Bessel {
   ...
   template <typename TOp> Tester<TOp>::Tester(const std::string& fname, const std::string& btype, const bool keepData)
   ...
   template <typename TOp> void Tester<TOp>::buildOperator(SparseMatrix& mat, const ParameterType& param, const TestType type) const
   {
      ...
      if constexpr(std::is_base_of_v<QuICC::SparseSM::IBesselOperator, TOp>)
      {
         ...
         auto btype = static_cast<QuICC::SparseSM::Bessel::BesselKind>(static_cast<int>(meta(2)));
         auto l = meta(3);
