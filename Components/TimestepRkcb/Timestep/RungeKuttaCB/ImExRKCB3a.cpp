...
// Project includes
//
#include "Timestep/RungeKuttaCB/ImExRKCB3a.hpp"
...
{
   MHDFloat c2 = (27.0 + std::pow(2187.0 - 1458.0 * std::sqrt(2.0), 1. / 3.) +
                    9.0 * std::pow(3.0 + 2.0 * std::sqrt(2.0), 1. / 3.)) /
                 54.0;
   MHDFloat c3 = c2 / (6.0 * std::pow(c2, 2) - 3.0 * c2 + 1.0);
   MHDFloat b2 = (3.0 * c2 - 1.0) / (6.0 * std::pow(c2, 2));
   MHDFloat b3 =
      (6.0 * std::pow(c2, 2) - 3.0 * c2 + 1.0) / (6.0 * std::pow(c2, 2));

   // Initialize implicit a factors
   ImExRKCB3a::mAIm(1, 1) = c2;
   ImExRKCB3a::mAIm(2, 2) =
      (1. / 6. - b2 * std::pow(c2, 2) - b3 * c2 * c3) / (b3 * (c3 - c2));
