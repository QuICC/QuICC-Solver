...
#ifndef QUICC_TIMESTEP_RUNGEKUTTACB_SPARSEIMEXRK2RTIMESTEPPER_HPP
#define QUICC_TIMESTEP_RUNGEKUTTACB_SPARSEIMEXRK2RTIMESTEPPER_HPP
...
class SparseImExRK2RTimestepper
...
template <typename TOperator, typename TData, template <typename> class TSolver>
SparseImExRK2RTimestepper<TOperator, TData, TSolver>::SparseImExRK2RTimestepper(
   const int start, const std::size_t timeId) :
    ISparseImExRKnRTimestepper<TOperator, TData, TSolver>(start, timeId)
{}

template <typename TOperator, typename TData, template <typename> class TSolver>
bool SparseImExRK2RTimestepper<TOperator, TData, TSolver>::preSolve()
{
   ...
   else
   {
      ...
      {
         // Build RHS for implicit term
         MHDFloat aIm = (this->mspScheme->aIm(this->mStep, this->mStep - 1) -
                           this->mspScheme->bIm(this->mStep - 1)) *
                        this->mDt;
         MHDFloat aEx = -(this->mspScheme->aEx(this->mStep, this->mStep - 1) -
                           this->mspScheme->bEx(this->mStep - 1)) *
                        this->mDt;
         for (size_t i = this->mZeroIdx; i < this->nSystem(); i++)
         {
            details::computeAMXPYPBZ(this->reg(Register::Explicit::id()).at(i),
...
template <typename TOperator, typename TData, template <typename> class TSolver>
bool SparseImExRK2RTimestepper<TOperator, TData, TSolver>::postSolve()
{
...
#endif // QUICC_TIMESTEP_RUNGEKUTTACB_SPARSEIMEXRK2RTIMESTEPPER_HPP
