/**
 * @file ViewDense.hpp
 * @brief A view is a n-dimensional dense or sparse tensor reference to a block
 * of memory. It is not the owner of the memory resource.
 */
...
/// @tparam ...Args
template <class Scalar, class... Args> class ViewDenseBase;
...
{
   static_assert(hasLevel<Attributes<Args...>>::value,
      "Attributes has no level");
   static_assert(hasOrder<Attributes<Args...>>::value,
      "Attributes has no order");
...
public:
   ...
   /// @brief typedef for aggregated compression level types, i.e. a
   /// std::variant of dense_t, compressed_t, etc
   using LevelType = typename Attributes<Args...>::level;
   /// @brief typedef for aggregated logical index types, i.e. a std::variant of
   /// i_t, j_t, etc
...
protected:
   ...
   /// @tparam Scalar
   /// @tparam ...Args
   static_assert(_rank == std::variant_size_v<OrderType>,
...
public:
   ...
   /// @brief get view rank
   /// @return _rank
   QUICC_CUDA_HOSTDEV static constexpr std::size_t rank()
   {
      return _rank;
   }
   ...
   QUICC_CUDA_HOSTDEV const IndexType* dims() const
   {
      return _dimensions;
   }
   ...
   QUICC_CUDA_HOSTDEV const IndexType* strides() const
   {
      return _strides;
   }
};
...
/// @tparam ...Args attributes
template <class Scalar, class... Args>
class View<Scalar, Attributes<dense1D_t, Args...>>
    : public ViewDenseBase<Scalar, Attributes<dense1D_t, Args...>>
{
public:
   /// @brief default ctor for empty view
   View() = default;
   ...
   using
      typename ViewDenseBase<Scalar, Attributes<dense1D_t, Args...>>::IndexType;
   ...
   /// @param dimensions std::array of logical dimensions, useful to accept
...
/// @tparam ...Args attributes
template <class Scalar, class... Args>
class View<Scalar, Attributes<dense2D_t, Args...>>
    : public ViewDenseBase<Scalar, Attributes<dense2D_t, Args...>>
{
public:
   /// @brief default ctor for empty view
   View() = default;
   ...
   /// @tparam ...Args
   using
      typename ViewDenseBase<Scalar, Attributes<dense2D_t, Args...>>::IndexType;

   /// @brief imported from ViewDenseBase
   /// @tparam Scalar
   /// @tparam ...Args
   using
      typename ViewDenseBase<Scalar, Attributes<dense2D_t, Args...>>::OrderType;
   ...
   View(const span<Scalar>& data, const std::array<IndexType, _rank> dimensions,
      const std::array<IndexType, _rank> strides = std::array<IndexType, _rank>{
         0, 0});
   ...
   /// @param dimensions pointer to array of logical dimensions
   View(const span<Scalar>& data, const IndexType* dimensions,
      const IndexType* strides = nullptr);
   ...
   /// @param j second logical index
   /// @return element with const attribute
   QUICC_CUDA_HOSTDEV const Scalar& operator()(const IndexType i,
...
/// View 3D dense
template <class Scalar, class... Args>
class View<Scalar, Attributes<dense3D_t, Args...>>
    : public ViewDenseBase<Scalar, Attributes<dense3D_t, Args...>>
{
public:
   /// @brief default ctor for empty view
   View() = default;
   ...
   /// @tparam ...Args
   using
      typename ViewDenseBase<Scalar, Attributes<dense3D_t, Args...>>::IndexType;

   /// @brief imported from ViewDenseBase
   /// @tparam Scalar
   /// @tparam ...Args
   using
      typename ViewDenseBase<Scalar, Attributes<dense3D_t, Args...>>::OrderType;
   ...
   /// @return element with const attribute
   QUICC_CUDA_HOSTDEV const Scalar& operator()(const IndexType i,
      const IndexType j, const IndexType k) const;
};


// 1D dense_t ctor
template <class Scalar, class... Args>
View<Scalar, Attributes<dense1D_t, Args...>>::View(const span<Scalar>& data,
   const std::array<IndexType, _rank> dimensions) :
    ViewDenseBase<Scalar, Attributes<dense1D_t, Args...>>(data.data(),
       data.size())
{
   this->_strides[0] = 1;
...
// 1D accessor
template <class Scalar, class... Args>
const Scalar& View<Scalar, Attributes<dense1D_t, Args...>>::operator()(
   const IndexType i) const
{
   // static_assert(_rank == 1, "This method is available only to rank 1
   // Views");
...
template <class Scalar, class... Args>
Scalar& View<Scalar, Attributes<dense1D_t, Args...>>::operator()(
...
// 2D dense_t ctor
template <class Scalar, class... Args>
View<Scalar, Attributes<dense2D_t, Args...>>::View(const span<Scalar>& data,
   const std::array<IndexType, _rank> dimensions,
   const std::array<IndexType, _rank> strides) :
    ViewDenseBase<Scalar, Attributes<dense2D_t, Args...>>(data.data(),
       data.size())
{
   for (std::size_t i = 0; i < _rank; ++i)
   {
...
template <class Scalar, class... Args>
View<Scalar, Attributes<dense2D_t, Args...>>::View(const span<Scalar>& data,
   const IndexType* dimensions, const IndexType* strides) :
    ViewDenseBase<Scalar, Attributes<dense2D_t, Args...>>(data.data(),
       data.size())
{
...
// 2D accessor
template <class Scalar, class... Args>
const Scalar& View<Scalar, Attributes<dense2D_t, Args...>>::operator()(
   const IndexType i, const IndexType j) const
{
...
template <class Scalar, class... Args>
Scalar& View<Scalar, Attributes<dense2D_t, Args...>>::operator()(
...
// 3D dense_t ctor
template <class Scalar, class... Args>
View<Scalar, Attributes<dense3D_t, Args...>>::View(const span<Scalar>& data,
   const std::array<IndexType, _rank> dimensions,
   const std::array<IndexType, _rank> strides) :
    ViewDenseBase<Scalar, Attributes<dense3D_t, Args...>>(data.data(),
       data.size())
{
   for (std::size_t i = 0; i < _rank; ++i)
   {
