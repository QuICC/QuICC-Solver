...
template <class T> struct isLevelType : std::false_type
{
};

/// @brief enable dense_t tag
template <> struct isLevelType<dense_t> : std::true_type
{
};

/// @brief enable compressed_t tag
template <> struct isLevelType<compressed_t> : std::true_type
{
};

/// @brief enable sparse_t tag
template <> struct isLevelType<sparse_t> : std::true_type
{
};

/// @brief enable step1K_t tag
template <> struct isLevelType<step1K_t> : std::true_type
{
};

/// @brief enable step2K_t tag
template <> struct isLevelType<step2K_t> : std::true_type
{
};

/// @brief helper
/// @tparam T level tag
template <class T> inline constexpr bool isLevelType_v = isLevelType<T>::value;

/// @brief check all levels
/// @tparam ...Ts
template <class... Ts> struct areLevelType
{
...
/// @tparam T
template <class T> struct isLevelTypeDense : std::false_type
{
...
template <> struct isLevelTypeDense<dense_t> : std::true_type
{
};
...
/// @tparam ...Ts
template <class... Ts> struct areLevelTypeDense
{
...
/// @tparam ...Ts
template <class... Ts> struct howManyLevelTypeDense
{
   static constexpr std::uint16_t value{
      (static_cast<uint16_t>(isLevelTypeDense_v<Ts>) + ...)};
};

/// @brief helper
/// @tparam T
template <class... Ts>
inline constexpr std::uint16_t howManyLevelTypeDense_v =
   howManyLevelTypeDense<Ts...>::value;

namespace details {
/// @brief check with human readable error message
/// @tparam ...T
/// @return
template <class... T> constexpr bool assertLevel()
{
   static_assert(areLevelType<T...>::value,
      "unknown level type, must be either dense_t, compressed_t, triDense_t, "
      "triCompressed_t or CSC_t");
   return true;
}
} // namespace details
...
using DimLevelType =
   std::enable_if_t<details::assertLevel<Types...>(), std::variant<Types...>>;

/// @brief generic template to check if aggregate level types is the correct
/// type
/// @tparam Tvar
template <class T> struct isDimLevelType : std::false_type
{
};

/// @brief specialized template to check if aggregate level types is the correct
/// type
/// @tparam ...Ts
template <class... Ts>
...
/// @brief generic template to check if aggregate level types (aka DimLevelType)
/// are all dense
/// @tparam Tvar
template <class T> struct isDimLevelTypeDense : std::false_type
{
   static_assert(isDimLevelType_v<T>, "Type not a DimLevel.");
};

/// @brief specialized template to check if aggregate level types (aka
/// DimLevelType) are all dense_t
/// @tparam ...Ts
template <class... Ts>
struct isDimLevelTypeDense<std::variant<Ts...>>
    : public areLevelTypeDense<Ts...>
{
};
...
/// are all dense
/// @tparam Tvar
template <class T> struct howManyDimLevelTypeDense
{
   static_assert(isDimLevelType_v<T>, "Type not a DimLevel.");
...
/// @tparam ...Ts
template <class... Ts>
struct howManyDimLevelTypeDense<std::variant<Ts...>>
    : public howManyLevelTypeDense<Ts...>
{
};
...
/// @tparam T
template <class T>
inline constexpr std::uint16_t howManyDimLevelTypeDense_v =
   howManyDimLevelTypeDense<T>::value;
...
/// @tparam T
template <class T> struct isLoopType : std::false_type
{
};

/// @brief enable i_t
template <> struct isLoopType<i_t> : std::true_type
{
};

/// @brief enable iInOrder_t
template <> struct isLoopType<iInOrder_t> : std::true_type
{
};

/// @brief enable j_t
template <> struct isLoopType<j_t> : std::true_type
{
};

/// @brief enable k_t
template <> struct isLoopType<k_t> : std::true_type
{
};

/// @brief enable l_t
template <> struct isLoopType<l_t> : std::true_type
{
};

/// @brief helper
/// @tparam T
template <class T> inline constexpr bool isLoopType_v = isLoopType<T>::value;

/// @brief check that all tags are valid loop tags
/// @tparam ...T
template <class... T> struct areLoopType
{
...
/// @brief helper
/// @tparam T
template <class T> inline constexpr bool areLoopType_v = areLoopType<T>::value;

namespace details {
/// @brief check with human readable error message
/// @tparam ...T
/// @return
template <class... T> constexpr bool assertOrder()
{
   // add unique tag check
   static_assert(areLoopType<T...>::value,
      "unknown level type, must be i_t, j_t, k_t or l_t");
   return true;
}
} // namespace details
...
template <class... Types>
using LoopOrderType =
   std::enable_if_t<details::assertOrder<Types...>(), std::variant<Types...>>;


/// @brief generic template for default loop order, i.e. column major (leftmost
/// index has stride 1)
/// @tparam I
template <std::uint32_t I> struct DefaultLoopOrder;

/// @brief specialized 1D default loop order
template <> struct DefaultLoopOrder<1>
{
   using type = LoopOrderType<i_t>;
};

/// @brief specialized 2D default loop order
template <> struct DefaultLoopOrder<2>
{
   using type = LoopOrderType<i_t, j_t>;
};

/// @brief specialized 3D default loop order
template <> struct DefaultLoopOrder<3>
{
   using type = LoopOrderType<i_t, j_t, k_t>;
};

/// @brief specialized 4D default loop order
template <> struct DefaultLoopOrder<4>
{
 ...
 * @tparam ...
 */
template <class...> struct Attributes
{
};

/** @brief attributes wrapper to store Level and Order
 *  \todo SFINAE std::enable_if_t<areLevelType_v<Level> && areLoopType_v<Order>>
 * = true
 *  @tparam Level
 *  @tparam Order
 *  @tparam ...Rest
 ...
 *  @tparam ...Rest
 */
template <class Level, class... Rest> struct Attributes<Level, Rest...>
{
   /// @brief store aggregate level tags
   using level = Level;
