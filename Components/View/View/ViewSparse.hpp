...
public:
   ...
   /// @brief typedef for aggregated compression level types, i.e. a std::varint
   /// of dense_t, compressed_t, etc
   using LevelType = typename Attributes<Args...>::level;
   /// @brief typedef for aggregated logical index types, i.e. a std::varint of
   /// i_t, j_t, etc
...
public:
   ...
   /// @param indices
   /// @param lds leading dimension size
   View(const Scalar* data, const std::size_t size, const IndexType* dimensions,
      const ViewBase<IndexType>* pointers, const ViewBase<IndexType>* indices,
      const IndexType lds = 0);
   ...
   /// @return _lds value
   QUICC_CUDA_HOSTDEV const IndexType lds() const
   {
      return _lds;
   }
   ...
   QUICC_CUDA_HOSTDEV const ViewBase<IndexType>* pointers() const
   {
      return _pointers;
   };
   ...
   QUICC_CUDA_HOSTDEV const ViewBase<IndexType>* indices() const
   {
      return _indices;
   };
   ...
   /// @param k third logical index
   /// @return element with const qualifier
   const ScalarType& operator()(const IndexType i, const IndexType j,
    ...
    ViewBase<Scalar>(data.data(), data.size())
{
   ...
   _lds = 0;
   if constexpr (std::is_same_v<i_t,
                    std::variant_alternative_t<0, OrderType>> ||
                 std::is_same_v<iInOrder_t,
                    std::variant_alternative_t<0, OrderType>>)
   {
      if constexpr (std::is_same_v<dense_t,
                       std::variant_alternative_t<0, LevelType>>)
      {
         if (lds == 0)
         {
            _lds = _dimensions[0];
   ...
   else if constexpr (std::is_same_v<j_t,
                         std::variant_alternative_t<0, OrderType>>)
   {
      if constexpr (std::is_same_v<dense_t,
                       std::variant_alternative_t<1, LevelType>>)
      {
         if (lds == 0)
         {
            _lds = _dimensions[1];
   ...
   else if constexpr (std::is_same_v<k_t,
                         std::variant_alternative_t<0, OrderType>>)
   {
      if constexpr (std::is_same_v<dense_t,
                       std::variant_alternative_t<2, LevelType>>)
      {
         if (lds == 0)
         {
            _lds = _dimensions[2];
...
View<Scalar, Attributes<Args...>>::View(const Scalar* data,
   const std::size_t size, const IndexType* dimensions,
   const ViewBase<IndexType>* pointers, const ViewBase<IndexType>* indices,
   const IndexType lds) :
...
// 2D accessor
template <class Scalar, class... Args>
const Scalar& View<Scalar, Attributes<Args...>>::operator()(const IndexType i,
   const IndexType j) const
{
   static_assert(_rank == 2, "This method is available only to rank 2 Views");
   assert(i < _dimensions[0]);
   assert(j < _dimensions[1]);
   if constexpr (std::is_same_v<LevelType,
                    DimLevelType<sparse_t, compressed_t>>)
   {
      // CSR
      static_assert(std::is_same_v<OrderType, LoopOrderType<i_t, j_t>>,
         "only default mem layout");
      ...
      throw std::logic_error("Trying to refer to an implicit zero.");
                         ...
                         DimLevelType<compressed_t, sparse_t>>)
   {
      // CSC
      static_assert(std::is_same_v<OrderType, LoopOrderType<i_t, j_t>>,
         "only default mem layout");
...
template <class Scalar, class... Args>
Scalar& View<Scalar, Attributes<Args...>>::operator()(const IndexType i,
   const IndexType j)
{
   ...
   const IndexType j, const IndexType k) const
{
   ...
   if constexpr (std::is_same_v<LevelType,
                    DimLevelType<dense_t, dense_t, compressed_t>>)
   {
      ...
      else if constexpr (std::is_same_v<OrderType,
      ...
      throw std::logic_error("Trying to refer to an implicit zero.");
                         ...
                         DimLevelType<dense_t, compressed_t, sparse_t>>)
   {
      if constexpr (std::is_same_v<OrderType, LoopOrderType<i_t, j_t, k_t>> ||
                    std::is_same_v<OrderType,
                       LoopOrderType<iInOrder_t, j_t, k_t>>)
      {
         ...
         // map i if needed
         IndexType ii = i;
         if constexpr (std::is_same_v<iInOrder_t,
                          std::variant_alternative_t<0, OrderType>>)
         {
         ...
         for (IndexType idn = _pointers[1][k]; idn < _pointers[1][k + 1]; ++idn)
         {
            if (_indices[1][idn] == j)
            {
               ...
               for (IndexType kk = 0; kk < k; ++kk)
               {
                  ...
                  kmn +=
                     _dimensions[0] * (_pointers[1][kk + 1] - _pointers[1][kk]);
               }
      ...
      throw std::logic_error("Trying to refer to an implicit zero.");
                         ...
                         DimLevelType<step1K_t, dense_t, dense_t>>)
   {
      if constexpr (std::is_same_v<OrderType, LoopOrderType<i_t, j_t, k_t>>)
      {
         // column major
      ...
      throw std::logic_error("Trying to refer to an implicit zero.");
                         ...
                         DimLevelType<step1K_t, dense_t, compressed_t>>)
   {
      if constexpr (std::is_same_v<OrderType, LoopOrderType<i_t, j_t, k_t>>)
      {
         // column major
      ...
      throw std::logic_error("Trying to refer to an implicit zero.");
                         ...
                         DimLevelType<dense_t, step1K_t, compressed_t>>)
   {
      if constexpr (std::is_same_v<OrderType, LoopOrderType<i_t, j_t, k_t>>)
      {
         // column major
      ...
      throw std::logic_error("Trying to refer to an implicit zero.");
                         ...
                         DimLevelType<step1K_t, compressed_t, sparse_t>>)
   {
      if constexpr (std::is_same_v<OrderType, LoopOrderType<i_t, j_t, k_t>>)
      {
         // column major
