/**
 * @file common.hpp
 * @brief Functions common to all DenseSM tests
 */

#ifndef QUICC_TESTSUITE_DENSESM_TESTERBASE_HPP
#define QUICC_TESTSUITE_DENSESM_TESTERBASE_HPP
...
namespace DenseSM {
   ...
   template <typename TOp> TesterBase<TOp>::TesterBase(const std::string& fname, const bool keepData, const bool usePointScale)
      : mKeepData(keepData), mUsePointScale(usePointScale), mMaxUlp(11), mIter(1),  mEpsilon(std::numeric_limits<MHDFloat>::epsilon()), mBasename(fname), mPath("DenseSM/")
   {
   }
...
#endif //QUICC_TESTSUITE_DENSESM_TESTERBASE_HPP
