    ...
    IProjCrossOperator(nNr, nNc, lOut, mOut, lA, mA, lB, mB, alpha, dBeta)
{
   ...
   {
      this->mpOpA = std::make_shared<DivR2FD1R1>(nNr, nNc, lOut, mOut, lA, mA, lB, mB, pTorA, alpha, dBeta);
      this->mpOpB = std::make_shared<DivR1D1F>(nNr, nNc, lOut, mOut, lA, mA, lB, mB, pTorA, alpha, dBeta);
   }
   ...
   else if(pPolB && pTorA == nullptr)
   {
      ...
      this->mpOpB = std::make_shared<DivR1D1F>(nNr, nNc, lOut, mOut, lB, mB, lA, mA, pPolB, alpha, dBeta);
   }
   else
   {
