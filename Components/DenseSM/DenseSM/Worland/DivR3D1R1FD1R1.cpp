...
DivR3D1R1FD1R1::DivR3D1R1FD1R1(const int nNr, const int nNc, const int lOut,
   ...
   const int cols) const
{
   ...
   W.compute<Internal::MHDFloat>(opFFFwd, opFFFwd.cols(),
      this->mLin + this->mLf - 2, igrid, iweights, ev::Set());
   Internal::Matrix opFFBwd(igrid.size(), this->cols() + this->mpF->nN());
   r_1W.compute<Internal::MHDFloat>(opFFBwd, opFFBwd.cols(),
      this->mLin + this->mLf - 2, igrid, Internal::Array(), ev::Set());
   ...
   mat = opFwd.transpose() * opFFBwd * opFFFwd.transpose() *
         (f.asDiagonal() * opBwd);
}
