    ...
    IProjCrossOperator(nNr, nNc, q, lOut, mOut, lA, mA, lB, mB, alpha, dBeta)
{
   ...
   {
      this->mpOpA = std::make_shared<DivR2D1R1F>(nNr + 2*q, nNc, lOut, mOut, lA, mA,
   ...
   {
      this->mpOpA = std::make_shared<DivR2FD1R1>(nNr + 2*q, nNc, lOut, mOut, lB, mB,
         lA, mA, pTorB, alpha, dBeta);
