...
// Project includes
//
#include "DenseSM/Worland/ProjCurlCurlPolCrossTor.hpp"
    ...
    IProjCrossOperator(nNr, nNc, lOut, mOut, lA, mA, lB, mB, alpha, dBeta)
{
   ...
   {
      this->mpOpA = std::make_shared<DivR2D1R1F>(nNr, nNc, lOut, mOut, lA, mA, lB, mB, pPolA, alpha, dBeta);
      this->mpOpB = std::make_shared<DivR1D1F>(nNr, nNc, lOut, mOut, lA, mA, lB, mB, pPolA, alpha, dBeta);
   }
   // Radial function B is given
   else if(pTorB && pPolA == nullptr)
   {
      this->mpOpA = std::make_shared<DivR2FD1R1>(nNr, nNc, lOut, mOut, lB, mB, lA, mA, pTorB, alpha, dBeta);
      this->mpOpB = std::make_shared<DivR1D1F>(nNr, nNc, lOut, mOut, lB, mB, lA, mA, pTorB, alpha, dBeta);
   }
   else
   {
