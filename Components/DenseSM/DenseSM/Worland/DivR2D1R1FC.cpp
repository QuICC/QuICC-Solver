 ...
 * @brief Source of the implementation of the spectral operator 1/r D(r f)
 * (-lapl(*))/r
 */
...
DivR2D1R1FC::DivR2D1R1FC(const int nNr, const int nNc, const int lOut,
   ...
   const int cols) const
{
   ...
   slaplW.compute<Internal::MHDFloat>(opCIBwd, opCIBwd.cols(), this->mLin,
      igrid, Internal::Array(), ev::Set());
   opCIBwd = -opCIBwd;

   Internal::Matrix opIFwd(igrid.size(), this->cols());
   W.compute<Internal::MHDFloat>(opIFwd, opIFwd.cols(), this->mLin, igrid,
      iweights, ev::Set());

   Internal::Matrix opBwd(igrid.size(), this->cols());
   r_1W.compute<Internal::MHDFloat>(opBwd, opBwd.cols(), this->mLin, igrid,
