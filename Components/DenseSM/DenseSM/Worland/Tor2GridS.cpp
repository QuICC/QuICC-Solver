...
namespace Worland {
   ...
   Tor2GridS::Tor2GridS(const int nN, const int nL, const int nS, const int nZ, const int nNug, const ArrayI& nli, const int nCpu, const Scalar_t ugAlpha, const Scalar_t ugDBeta, const Scalar_t alphaA, const Scalar_t dBetaA, const Scalar_t alphaB, const Scalar_t dBetaB, const bool isTriangular)
   ...
   void Tor2GridS::buildOpImpl(Internal::Matrix& mat, const int rows, const int cols) const
   {
   ...
   for (int l = 1; l < nL; l += 2)
   {
      if (QuICCEnv().id() == (pid % nCpu))
      {
         if (this->mIsTriangular)
         {
            ...
            Internal::Matrix iintgz_misfit;
            if (this->isUgBasis())
            {
               this->intgzWorland(l, nli(l), iintgz_misfit, igridsB, igridz,
                  iweightz);
            }
            else
            {
               iintgz_misfit = iintgz;
            }
            mat.block(0, l * nN, nS, nli(l) + 1) =
               Internal::cast(iintgz_misfit);
         }
         else
         {
            Internal::Matrix iintgz;
            this->intgzWorland(l, nN - 1, iintgz, igrids, igridz, iweightz);
            mat.block(0, l * nN, nS, nN) =
               Internal::cast(iintgz);
         }
      }
      pid++;
   }
...
void Tor2GridS::computeQuadraturez(Internal::Array& igridz,
...
void Tor2GridS::intgzWorland(int l, int n,
