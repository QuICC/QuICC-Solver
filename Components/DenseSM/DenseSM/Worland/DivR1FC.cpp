...
DivR1FC::DivR1FC(const int nNr, const int nNc, const int lOut, const int mOut, const int lF, const int mF, const int lIn, const int mIn,
...
void DivR1FC::buildOpImpl(Internal::Matrix& mat, const int rows,
   const int cols) const
{
   ...
   const int nR = (3*(2*this->rows() + std::max(this->mLin, this->mLout) + 2*this->mpF->nN() + this->mLf))/2;
   ...
   slaplW.compute<Internal::MHDFloat>(opBwd, opBwd.cols(), this->mLin, igrid, Internal::Array(), ev::Set());
   opBwd = -opBwd;
   ...
   r_1W.compute<Internal::MHDFloat>(opFBwd, opFBwd.cols(), lF, igrid, Internal::Array(), ev::Set());
