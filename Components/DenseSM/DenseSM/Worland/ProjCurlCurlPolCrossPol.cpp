    ...
    IProjCrossOperator(nNr, nNc, lOut, mOut, lA, mA, lB, mB, alpha, dBeta)
{
   ...
   {
      this->mpOpA = std::make_shared<DivR1D1DivR1FD1R1>(nNr, nNc, lOut, mOut,
         lA, mA, lB, mB, pPolA, alpha, dBeta);
      this->mpOpB = std::make_shared<DivR1D1DivR1D1R1F>(nNr, nNc, lOut, mOut,
         lA, mA, lB, mB, pPolA, alpha, dBeta);
      this->mpOpC = std::make_shared<DivR3D1R1FD1R1>(nNr, nNc, lOut, mOut, lA,
         mA, lB, mB, pPolA, alpha, dBeta);
   }
   // Radial function B is given
   else if (pPolB && pPolA == nullptr)
   {
      this->mpOpA = std::make_shared<DivR1D1DivR1D1R1F>(nNr, nNc, lOut, mOut,
         lB, mB, lA, mA, pPolB, alpha, dBeta);
      this->mpOpB = std::make_shared<DivR1D1DivR1FD1R1>(nNr, nNc, lOut, mOut,
         lB, mB, lA, mA, pPolB, alpha, dBeta);
      this->mpOpC = std::make_shared<DivR3D1R1FD1R1>(nNr, nNc, lOut, mOut, lB,
