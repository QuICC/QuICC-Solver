 ...
 * @brief Source of the implementation of the spectral operator 1/r (-lapl(f))/r D(r *)
 */
...
DivR2CFD1R1::DivR2CFD1R1(const int nNr, const int nNc, const int lOut, const int mOut, const int lF, const int mF, const int lIn, const int mIn,
   ...
   const int cols) const
{
   ...
   r_1drW.compute<Internal::MHDFloat>(opBwd, opBwd.cols(), this->mLin, igrid, Internal::Array(), ev::Set());
   ...
   slaplW.compute<Internal::MHDFloat>(opCBwd, opCBwd.cols(), this->mLf, igrid, Internal::Array(), ev::Set());
   opCBwd = -opCBwd;
   Internal::Matrix opFBwd(igrid.size(), this->mpF->nN());
   r_1W.compute<Internal::MHDFloat>(opFBwd, opFBwd.cols(), this->mLf, igrid, Internal::Array(), ev::Set());
   
   Internal::Matrix opFwd(igrid.size(), this->rows());
   W.compute<Internal::MHDFloat>(opFwd, opFwd.cols(), this->mLout, igrid, iweights, ev::Set());

   Internal::Array f = opFBwd * (opFFwd.transpose() * (opCBwd * (opFFwd.transpose() * this->mpF->evaluate(igrid, this->mLf, this->mMf))));
