    ...
    IProjCrossOperator(nNr, nNc, lOut, mOut, lA, mA, lB, mB, lower, upper)
{
   ...
   if (pPolA && pPolB == nullptr)
   {
      if (p == 4)
      {
         this->mpOpA = std::make_shared<R4DivR1D1DivR1FD1R1>(nNr, nNc, lOut,
            mOut, lA, mA, lB, mB, pPolA, lower, upper);
         this->mpOpB = std::make_shared<R4DivR1D1DivR1D1R1F>(nNr, nNc, lOut,
            mOut, lA, mA, lB, mB, pPolA, lower, upper);
         this->mpOpC = std::make_shared<R4DivR3D1R1FD1R1>(nNr, nNc, lOut, mOut,
   ...
   else if (pPolB && pPolA == nullptr)
   {
      if (p == 4)
      {
         this->mpOpA = std::make_shared<R4DivR1D1DivR1D1R1F>(nNr, nNc, lOut,
            mOut, lB, mB, lA, mA, pPolB, lower, upper);
         this->mpOpB = std::make_shared<R4DivR1D1DivR1FD1R1>(nNr, nNc, lOut,
            mOut, lB, mB, lA, mA, pPolB, lower, upper);
         this->mpOpC = std::make_shared<R4DivR3D1R1FD1R1>(nNr, nNc, lOut, mOut,
