 ...
 * @brief Source of the implementation of the spectral operator r^4 1/r D(r f) (-lapl(*))/r
 */
...
R4DivR2D1R1FC::R4DivR2D1R1FC(const int nNr, const int nNc, const int lOut, const int mOut, const int lF, const int mF, const int lIn, const int mIn,
   ...
   const int cols) const
{
   ...
   tA = -((f.array()+d1f.array()*r.cast<MHDFloat>().array()).matrix().asDiagonal()*(r.cast<MHDFloat>().asDiagonal()*(2.0*td1B+r.cast<MHDFloat>().asDiagonal()*td2B)- l*(1.0+l)*tB));
