/**
 * @file R4DivR1CF.cpp
 * @brief Source of the implementation of the spectral operator r^4 1/r (-lapl(f)) *
 */
...
R4DivR1CF::R4DivR1CF(const int nNr, const int nNc, const int lOut, const int mOut, const int lF, const int mF, const int lIn, const int mIn,
...
void R4DivR1CF::buildOpImpl(Internal::Matrix& mat, const int rows,
   const int cols) const
{
   ...
   tA = (r.array()*(f.array()*l*(1+l) - r.array()*(2*d1f.array()+d2f.array()*r.array()))).cast<MHDFloat>().matrix().asDiagonal()*tB;
