 ...
 * @brief Source of the implementation of the spectral operator r^4 1/r (-lapl(f))/r D(r *)
 */
...
R4DivR2CFD1R1::R4DivR2CFD1R1(const int nNr, const int nNc, const int lOut, const int mOut, const int lF, const int mF, const int lIn, const int mIn,
   ...
   const int cols) const
{
   ...
   tA = -((-f.array()*l*(1+l)+r.cast<MHDFloat>().array()*(2.0*d1f.array()+d2f.array()*r.cast<MHDFloat>().array())).matrix().asDiagonal()*(tB+r.cast<MHDFloat>().asDiagonal()*td1B));
