/**
 * @file R4FC2.cpp
 * @brief Source of the implementation of the spectral operator r^4 f lapl2(*)
 * 
 * Copied from R4DivR1FC -> r^4 1/r f (-lapl(*))
 */
...
// Project includes
//
#include "DenseSM/Chebyshev/LinearMap/R4FC2.hpp"
...
R4FC2::R4FC2(const int nNr, const int nNc, const int lOut, const int mOut, const int lF, const int mF, const int lIn, const int mIn,
...
void R4FC2::buildOpImpl(Internal::Matrix& mat, const int rows,
   const int cols) const
{
   ...
   // NEED TO IMPLEMENT
   // operator:   f * r^4 * (lapl2)
   //                = f * r^4 * (laplH + (2*D1/r+ D2))*(-laplH - (2*D1/r+ D2)) = 
   //                =  f* ( (-2+L2)*L2 ) + 2*L2*r^2*D2 - r^3*( 4*D3 + r*D4 )
   // RIGHT NOW IT S THE R4DivR1FC FORMULA
   tA = (f.array()*r.array()).cast<MHDFloat>().matrix().asDiagonal()*(l*(1+l)*tB - r.cast<MHDFloat>().asDiagonal()*(2.0*td1B + r.cast<MHDFloat>().asDiagonal()*td2B));
