/**
 * @file IqRpDivR2D1R1F.cpp
 * @brief Source of the implementation of the spectral operator I^q r^p 1/r^2 D(r f)
 * *
 */
...
IqRpDivR2D1R1F::IqRpDivR2D1R1F(const int nNr, const int nNc, const int q, const int p, const int lOut,
...
void IqRpDivR2D1R1F::buildOpImpl(Internal::Matrix& mat, const int rows,
   const int cols) const
{
   int rN = this->mpF->nN() + this->mP - 1;
   ...
   Matrix tf =
      (f.array() + d1f.array()*r.array())
         .cast<MHDFloat>();
   if(this->mP > 2)
   {
      tf = (r.array().pow(this->mP-2)*tf.array()).cast<MHDFloat>();
   }
