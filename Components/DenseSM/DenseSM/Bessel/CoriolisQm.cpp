...
#include "QuICC/Polynomial/Bessel/raiseSphJnl.hpp"
...
namespace Bessel {
   ...
   CoriolisQm::CoriolisQm(const Internal::MHDFloat outDNu, const Internal::MHDFloat inDNu, const int rows, const int cols, const int l)
      : IEmbeddedSMOperator(rows, cols), mOutDNu(outDNu), mInDNu(inDNu), mL(l)
   {
   }

   void CoriolisQm::buildOpImpl(Internal::Matrix& mat, const int rows, const int cols) const
   {
      this->buildGenericOp(mat, rows, cols);
   }

   void CoriolisQm::buildGenericOp(Internal::Matrix& mat, const int rows, const int cols) const
   {
      ...
      Polynomial::Bessel::Generic<Polynomial::Bessel::raiseSphJnl> qmJnl(this->mInDNu);

      Internal::Matrix tmpBwd(igrid.size(), rows);
      qmJnl.compute<Internal::MHDFloat>(tmpBwd, rows, l-1, igrid, Internal::Array());
