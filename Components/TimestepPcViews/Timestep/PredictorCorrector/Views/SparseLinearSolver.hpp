...
#ifndef QUICC_TIMESTEP_PREDICTORCORRECTOR_VIEWS_SPARSELINEARTIMESTEP_PREDICTORCORRECTOR_VIEWS_HPP
#define QUICC_TIMESTEP_PREDICTORCORRECTOR_VIEWS_SPARSELINEARTIMESTEP_PREDICTORCORRECTOR_VIEWS_HPP
...
#include "Timestep/PredictorCorrector/Views/SparseSolverBase.hpp"
...
namespace Views {
   ...
   template <typename TOperator,typename TData,template <typename> class TSolver> void SparseLinearSolver<TOperator,TData,TSolver>::addInhomogeneous()
   {
      for(size_t i = 0; i < this->nSystem(); i++)
      {
         ...
         {
            details::addCorrection(this->reg(Register::Rhs::id()).at(i), this->mInhomogeneous.at(i));
         }
      }
   }
...
#endif // QUICC_TIMESTEP_PREDICTORCORRECTOR_VIEWS_SPARSELINEARTIMESTEP_PREDICTORCORRECTOR_VIEWS_HPP
