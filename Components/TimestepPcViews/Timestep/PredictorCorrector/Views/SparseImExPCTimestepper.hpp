...
#ifndef QUICC_TIMESTEP_PREDICTORCORRECTOR_VIEWS_SPARSEIMEXPCTIMESTEPPER_HPP
#define QUICC_TIMESTEP_PREDICTORCORRECTOR_VIEWS_SPARSEIMEXPCTIMESTEPPER_HPP
...
#include "Timestep//PredictorCorrector/Views/ISparseTimestepper.hpp"
...
template <typename TOperator, typename TData, template <typename> class TSolver>
SparseImExPCTimestepper<TOperator, TData, TSolver>::SparseImExPCTimestepper(
   const int start, const std::size_t timeId) :
    ISparseTimestepper<TOperator, TData, TSolver>(start, timeId)
{}

template <typename TOperator, typename TData, template <typename> class TSolver>
void SparseImExPCTimestepper<TOperator, TData, TSolver>::setScheme(
...
template <typename TOperator, typename TData, template <typename> class TSolver>
int SparseImExPCTimestepper<TOperator, TData, TSolver>::steps() const
{
...
template <typename TOperator, typename TData, template <typename> class TSolver>
MHDFloat
SparseImExPCTimestepper<TOperator, TData, TSolver>::stepFraction() const
{
   return this->mspScheme->cEx(this->mStep);
}

template <typename TOperator, typename TData, template <typename> class TSolver>
MHDFloat SparseImExPCTimestepper<TOperator, TData, TSolver>::aIm(
...
template <typename TOperator, typename TData, template <typename> class TSolver>
void SparseImExPCTimestepper<TOperator, TData, TSolver>::initSolutions()
{
...
template <typename TOperator, typename TData, template <typename> class TSolver>
void SparseImExPCTimestepper<TOperator, TData, TSolver>::updateSolutions()
{
   for (size_t i = this->mZeroIdx; i < this->nSystem(); i++)
   {
      details::computeSet(this->reg(Register::Intermediate::id()).at(i),
...
template <typename TOperator, typename TData, template <typename> class TSolver>
void SparseImExPCTimestepper<TOperator, TData, TSolver>::addStorage(
   const int rows, const int cols)
{
   ...
   std::vector<std::size_t> ids = {Register::Error::id(),
...
template <typename TOperator, typename TData, template <typename> class TSolver>
bool SparseImExPCTimestepper<TOperator, TData, TSolver>::preSolve()
{
   const bool hasInfluence =
      this->hasSolverMatrix(Tag::Operator::Influence::id());
   ...
   if (this->mStep == 0)
   {
      ...
      for (size_t i = this->mZeroIdx; i < this->nSystem(); i++)
      {
         ...
         details::computeAMXPY(this->reg(Register::Rhs::id()).at(i),
            this->mMassMatrix.at(i), aMass,
            this->reg(Register::Intermediate::id()).at(i));
         details::computeAMXPY(this->reg(Register::Rhs::id()).at(i),
            this->mRHSMatrix.at(i), aIm,
            this->reg(Register::Intermediate::id()).at(i));
      }
   ...
   else
   {
      ...
      for (size_t i = this->mZeroIdx; i < this->nSystem(); i++)
      {
         ...
         details::computeXPAY(this->reg(Register::Rhs::id()).at(i),
            this->reg(Register::Error::id()).at(i), 1.0);
      }
...
template <typename TOperator, typename TData, template <typename> class TSolver>
bool SparseImExPCTimestepper<TOperator, TData, TSolver>::postSolve()
{
   const bool hasInfluence =
      this->hasSolverMatrix(Tag::Operator::Influence::id());
      ...
      else
      {
         ...
         for (std::size_t i = this->mZeroIdx; i < this->nSystem(); i++)
         {
            details::computeInfluenceCorrection(
               this->reg(Register::Solution::id()).at(i),
               this->reg(Register::Influence::id()).at(i));
         }
      }
   }
   ...
   else
   {
      ...
      for (size_t i = this->mZeroIdx; i < this->nSystem(); i++)
      {
         ...
         details::computeXPAY(this->reg(Register::Solution::id()).at(i),
            this->reg(Register::Intermediate::id()).at(i), 1.0);
         details::computeSet(this->reg(Register::Intermediate::id()).at(i),
            this->reg(Register::Solution::id()).at(i));
      }
...
#endif // QUICC_TIMESTEP_PREDICTORCORRECTOR_VIEWS_SPARSEIMEXPCTIMESTEPPER_HPP
