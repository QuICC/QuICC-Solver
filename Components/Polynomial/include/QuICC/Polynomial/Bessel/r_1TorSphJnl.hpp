/**
 * @file r_1TorSphJnl.hpp
 * @brief Implementation of the spherical Bessel basis for magnetic toroidal boundary conditions
 */

#ifndef QUICC_POLYNOMIAL_BESSEL_R_1TORSPHJNL_HPP
#define QUICC_POLYNOMIAL_BESSEL_R_1TORSPHJNL_HPP
...
namespace Bessel {
   ...
   template <typename T> inline void r_1TorSphJnl::compute(Eigen::Ref<Eigen::Matrix<T,Eigen::Dynamic,Eigen::Dynamic> > rOut, const int nPoly, const int lIn, const Internal::Array& igrid, const Internal::Array& scale)
   {
      ...
      for(int j = 0; j < nPoly; j++)
      {
         ...
         for(int i = 0; i < igrid.size(); i++)
         {
            ...
            rOut(i,j) = boost::math::sph_bessel(lIn, k*r)/r;
         }
      }
   }
...
#endif // QUICC_POLYNOMIAL_BESSEL_R_1TORSPHJNL_HPP
