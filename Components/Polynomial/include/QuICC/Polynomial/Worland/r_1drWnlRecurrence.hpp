/**
 * @file r_1drWnlRecurrence.hpp
 * @brief Implementation of the 1/r D r Worland polynomial
 */

#ifndef QUICC_POLYNOMIAL_WORLAND_R_1DRWNLRECURRENCE_HPP
#define QUICC_POLYNOMIAL_WORLAND_R_1DRWNLRECURRENCE_HPP
...
namespace Worland {
   ...
   /**
    * @brief Implementation of the 1/r D r Worland polynomial with recurrence relation
    */
   template <>
   class r_1drWnl<recurrence_t>: public WorlandBase
   {
      public:
   ...
   template <typename T, typename TEvaluator> void r_1drWnl<recurrence_t>::compute(Eigen::Ref<Eigen::Matrix<T,Eigen::Dynamic,Eigen::Dynamic> > rOut, const int nPoly, const int l, const Internal::Array& igrid, const Internal::Array& scale, TEvaluator evaluator)
   {
      int gN = igrid.rows();

      if(l < 0)
      {
         throw std::logic_error("Tried to compute Worland 1/r D r operator with l < 0");
      }
...
#endif // QUICC_POLYNOMIAL_WORLAND_R_1DRWNLRECURRENCE_HPP
