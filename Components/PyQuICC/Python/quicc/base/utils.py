...
def build_diagonals(ns, nzrow, ds, offsets, cross_parity = None, has_wrap = True):
    ...
    if has_wrap:
        ...
        for d,f in enumerate(ds):
            lb = max(0, -offsets[d])
            for i in range(0, min(lb,len(ns))):
                if ns[i] > nzrow:
                    lbw = sum(offsets < 0)
                    step = (ns[1]-ns[0])
    ...
    for d,f in enumerate(ds):
        ...
        if izrow > lb:
            if ub - izrow > 0:
                diags[d-r] = np.concatenate((np.zeros(izrow-lb), f(ns[izrow:ub])))
            elif izrow-lb > 0 and ub > 0:
                diags[d-r] = (np.zeros(min(izrow-lb,ub)))
            else:
                small_resolution = True
        else:
            if ub - lb > 0:
                diags[d-r] = f(ns[lb:ub])
            else:
                small_resolution = True
        if small_resolution:
            diags.pop()
            ...
            if has_wrap:
                if len(wrap[d]) > 0:
                    diags[d-r][0:len(wrap[d])] = [x + y for x, y in zip(diags[d-r][0:len(wrap[d])], wrap[d][:])]
    offs = np.array(offs)
...
def nparr2list(mat):
    """Convert an numpy array into a list, column-major"""
