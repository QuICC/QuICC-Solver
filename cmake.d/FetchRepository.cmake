function(quicc_set_default_git_branch name)
  ...
  set(_QUICC_${_type}_GIT_BRANCH_${name} "${QSGB_BRANCH}")

  set(QUICC_${_type}_GIT_BRANCH_${name}
    ${_QUICC_${_type}_GIT_BRANCH_${name}} CACHE STRING " ${QSGB_TYPE} repository branch.")
endfunction()

function(quicc_fetch_repository name)
  # parse inputs
  set(oneValueArgs TYPE MAKE_AVAILABLE)
  cmake_parse_arguments(QFR "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
  string(TOUPPER "${QFR_TYPE}" _type)

  message(DEBUG "name: ${name}")
  if(NOT QFR_MAKE_AVAILABLE)
    set(QFR_MAKE_AVAILABLE OFF)
  endif()
  message(DEBUG "QFR_MAKE_AVAILABLE: ${QFR_MAKE_AVAILABLE}")
  ...
  if(NOT _name_exist)
      ...
      elseif(QUICC_${_type}_GIT_REPO_BASE)
          ...
          set(_repo_base ${_default_name_git_repo_base})
      endif()
  ...
  if(QFR_MAKE_AVAILABLE)
    message(STATUS "MAKE AVAILABLE ${name}")
    # if the model folder exists with a valid CMakeLists.txt, add it
    if(NOT EXISTS ${_name_path}/CMakeLists.txt)
        message(FATAL_ERROR "${_name_path} exists, but does not contain a valid model.")
    endif()
    ...
    if(QUICC_${_type}_GIT_BRANCH_${name})
        ...
        if(${_pos} LESS 0)
            message(WARNING "${name} default (${_repo_branch}) and existing model branch (${_name_branch}) do not match")
        endif()
    endif()
